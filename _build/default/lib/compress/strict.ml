...
struct
    ...
    *)
    (* GEN BEGIN TAG INSIDE LET *) fun eta_contract_var (Elt t) = eta_contract_var' 0 t
    ...
    and eta_contract_var' n (ATerm(ARoot(Var n', s))) = 
      ...
      | eta_contract_var' _ _ = raise EtaContract (* GEN END TAG INSIDE LET *)

    (* GEN BEGIN TAG INSIDE LET *) fun pattern_spine' (D, []) = true
    ...
    (* GEN BEGIN TAG INSIDE LET *) fun pattern_spine (D, s) = 
    	((pattern_spine' (D, map eta_contract_var s))
    	 handle EtaContract => false) (* GEN END TAG INSIDE LET *)


    (* GEN BEGIN TAG INSIDE LET *) fun spine_occ n (D, nil) = false
    ...
    and type_occ n (D, TRoot (_,s)) = spine_occ n (D, s)
      | type_occ n (D, TPi (_, a, b)) = type_occ n (D, a) orelse
          (* PERF: suspend these context shifts, do them at the end *)
      					type_occ (n+1) (0 :: (map (fn x => x+1) D), b) (* GEN END TAG INSIDE LET *)

    (* toplevel strictness judgments *)
    (* GEN BEGIN TAG INSIDE LET *) fun check_strict_type' n p (TRoot(n', s)) = if p then false else spine_occ n ([], s)
      | check_strict_type' n p (TPi(PLUS,a,b)) = type_occ n ([],a) orelse
      					       check_strict_type' (n+1) p b
      | check_strict_type' n p (TPi(_,a,b)) = check_strict_type' (n+1) p b (* GEN END TAG INSIDE LET *)

    (* GEN BEGIN TAG INSIDE LET *) fun check_strict_kind' n Type = false
      | check_strict_kind' n (KPi(PLUS,a,k)) = type_occ n ([],a) orelse
      					       check_strict_kind' (n+1) k
      | check_strict_kind' n (KPi(_,a,k)) = check_strict_kind' (n+1) k (* GEN END TAG INSIDE LET *)


(* p is whether we should imagine we are checking a c+ (rather than c-) constant *)
    (* GEN BEGIN TAG INSIDE LET *) fun check_strict_type p b = check_strict_type' 0 p b (* GEN END TAG INSIDE LET *)
    (* GEN BEGIN TAG INSIDE LET *) fun check_strict_kind k = check_strict_kind' 0 k (* GEN END TAG INSIDE LET *)
end