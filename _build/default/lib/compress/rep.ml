...
(* GEN BEGIN TAG OUTSIDE LET *) val k : Reductio.eq_c option ref = ref NONE (* GEN END TAG OUTSIDE LET *)
...
fun sanityCheck cid = ((case I.sgnLookup cid of 
 ...
 | I.ConDef(_,_,_,_,_,I.Type,_) => let (* GEN BEGIN TAG OUTSIDE LET *) val Sgn.DEF_TERM y = Sgn.def cid (* GEN END TAG OUTSIDE LET *)
				     (* GEN BEGIN TAG OUTSIDE LET *) val Syntax.tclass z = Sgn.classifier cid (* GEN END TAG OUTSIDE LET *) 
				 in
(*				     l := (y,z):: !l; *)
				     Reductio.check ([], y, z) 
				 end
 | I.ConDef(_,_,_,_,_,I.Kind,_) => let (* GEN BEGIN TAG OUTSIDE LET *) val Sgn.DEF_TYPE y = Sgn.def cid (* GEN END TAG OUTSIDE LET *)
				     (* GEN BEGIN TAG OUTSIDE LET *) val Syntax.kclass z = Sgn.classifier cid (* GEN END TAG OUTSIDE LET *) 
				 in
				     Reductio.check_type  Reductio.CON_LF (Syntax.explodeKind z, y) 
				 end
 | I.AbbrevDef(_,_,_,_,_,I.Type) => let (* GEN BEGIN TAG OUTSIDE LET *) val Sgn.DEF_TERM y = Sgn.def cid (* GEN END TAG OUTSIDE LET *)
				     (* GEN BEGIN TAG OUTSIDE LET *) val Syntax.tclass z = Sgn.classifier cid (* GEN END TAG OUTSIDE LET *) 
				 in
(*				     l := (y,z):: !l; *)
				     Reductio.check ([], y, z) 
				 end
 | I.AbbrevDef(_,_,_,_,_,I.Kind) => let (* GEN BEGIN TAG OUTSIDE LET *) val Sgn.DEF_TYPE y = Sgn.def cid (* GEN END TAG OUTSIDE LET *)
				     (* GEN BEGIN TAG OUTSIDE LET *) val Syntax.kclass z = Sgn.classifier cid (* GEN END TAG OUTSIDE LET *) 
				 in
				     Reductio.check_type Reductio.CON_LF (Syntax.explodeKind z, y) 
				 end
...
fun gen_graph n autoCompress =
    let
	(* GEN BEGIN TAG OUTSIDE LET *) val _ = autoCompress n (* GEN END TAG OUTSIDE LET *) 
	fun sanity n = if n < 0 then true else 
		       (sanity (n-1) andalso (if sanityCheck n then true else (print ("insane: <" ^ (Int.toString n) ^ ">\n"); raise Crap)) )
	(* GEN BEGIN TAG OUTSIDE LET *) val _ = sanity n (* GEN END TAG OUTSIDE LET *) 
	(* GEN BEGIN TAG OUTSIDE LET *) val pairs = List.tabulate (n+1, ((* GEN BEGIN FUNCTION EXPRESSION *) fn x => ( o_cidSize x, cidSize x) (* GEN END FUNCTION EXPRESSION *) )) (* GEN END TAG OUTSIDE LET *)
	(* GEN BEGIN TAG OUTSIDE LET *) val s = foldl op^ "" 
		      (map ((* GEN BEGIN FUNCTION EXPRESSION *) fn (x,y) => 
			       Int.toString x ^ " " ^ Int.toString y ^ "\n" (* GEN END FUNCTION EXPRESSION *) ) pairs) (* GEN END TAG OUTSIDE LET *) 
	(* GEN BEGIN TAG OUTSIDE LET *) val f = TextIO.openOut "/tmp/graph" (* GEN END TAG OUTSIDE LET *)
	(* GEN BEGIN TAG OUTSIDE LET *) val _ = TextIO.output(f,s) (* GEN END TAG OUTSIDE LET *)
	(* GEN BEGIN TAG OUTSIDE LET *) val _ = TextIO.closeOut(f) (* GEN END TAG OUTSIDE LET *)
    in
	()
    end
