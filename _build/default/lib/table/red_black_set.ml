...
struct
 ...
 datatype 'a dict =
  ...
  (* GEN BEGIN TAG INSIDE LET *) fun isEmpty (Set(_, Empty)) = true
    | isEmpty (Set(_,T)) = false (* GEN END TAG INSIDE LET *)

  (* GEN BEGIN TAG INSIDE LET *) val empty = Set(0, Empty) (* GEN END TAG INSIDE LET *)
  
  (* GEN BEGIN TAG INSIDE LET *) fun singleton x = Set(1, Red(x, Empty, Empty)) (* GEN END TAG INSIDE LET *)
  ...
  *)
  ...
  (* GEN BEGIN TAG INSIDE LET *) fun lookup (Set(n, dict)) key =
    let
      fun lk (Empty) = NONE
  	| lk (Red tree) = lk' tree
  ...
  (* GEN BEGIN TAG INSIDE LET *) fun last (Set(n, dict)) = (n, valOf (lookup (Set(n, dict)) n)) (* GEN END TAG INSIDE LET *)
  ...
  *)
  (* GEN BEGIN TAG INSIDE LET *) fun restore_right (Black(e, Red lt, Red (rt as (_,Red _,_)))) =
    ...
    | restore_right dict = dict (* GEN END TAG INSIDE LET *)

  (* restore_left is like restore_right, except *)
  (* the color invariant may be violated only at the root of left child *)
  (* GEN BEGIN TAG INSIDE LET *) fun restore_left (Black(e, Red (lt as (_,Red _,_)), Red rt)) =
  	 Red(e, Black lt, Black rt)	(* re-color *)
    ...
    | restore_left dict = dict (* GEN END TAG INSIDE LET *)

  (* GEN BEGIN TAG INSIDE LET *) fun insert (Set(n, dict), entry as (key, datum)) = 
    let      
      val nItems = ref n
  ...
  (* GEN BEGIN TAG INSIDE LET *) fun insertList (S, nil) = S
    | insertList (S, e::list) = insertList (insert (S, e), list) (* GEN END TAG INSIDE LET *)


  (* GEN BEGIN TAG INSIDE LET *) fun insertLast (Set(n, dict), datum) = 
    let
  ...
  (* GEN BEGIN TAG INSIDE LET *) fun insertShadow (Set(n, dict), entry as (key, datum)) =  
  ...
  (* Remove an item.  Raises LibBase.NotFound if not found. *)
    ...
    in
    (* GEN BEGIN TAG INSIDE LET *) fun delete (Set(nItems, t), k) = 
        let
    	  fun zip (Top, t) = t
  ...
  (* GEN BEGIN TAG INSIDE LET *) fun app f (Set(n, dict)) =
      let fun ap (Empty) = ()
  	    | ap (Red tree) = ap' tree
  ...
  (* GEN BEGIN TAG INSIDE LET *) fun update f (Set(n, dict)) =
  ...
  (* GEN BEGIN TAG INSIDE LET *) fun forall (Set(n, dict)) f =
      let fun ap (Empty) = ()
  	    | ap (Red tree) = ap' tree
  	    | ap (Black tree) = ap' tree
  ...
  (* GEN BEGIN TAG INSIDE LET *) fun existsOpt (Set(n, dict)) f =
  ...
  (* GEN BEGIN TAG INSIDE LET *) fun exists (Set(n, dict)) f =
  ...
  (* GEN BEGIN TAG INSIDE LET *) fun setsize (Set (n, _)) = n (* GEN END TAG INSIDE LET *)
   ...
   * to be visited.
   *)
    (* GEN BEGIN TAG INSIDE LET *) fun next ((t as Red( _, _, b))::rest) = (t, left(b, rest))
      | next ((t as Black( _, _, b))::rest) = (t, left(b, rest))
      | next _ = (Empty, [])
    and left (Empty, rest) = rest
      | left (t as Red(_, a, _), rest) = left(a, t::rest)
      | left (t as Black(_, a, _), rest) = left(a, t::rest) (* GEN END TAG INSIDE LET *)
    (* GEN BEGIN TAG INSIDE LET *) fun start m = left(m, []) (* GEN END TAG INSIDE LET *)
  ...
  (* add an item that is guaranteed to be larger than any in l *)
    (* GEN BEGIN TAG INSIDE LET *) fun addItem (a, l) = 
      let
  ...
  (* link the digits into a tree *)
    (* GEN BEGIN TAG INSIDE LET *) fun linkAll t = let
    	  fun link (t, ZERO) = t
    ...
    (* GEN BEGIN TAG INSIDE LET *) fun getEntry (Red (x, _, _)) = x
      | getEntry (Black (x, _, _)) = x (* GEN END TAG INSIDE LET *)

      
  (* return the union of the two sets *)
    (* GEN BEGIN TAG INSIDE LET *) fun union (Set (n1, s1), Set (n2, s2)) = 
      let
  ...
  (* return the intersection of the two sets *)
    (* GEN BEGIN TAG INSIDE LET *) fun intersection (Set(_, s1), Set(_, s2)) = 
      let
  ...
  (* return the set difference  S1 - S2 
     ...
     they are ignored !*)
    (* GEN BEGIN TAG INSIDE LET *) fun difference (Set(_, s1), Set(_, s2)) = 
      let
    	fun ins ((Empty, _), n, result) = (n, result)
    	  | ins ((Red (x, _, _), r), n, result) =
    	    ins(next r, n+1, addItem(x, result)) 
    ...
    (* returns difference (d1, d2) where d1 
     ...
     and d2 contains all elements occurring in S2 but not in S1
       *)
    (* GEN BEGIN TAG INSIDE LET *) fun difference2 (Set(_, s1), Set(_, s2)) = 
      let
    	fun ins ((Empty, _), n, result) = (n, result)
    	  | ins ((Red (x, _, _), r), n, result) =
    	    ins(next r, n+1, addItem(x, result)) 
    ...
    *)
    ...
    (* GEN BEGIN TAG INSIDE LET *) fun diffMod F (Set(_, s1), Set(_, s2)) = 
     let
    	fun ins ((Empty, _), n, result) = (n, result)
    	  | ins ((Red (x, _, _), r), n, result) =
    	    ins(next r, n+1, addItem(x, result)) 
    ...
    (* GEN BEGIN TAG INSIDE LET *) fun splitSets F (Set(_, s1), Set(_, s2)) = 
     let
    	fun ins ((Empty, _), n, result) = (n, result)
    	  | ins ((Red (x, _, _), r), n, result) =
    	    ins(next r, n+1, addItem(x, result)) 
  ...
  in
    (* GEN BEGIN TAG INSIDE LET *) fun new () = ref (empty) (* GEN END TAG INSIDE LET *) (* ignore size hint *)
    (* GEN BEGIN TAG INSIDE LET *) fun copy S = let val S' = new() in S' := (!S); S' end (* GEN END TAG INSIDE LET *)
    (* GEN BEGIN TAG INSIDE LET *) val insert = (fn set => fn entry => (set := insert (!set, entry))) (* GEN END TAG INSIDE LET *)
    (* GEN BEGIN TAG INSIDE LET *) val insertLast = (fn set => fn datum => (set := insertLast (!set, datum))) (* GEN END TAG INSIDE LET *)
    (* GEN BEGIN TAG INSIDE LET *) val insertList = (fn set => fn list => (set := insertList (!set, list))) (* GEN END TAG INSIDE LET *)
    (* GEN BEGIN TAG INSIDE LET *) val insertShadow = (fn set => fn entry => (set := insertShadow (!set, entry))) (* GEN END TAG INSIDE LET *)

    (* GEN BEGIN TAG INSIDE LET *) val isEmpty = (fn ordSet => isEmpty (!ordSet)) (* GEN END TAG INSIDE LET *)
    (* GEN BEGIN TAG INSIDE LET *) val last = (fn ordSet => last (!ordSet)) (* GEN END TAG INSIDE LET *)

    
    (* GEN BEGIN TAG INSIDE LET *) val lookup = (fn ordSet => fn key => lookup (!ordSet) key) (* GEN END TAG INSIDE LET *)

    (* GEN BEGIN TAG INSIDE LET *) val clear = (fn ordSet => (ordSet := empty)) (* GEN END TAG INSIDE LET *)

    (* GEN BEGIN TAG INSIDE LET *) val app = (fn ordSet => fn f => app f (!ordSet)) (* GEN END TAG INSIDE LET *)
    (* GEN BEGIN TAG INSIDE LET *) val update = (fn ordSet => fn f => ((ordSet := (update f (!ordSet)); ordSet))) (* GEN END TAG INSIDE LET *)
    (* GEN BEGIN TAG INSIDE LET *) val forall = (fn ordSet => fn f => forall (!ordSet) f) (* GEN END TAG INSIDE LET *)
    (* GEN BEGIN TAG INSIDE LET *) val exists = (fn ordSet => fn f => exists (!ordSet) f) (* GEN END TAG INSIDE LET *)
    (* GEN BEGIN TAG INSIDE LET *) val existsOpt = (fn ordSet => fn f => existsOpt (!ordSet) f) (* GEN END TAG INSIDE LET *)

    (* GEN BEGIN TAG INSIDE LET *) fun size S = setsize (!S) (* GEN END TAG INSIDE LET *) 

    (* GEN BEGIN TAG INSIDE LET *) val difference = (fn set1 => fn set2 => (let val set = new() in set := difference (!set1, !set2); set end)) (* GEN END TAG INSIDE LET *)

    (* GEN BEGIN TAG INSIDE LET *) val difference2 = (fn set1 => fn set2 => (let val r1 = new()
    					      val r2 = new() 
    					      val (rset1, rset2) = difference2 (!set1, !set2)
    					      in r1 := rset1; r2:= rset2 ;
    					      (r1, r2) end)) (* GEN END TAG INSIDE LET *)


    (* GEN BEGIN TAG INSIDE LET *) val differenceModulo = (fn set1 => fn set2 => fn F => 
    			    (let val r1 = new()
    				 val r2 = new() 
    ...
    (* GEN BEGIN TAG INSIDE LET *) val splitSets = (fn set1 => fn set2 => fn F => 
    		    (let val r1 = new()
    			 val r2 = new() 
    			 val r = new() 
    ...
    (* GEN BEGIN TAG INSIDE LET *) val intersection = (fn set1 => fn set2 => (let val set = new() in set := intersection (!set1, !set2); set end)) (* GEN END TAG INSIDE LET *)
    (* GEN BEGIN TAG INSIDE LET *) val union = (fn set1 => fn set2 => (let val set = new() in set := union (!set1, !set2); set end)) (* GEN END TAG INSIDE LET *)
