...
module Parse
    (Parserr :
      PARSERR
        with type Parserr.arg = Interface.arg
        with type Parserr.pos = Interface.pos
        with type Parserr.result = DextSyn.ast)
    (Tokens : Delphin_TOKENS)
    (DextSyn : Dextsyn.DEXTSYN)
    (Interface : Interface.INTERFACE)
    (Parserr : Parser.PARSERR) : PARSE = struct
  ...
  let rec sparse () =
    ...
    let rec loop lexer =
      let result, lexer = invoke lexer in
      let nextToken, lexer = Parserr.Streamm.get lexer in
      if Parserr.sameToken (nextToken, dummyEOF) then (* () *)
        result
      else loop lexer
    in
    loop lexer
