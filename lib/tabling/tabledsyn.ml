...
module TabledSyn (Names : NAMES) (Index : INDEX) : TABLEDSYN = struct (*! structure IntSyn = IntSyn' !*)
...
let rec installKeepTable a  = ((* Table.delete tabledSignature a; *)
...
let rec tabledLookup a  = (match (Table.lookup tabledSignature a) with None -> false | Some _ -> true)
(* keepTable a = bool
...
let rec keepTable a  = (match (Table.lookup tabledSignature a) with None -> false | Some true -> true | Some false -> false)
let reset = reset
