...
functor (* GEN BEGIN FUNCTOR DECL *) TableParam (structure Global : GLOBAL
...
struct
  ...
  (*! structure RBSet = RBSet !*)
   ...
   (* GEN BEGIN TAG OUTSIDE LET *) val globalTable : (IntSyn.dctx * IntSyn.dctx * IntSyn.dctx *
                       IntSyn.exp * res_eqn * answer * status ) list ref
                      = ref [] (* GEN END TAG OUTSIDE LET *)
   ...
   fun addSolution (S, answRef) =
     let
       (* GEN BEGIN TAG OUTSIDE LET *) val {solutions = SList, lookup = k} = !answRef (* GEN END TAG OUTSIDE LET *)
     in
   ...
   (* GEN BEGIN TAG OUTSIDE LET *) val aid : unit -> asub = RBSet.new (* GEN END TAG OUTSIDE LET *)
...
(* global search parameters *)
  ...
  (* GEN BEGIN TAG OUTSIDE LET *) val strategy  = ref Variant (* GEN END TAG OUTSIDE LET *) (* Subsumption *)

  (* GEN BEGIN TAG OUTSIDE LET *) val divHeuristic = ref false (* GEN END TAG OUTSIDE LET *);
(*  val divHeuristic = ref true;*)

  (* GEN BEGIN TAG OUTSIDE LET *) val stageCtr = ref 0 (* GEN END TAG OUTSIDE LET *);
 ...
 (* currently not used *)
  ...
  (* GEN BEGIN TAG OUTSIDE LET *) val strengthen = ref false (* GEN END TAG OUTSIDE LET *) ;


end (* GEN END FUNCTOR DECL *);  (* structure TableParam *)
