  ...
  *)
  ...
  let rec solve' = function
    ...
    | (C.Impl (r, A, Ha, g), s), C.DProg (G, dPool), sc ->
        let D' = N.decUName (G, I.Dec (None, I.EClo (A, s))) in
    ...
    | (C.All (D, g), s), C.DProg (G, dPool), sc ->
        let D' = N.decUName (G, I.decSub (D, s)) in
  ...
  and rSolve = function
    | ps', (C.Eq Q, s), C.DProg (G, dPool), HcHa, sc -> (
        T.signal (G, T.Unify (HcHa, I.EClo (Q, s), I.EClo ps'));
        match
          Unify.unifiable' (G, (Q, s), ps') (* effect: instantiate EVars *)
        with
        | None ->
        ...
        | Some msg ->
            ...
            false)
    | ps', (C.Assign (Q, eqns), s), dp, HcHa, sc -> (
        (* T.signal (G, T.Unify (HcHa, I.EClo (Q, s), I.EClo ps')); *)
        match Assign.assignable (G, ps', (Q, s)) with
        | Some cnstr -> aSolve ((eqns, s), dp, HcHa, cnstr, fun () -> sc I.Nil)
        | None ->
            (* T.signal (G, T.FailUnify (HcHa, "Assignment failed")); *)
            false)
    | ps', (C.And (r, A, g), s), dp, HcHa, sc ->
        ...
        rSolve
          ( ps',
            ...
            HcHa,
            fun S ->
              T.signal (G, T.Subgoal (HcHa, fun () -> subgoalNum S));
              solve' ((g, s), dp, fun M -> sc (I.App (M, S))) )
    | ps', (C.Exists (I.Dec (_, A), r), s), dp, HcHa, sc ->
        let X = I.newEVar (G, I.EClo (A, s)) in
        rSolve
          (ps', (r, I.Dot (I.Exp X, s)), dp, HcHa, fun S -> sc (I.App (X, S)))
    | ps', (C.Axists (I.ADec (_, d), r), s), dp, HcHa, sc ->
        let X = I.newAVar () in
        rSolve
          (ps', (r, I.Dot (I.Exp (I.EClo (X, I.Shift ~-d)), s)), dp, HcHa, sc)
  (* we don't increase the proof term here! *)

  and aSolve = function
    | (C.Trivial, s), dp, HcHa, cnstr, sc ->
        if Assign.solveCnstr cnstr then (
          ...
          false
    | (C.UnifyEq (G', e1, N, eqns), s), dp, HcHa, cnstr, sc ->
    ...
    let rec matchSigDet = function
      ...
      | Hc :: sgn' -> (
          ...
          try
            ...
            matchSigDet sgn'
          with SucceedOnce S ->
            T.signal (G, T.CommitGoal (tag, (Hc, Ha), I.EClo ps'));
            sc (I.Root (Hc, S)))
    in
    let rec matchDProg = function
      | I.Null, _ ->
          if deterministic then matchSigDet (Index.lookup (cidFromHead Ha))
      ...
      | I.Decl (dPool', C.Dec (r, s, Ha')), k ->
          if eqHead (Ha, Ha') then
            if deterministic then (* #succeeds = 1 *)
                      ...
                      rSolve
                        ( ps',
                          ...
                          fun S ->
                            T.signal
                              ( G,
                                T.SucceedGoal (tag, (I.BVar k, Ha), I.EClo ps')
                              );
                            raise (SucceedOnce S) ))
                then (
                  (* deep backtracking *)
              ...
              with SucceedOnce S ->
                T.signal (G, T.CommitGoal (tag, (I.BVar k, Ha), I.EClo ps'));
    ...
    let rec matchConstraint (cnstrSolve, try_) =
      let succeeded =
        Cs.CSManager.trail (fun () ->
            ...
            | None -> false)
      in
      if succeeded then matchConstraint (cnstrSolve, try_ + 1) else ()
    in
