...
module MemoTableInst ((*! module IntSyn' : INTSYN !*)
  ...
  *)
  ...
  (* for debugging only *)
    ...
    let rec printSub = function (G, I.Shift n) -> print ("I.Shift " ^ Int.toString n ^ "\n")
      ...
      | (G, I.Dot (I.Exp(X as I.EVar (ref(SOME(U)),_ ,  _, _)), s)) -> 
        (print ("Exp ( EVar " ^ expToS(G, X) ^ ").")  ; printSub (G, s))
      | (G, I.Dot (I.Exp(X as I.EVar (_, _, _, _)), s)) -> 
        (print ("Exp ( EVar  " ^ expToS(G, X) ^ ").")  ; printSub (G, s))
      | (G, I.Dot (I.Exp(I.AVar (_)), s)) -> 
        (print ("Exp (AVar _ ). "); printSub (G, s))
      | (G, I.Dot (I.Exp(I.EClo (I.AVar (ref (SOME(U))), s')), s)) -> 
        (print ("Exp (AVar " ^ expToS (G, I.EClo(U,s'))  ^ ").") ; printSub (G, s))
      | (G, I.Dot (I.Exp(X as I.EClo (I.EVar (ref(SOME(U)), _, _, _),s')), s)) -> 
        (print ("Exp (EVarClo " ^ expToS (G, I.EClo(U,s')) ^ ") "); printSub (G, s))
      | (G, I.Dot (I.Exp(X as I.EClo (U, s')), s)) -> 
        (print ("Exp (EClo " ^ expToS (G, Whnf.normalize(U,s')) ^ ") "); printSub (G, s))
      | (G, I.Dot (I.Exp(E), s)) -> 
        (print ("Exp ( " ^ expToS (G,E) ^ " ). "); printSub (G, s))
      | (G, I.Dot (I.Undef, s)) -> (print ("Undef . "); printSub (G, s))

  (* auxiliary function  -- needed to dereference AVars -- expensive?*)
    let rec normalizeSub = function (I.Shift n) -> I.Shift n
      | (I.Dot(I.Exp(I.EClo(I.AVar (ref (SOME(U))), s')), s)) -> 
      I.Dot(I.Exp(Whnf.normalize (U, s')), normalizeSub s)
    | (I.Dot (I.Exp(I.EClo(I.EVar(ref(SOME(U)), _, _, _), s')), s)) -> 
      I.Dot(I.Exp(Whnf.normalize (U, s')), normalizeSub s)
 ...
 (* Matching for linear terms based on assignment *)
  ...
  (* [asub]E1  = U *)
  let rec assign = function ((* total as (t, passed)*)d, Dec1 as I.Dec(n, V), E1 as I.Root(I.BVar k, S1), U, asub) -> 
     ...
     | ((* total as (t, passed)*) d, Dec1 as I.ADec(n, d'), E1 as I.Root(I.BVar k, S1), U, asub) -> 
  ...
  (* compatibleCtx (asub, (Dsq, Gsq, eqn_sq), GR) = option
    ...
    then
      ...
      NONE
   *)
  let rec compatibleCtx = function (asub, (Dsq, Gsq, eqn_sq), []) -> NONE
  ...
  let rec instChild = function (N as Leaf((D_t, nsub_t), GList), (D_sq, sq), asub) -> 
 ...
 (* solveEqn' ((VarDef, s), G) = bool
    ...
    then
       return true, if VarDefs are solvable
              false otherwise
 *)
  let rec solveEqn' = function (T.Trivial, s) -> true
    | (T.Unify(G',e1, N (* evar *), eqns), s) -> 
      let
        let s' = shift (G', s)
      in
        Assign.unifiable (G', (N, s'),(e1, s'))
 ...
 (* solveEqnI' ((VarDef, s), G) = bool
    ...
    then
       return true, if VarDefs are solvable
              false otherwise
 *)
  let rec solveEqnI' = function (T.Trivial, s) -> true
    | (T.Unify(G',e1, N (* evar *), eqns), s) -> 
      let
        let s' = shift (G', s)
 ...
 (* solveEqnI' ((VarDef, s), G) = bool
    ...
    then
       return true, if VarDefs are solvable
              false otherwise
 *)
  let rec solveEqnI' = function ((T.Trivial, s), G) -> true
    | ((T.Unify(G',e1, N (* evar *), eqns), s), G) -> 
      let
        let G'' = compose (G', G)
        let s' = shift (G', s)
...
(*              if solveEqnI' (eqn', easub) *)
      ...
      | retrieve' (N as Node((D, sub), children), (Dq, sq), asub,
        ...
        let
          ...
          let rec checkCandidates = function nil -> 
 ...
 *)
 ...
 (* Insert via variant checking *)
  ...
  let rec variantCtx = function ((G, eqn), []) -> NONE
 ...
 let rec insert (Nref, (Dsq, sq), GR) =
  ...
  in
    ...
    *)
    let rec makeCtx = function (n, I.Null, DEVars : ctx) -> ()
