...
struct
  ...
  local
    fun (* GEN BEGIN FUN FIRST *) compare ({sd=k1, ind=NONE, c=c1, m=m1, r=r1, p=p1},
        ...
        (case (Int.compare (c1*m2, c2*m1), Int.compare (k2, k1),
           ...
           of (EQUAL, EQUAL, EQUAL, result) => result
            ...
            | (result, _, _, _) => result) (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) compare ({sd=k1, ind=NONE, c=c1, m=m1, r=r1, p=p1},
                 {sd=k2, ind=SOME (i2), c=c2, m=m2, r=r2, p=p2}) =
        (case (Int.compare (c1*m2, c2*m1))
           of LESS => LESS
            | EQUAL => GREATER
            | GREATER => GREATER) (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) compare ({sd=k1, ind=SOME (i1), c=c1, m=m1, r=r1, p=p1},
                 {sd=k2, ind=NONE, c=c2, m=m2, r=r2, p=p2}) =
        (case (Int.compare (c1*m2, c2*m1))
           of LESS => LESS
            | EQUAL => LESS
      ...
      | (* GEN BEGIN FUN BRANCH *) compare ({sd=k1, ind=SOME (i1), c=c1, m=m1, r=r1, p=p1},
        ...
        (case (Int.compare (c1*m2, c2*m1), Int.compare (k2, k1), Int.compare (r1, r2),
           ...
           of (EQUAL, EQUAL, EQUAL, EQUAL, result) => result
            ...
            | (result, _, _, _, _) => result) (* GEN END FUN BRANCH *)


    fun (* GEN BEGIN FUN FIRST *) recToString 0 = "non-rec" (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) recToString 1 = "rec" (* GEN END FUN BRANCH *)
    ...
    fun (* GEN BEGIN FUN FIRST *) sum {sd=k1, ind=NONE, c=c1, m=m1, r=r1, p=p1} =
        realFmt ((Real.fromInt k1) + ratio(m1,c1) + (Real.fromInt r1)) (* GEN END FUN FIRST *)
        | (* GEN BEGIN FUN BRANCH *) sum {sd=k1, ind=SOME (i1), c=c1, m=m1, r=r1, p=p1} =
        realFmt ((Real.fromInt k1) + ratio(1,i1) + ratio(m1,c1) + (Real.fromInt r1)) (* GEN END FUN BRANCH *)

    fun (* GEN BEGIN FUN FIRST *) indexToString {sd = s1, ind=NONE, c=c1, m=m1, r=r1, p=p1} =
          "(c/m=" ^ (Int.toString c1) ^ "/" ^ (Int.toString m1) ^ "=" ^
          (realFmt (ratio (c1, m1))) ^
          ", ind=., sd=" ^ (Int.toString s1) ^ ", " ^ (recToString r1) ^
          ", p=" ^ (Int.toString p1) ^ "sum = " ^ (sum {sd=s1, ind=NONE, c=c1, m=m1, r=r1, p=p1}) ^ " )" (* GEN END FUN FIRST *)

      | (* GEN BEGIN FUN BRANCH *) indexToString {sd=s1, ind=SOME (idx) , c=c1, m=m1, r=r1, p=p1} =
          "(c/m=" ^ (Int.toString c1) ^ "/" ^ (Int.toString m1) ^ "=" ^
          (realFmt (ratio (c1, m1))) ^
          ", ind=" ^ (Int.toString idx) ^
          ", sd=" ^ (Int.toString s1) ^ ", " ^ (recToString r1) ^
          ", p=" ^ (Int.toString p1) ^ " sum = " ^ (sum {sd=s1, ind=SOME (idx) , c=c1, m=m1, r=r1, p=p1}) ^ ")" (* GEN END FUN BRANCH *)
