...
module CSIntWord
    ...
    (Whnf : Whnf.WHNF)
    (Unify : Unify.UNIFY) : Cs.CS = struct
  ...
  let rec provePlusExp (U, V, W, P) =
    Root (Const !provePlusID, App (U, App (V, App (W, App (P, Nil)))))

  let rec proofPlusExp (U, V, W, P) =
    Root (Const !proofPlusID, App (U, App (V, App (W, App (P, Nil)))))

  let rec proofTimesExp (U, V, W, P) =
    Root (Const !proofTimesID, App (U, App (V, App (W, App (P, Nil)))))

  let rec proveTimesExp (U, V, W, P) =
    Root (Const !proveTimesID, App (U, App (V, App (W, App (P, Nil)))))

  let rec proveQuotExp (U, V, W, P) =
    Root (Const !proveQuotID, App (U, App (V, App (W, App (P, Nil)))))

  let rec proofQuotExp (U, V, W, P) =
    Root (Const !proofQuotID, App (U, App (V, App (W, App (P, Nil)))))
  ...
  and solvePlus = function
    | G, S, 0 -> (
        ...
        | Expr Us1, Num d2, Num d3 ->
            if
              W.( > ) = (d3, d2)
              && Unify.unifiable (G, Us1, (numberExp (W).-(d3, d2), id))
            then Some (plusPfExp ((W).-(d3, d2), d2))
            else None
        | Num d1, Expr Us2, Num d3 ->
            if
              W.( > ) = (d3, d1)
              && Unify.unifiable (G, Us2, (numberExp (W).-(d3, d1), id))
            then Some (plusPfExp (d1, (W).-(d3, d1)))
            else None
        | Num d1, Num d2, Expr Us3 ->
            if
              plusCheck (d1, d2)
              && Unify.unifiable (G, Us3, (numberExp (W).+(d1, d2), id))
            then Some (plusPfExp (d1, d2))
            else None
        | _ ->
            let proof = newEVar (G, plusExp (EClo Us1, EClo Us2, EClo Us3)) in
            let cnstr =
              makeCnstrPlus (G, proof, EClo Us1, EClo Us2, EClo Us3)
            in
            let _ =
              List.app (fun Us -> Unify.delay (Us, ref cnstr)) [ Us1; Us2; Us3 ]
            in
            Some proof)
    | G, S, n -> None
  ...
  and solveTimes = function
    | G, S, 0 -> (
        ...
        | Expr Us1, Num d2, Num d3 ->
            if d3 = zero && Unify.unifiable (G, Us1, (numberExp zero, id)) then
              Some (timesPfExp (zero, d2))
            else if
              W.( > ) (d2, zero)
              && W.( > ) (d3, zero)
              && W.mod_ (d3, d2) = zero
        ...
        | Num d1, Expr Us2, Num d3 ->
            if d3 = zero && Unify.unifiable (G, Us2, (numberExp zero, id)) then
              Some (timesPfExp (d1, zero))
            else if
              W.( > ) (d1, zero)
              && W.( > ) (d3, zero)
              && W.mod_ (d3, d1) = zero
              && Unify.unifiable (G, Us2, (numberExp (W.div (d3, d1)), id))
        ...
        | _ ->
            let proof = newEVar (G, timesExp (EClo Us1, EClo Us2, EClo Us3)) in
            let cnstr =
              makeCnstrTimes (G, proof, EClo Us1, EClo Us2, EClo Us3)
            in
            let _ =
              List.app (fun Us -> Unify.delay (Us, ref cnstr)) [ Us1; Us2; Us3 ]
            in
  ...
  and solveQuot = function
    | G, S, 0 -> (
        ...
        | Num d1, Num d2, Expr Us3 ->
            if
              quotCheck (d1, d2)
              && Unify.unifiable (G, Us3, (numberExp (W.div (d1, d2)), id))
            then Some (quotPfExp (d1, d2))
            else None
        | _ ->
            let proof = newEVar (G, quotExp (EClo Us1, EClo Us2, EClo Us3)) in
            let cnstr =
              makeCnstrQuot (G, proof, EClo Us1, EClo Us2, EClo Us3)
            in
            let _ =
              List.app (fun Us -> Unify.delay (Us, ref cnstr)) [ Us1; Us2; Us3 ]
            in
  ...
  let rec solveProvePlus (G, S, k) =
    ...
    let Us4 = fth (S, id) in
    match
      solvePlus (G, App (EClo Us1, App (EClo Us2, App (EClo Us3, Nil))), k)
    with
    | Some U ->
        if Unify.unifiable (G, Us4, (U, id)) then
          Some (proofPlusExp (EClo Us1, EClo Us2, EClo Us3, EClo Us4))
        else None
    | None -> None
  ...
  let rec solveProveTimes (G, S, k) =
    ...
    let Us4 = fth (S, id) in
    match
      solveTimes (G, App (EClo Us1, App (EClo Us2, App (EClo Us3, Nil))), k)
    with
    | Some U ->
        if Unify.unifiable (G, Us4, (U, id)) then
          Some (proofTimesExp (EClo Us1, EClo Us2, EClo Us3, EClo Us4))
        else None
    | None -> None
  ...
  let rec solveProveQuot (G, S, k) =
    ...
    let Us4 = fth (S, id) in
    match
      solveQuot (G, App (EClo Us1, App (EClo Us2, App (EClo Us3, Nil))), k)
    with
    | Some U ->
        if Unify.unifiable (G, Us4, (U, id)) then
          Some (proofQuotExp (EClo Us1, EClo Us2, EClo Us3, EClo Us4))
        else None
    | None -> None
      ...
      installF
        ( ConDec
            ( "+",
              ...
              arrow (word (), arrow (word (), arrow (word (), Uni Type))),
              Kind ),
          None,
          [
            MS.Mapp
                  ...
                  pi
                    ( "Y",
                      ...
                      pi
                        ( "Z",
                          word (),
                          pi ("P", plusExp (bvar 3, bvar 2, bvar 1), Uni Type)
                        ) ) ),
              Kind ),
          None,
          [
                  ...
                  pi
                    ( "Y",
                      ...
                      pi
                        ( "Z",
                          word (),
                          pi ("P", timesExp (bvar 3, bvar 2, bvar 1), Uni Type)
                        ) ) ),
              Kind ),
          None,
          [
                  ...
                  pi
                    ( "Y",
                      ...
                      pi
                        ( "Z",
                          word (),
                          pi ("P", quotExp (bvar 3, bvar 2, bvar 1), Uni Type)
                        ) ) ),
              Kind ),
          None,
          [
