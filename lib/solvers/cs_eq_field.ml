...
struct
  ...
  *)
  ...
  local
    ...
    (* GEN BEGIN TAG OUTSIDE LET *) val unaryMinusID  = ref ~1 : cid ref (* GEN END TAG OUTSIDE LET *)  (* ~ : number -> number           *)
    (* GEN BEGIN TAG OUTSIDE LET *) val plusID        = ref ~1 : cid ref (* GEN END TAG OUTSIDE LET *)  (* + : number -> number -> number *)
    (* GEN BEGIN TAG OUTSIDE LET *) val minusID       = ref ~1 : cid ref (* GEN END TAG OUTSIDE LET *)  (* - : number -> number -> number *)
    (* GEN BEGIN TAG OUTSIDE LET *) val timesID       = ref ~1 : cid ref (* GEN END TAG OUTSIDE LET *)  (* * : number -> number -> number *)
    ...
    *)
    fun unifySum (G, sum1, sum2) =
          let
            fun (* GEN BEGIN FUN FIRST *) invertMon (G, Mon (n, [(LHS as EVar (r, _, _, _), s)]), sum) =
                  if Whnf.isPatSub s
                  then
                    let
                      (* GEN BEGIN TAG OUTSIDE LET *) val ss = Whnf.invert s (* GEN END TAG OUTSIDE LET *)
                      (* GEN BEGIN TAG OUTSIDE LET *) val RHS = toFgn (timesSum (Sum (~ (inverse n), nil),
                                                 sum)) (* GEN END TAG OUTSIDE LET *)
                    in
