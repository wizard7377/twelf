...
module CSIneqField (OrderedField : ORDERED_FIELD) (Trail : TRAIL) (Unify : UNIFY) (SparseArray : SPARSE_ARRAY) (SparseArray2 : SPARSE_ARRAY2) (CSEqField : CS_EQ_FIELD) (Compat : COMPAT) : CS = struct (*! structure CSManager = CSManager !*)
...
let rec gtAdd (U1, U2, V, W)  = Root (Const (! gtAddID), App (U1, App (U2, App (V, App (W, Nil)))))
...
let rec gtNConDec (d)  = ConDec (toString (d) ^ ">" ^ toString (zero), None, 0, Normal, gt0 (constant (d)), Type)
(* foreign constant for_sml the proof object d>0 *)

let rec gtNExp (d)  = Root (FgnConst (! myID, gtNConDec (d)), Nil)
(* specialized constructors for_sml the declared objects *)

let rec geqN0 (d)  = if (d = zero) then geq00 () else gtGeq (constant (d), constant (zero), gtNExp (d))
(* parsing proof objects d>0 *)

let rec parseGtN string  = ( let suffix = (">" ^ (toString (zero))) in let stringLen = String.size string in let suffixLen = String.size suffix in let numLen = Int.- (stringLen, suffixLen) in  if Int.> (stringLen, suffixLen) && (String.substring (string, numLen, suffixLen) = suffix) then (match fromString (String.substring (string, 0, numLen)) with Some (d) -> if (d > zero) then Some (gtNConDec (d)) else None | None -> None) else None )
...
let tableau = ( let l = {owner = Exp (Null, Sum (zero, [])); tag = ref 0; restr = ref None; dead = ref true} in  ({rlabels = Array.array (l); clabels = Array.array (l); consts = Array.array (zero); coeffs = Array2.array (zero); nrows = ref 0; ncols = ref 0; trail = Trail.trail ()} : tableau) )
...
let rec ownerSum = function (Var (G, mon)) -> Sum (zero, [mon]) | (Exp (G, sum)) -> sum
(* debugging code - REMOVE *)

let rec displayPos = function (Row (row)) -> print ("row " ^ Int.toString (row) ^ "\n") | (Col (col)) -> print ("column " ^ Int.toString (col) ^ "\n")
(* debugging code - REMOVE *)

let rec displaySum = function (Sum (m, Mon (n, _) :: monL)) -> (print (OrderedField.toString n); print " ? + "; displaySum (Sum (m, monL))) | (Sum (m, [])) -> (print (OrderedField.toString m); print " >= 0\n")
(* debugging code - REMOVE *)
...
let rec findPivot (row)  = ( (* extend Field.compare to deal with NONE (= infinity) *)
...
let rec maximizeRow (row)  = ( let value = const (row) in  if (value <= zero) then match findPivot (row) with Some (i, j) -> if (i <> row) then (Trail.log (trail (tableau), Pivot (i, j)); pivot (i, j); maximizeRow row) else Unbounded j | None -> Maximized value else Positive )
...
let rec unifyRestr (Restr (G, proof, strict), proof')  = if Unify.unifiable (G, (proof, id), (proof', id)) then () else raise (Error)
(* unify a sum with a number *)

let rec unifySum (G, sum, d)  = if Unify.unifiable (G, (toExp (sum), id), (constant (d), id)) then () else raise (Error)
...
and minimize (row)  = ( (* equate the given column to zero if coeff(row, j) <> zero *)
...
(match owner (l) with (owner) -> unifySum (ownerContext (owner), ownerSum (owner), const (i)) | _ -> ())) else match isSubsumed (i) with Some (pos') -> ( let l' = label (pos') in  (Trail.log (trail (tableau), Kill (Row (i))); dead (Array.sub (rlabels (tableau), i)) := true; (match (restriction (l), restriction (l')) with (Some (restr), Some (Restr (_, proof', _))) -> unifyRestr (restr, proof') | (Some _, None) -> ((* it is safe to restrict without doing all
...
and restrict = function (pos, restr) -> ( let l = label (pos) in  if dead (l) then unifyRestr (restr, geq00 ()) else match restriction (l) with Some (Restr (_, proof', _)) -> unifyRestr (restr, proof') | None -> ( (* compute the list of non-null row entries *)
...
Trail.log (trail (tableau), Pivot (row, col)); pivot (row, col); restrict (Row (row), restr)) | [] -> ((* the column is zero at all the active row
                                ...
                                *)
Trail.log (trail (tableau), Restrict (Col (col))); restr (label (Col (col))) := Some (restr)) ) ) | (pos, restr) -> ( let l = label (pos) in  if dead (l) then unifyRestr (restr, geqN0 (const (row))) else match restriction (l) with Some (Restr (_, proof', _)) -> unifyRestr (restr, proof') | None -> match maximizeRow row with Unbounded col -> (Trail.log (trail (tableau), Restrict (Row (row))); restr (Array.sub (rlabels (tableau), row)) := Some (restr); if (const (row) < zero) then (Trail.log (trail (tableau), Pivot (row, col)); pivot (row, col)) else ()) | Positive -> ((* the tableau is satisfiable and minimal *)
Trail.log (trail (tableau), Restrict (Row (row))); restr (Array.sub (rlabels (tableau), row)) := Some (restr)) | Maximized value -> if (value = zero) then ((* the tableau is satisfiable but not minimal*)
Trail.log (trail (tableau), Restrict (Row (row))); restr (Array.sub (rlabels (tableau), row)) := Some (restr); minimize (row)) else raise (Error) )
and insertEqual (G, pos, sum)  = ( let (m, wposL) = decomposeSum (G, sum) in let decomp' = (m, (~ one, pos) :: wposL) in let pos' = insertDecomp (decomp', Exp (G, Sum (zero, []))) in let decomp'' = unaryMinusDecomp (decomp') in let tag'' = tag (label (insertDecomp (decomp'', Exp (G, Sum (zero, []))))) in  ((* the second expression may change position when we
                  restrict the first. We use tags to keep track of it *)
restrict (pos', Restr (G, geq00 (), false)); (match findTag (tag'') with Some (pos'') -> restrict (pos'', Restr (G, geq00 (), false)))) )
...
unifySum (G, sum, zero)) else ( let rec isVar = function (Sum (m, [mon])) -> if (m = zero) && (n = one) then Some (mon) else None | (sum) -> None in  match isVar (sum) with Some (mon) -> (* the nf is another variable *)
...
let rec reset ()  = ( let l = {owner = Exp (Null, Sum (zero, [])); tag = ref 0; restr = ref None; dead = ref true} in  (Array.modify (fun _ -> l) (rlabels (tableau), 0, nRows ()); Array.modify (fun _ -> l) (clabels (tableau), 0, nCols ()); Array.modify (fun _ -> zero) (consts (tableau), 0, nRows ()); Array2.modify Array2.RowMajor (fun _ -> zero) {base = coeffs (tableau); row = 0; col = 0; nrows = nRows (); ncols = nCols ()}; nrows (tableau) := 0; ncols (tableau) := 0; Trail.reset (trail (tableau))) )
(* trailing functions *)
...
let rec isZeroExp (U)  = (match isConstantExp (U) with Some (d) -> (d = zero) | None -> false)
(* solveGt (G, S, n) tries to find the n-th solution to G |- '>' @ S : type *)

let rec solveGt = function (G, S, 0) -> ( let rec solveGt0 (W)  = (match isConstantExp (W) with Some (d) -> if (d > zero) then gtNExp (d) else raise (Error) | None -> ( let proof = newEVar (G, gt0 (W)) in let _ = restrict (insert (G, (W, id)), Restr (G, gtGeq (W, constant (zero), proof), true)) in  proof )) in let U1 = EClo (fst (S, id)) in let U2 = EClo (snd (S, id)) in  try (if isZeroExp (U2) then Some (solveGt0 (U1)) else ( let W = minus (U1, U2) in let proof = solveGt0 (W) in  Some (gtAdd (W, constant (zero), U2, proof)) )) with Error -> None ) | (G, S, n) -> None
(* solveGeq (G, S, n) tries to find the n-th solution to G |- '>=' @ S : type *)

let rec solveGeq = function (G, S, 0) -> ( let rec solveGeq0 (W)  = (match isConstantExp (W) with Some (d) -> if (d >= zero) then geqN0 (d) else raise (Error) | None -> ( let proof = newEVar (G, geq0 (W)) in let _ = restrict (insert (G, (W, id)), Restr (G, proof, false)) in  proof )) in let U1 = EClo (fst (S, id)) in let U2 = EClo (snd (S, id)) in  try (if isZeroExp (U2) then Some (solveGeq0 (U1)) else ( let W = minus (U1, U2) in let proof = solveGeq0 (W) in  Some (geqAdd (W, constant (zero), U2, proof)) )) with Error -> None ) | (G, S, n) -> None
...
let rec init (cs, installF)  = (myID := cs; gtID := installF (ConDec (">", None, 0, Constraint (! myID, solveGt), arrow (number (), arrow (number (), Uni (Type))), Kind), Some (FX.Infix (FX.minPrec, FX.None)), [MS.Mapp (MS.Marg (MS.Star, None), MS.Mapp (MS.Marg (MS.Star, None), MS.Mnil))]); geqID := installF (ConDec (">=", None, 0, Constraint (! myID, solveGeq), arrow (number (), arrow (number (), Uni (Type))), Kind), Some (FX.Infix (FX.minPrec, FX.None)), [MS.Mapp (MS.Marg (MS.Star, None), MS.Mapp (MS.Marg (MS.Star, None), MS.Mnil))]); gtAddID := installF (ConDec ("+>", None, 2, Normal, pi ("X", number (), pi ("Y", number (), pi ("Z", number (), arrow (gt (Root (BVar 3, Nil), Root (BVar 2, Nil)), gt (plus (Root (BVar 4, Nil), Root (BVar 2, Nil)), plus (Root (BVar 3, Nil), Root (BVar 2, Nil))))))), Type), None, []); geqAddID := installF (ConDec ("+>=", None, 2, Normal, pi ("X", number (), pi ("Y", number (), pi ("Z", number (), arrow (geq (Root (BVar 3, Nil), Root (BVar 2, Nil)), geq (plus (Root (BVar 4, Nil), Root (BVar 2, Nil)), plus (Root (BVar 3, Nil), Root (BVar 2, Nil))))))), Type), None, []); gtGeqID := installF (ConDec (">>=", None, 2, Normal, pi ("X", number (), pi ("Y", number (), arrow (gt (Root (BVar 2, Nil), Root (BVar 1, Nil)), geq (Root (BVar 3, Nil), Root (BVar 2, Nil))))), Type), None, []); geq00ID := installF (ConDec ("0>=0", None, 0, Normal, geq0 (constant (zero)), Type), None, []); installFgnCnstrOps (); ())
let solver = ({name = ("inequality/" ^ OrderedField.name ^ "s"); keywords = "arithmetic,inequality"; needs = ["Unify"; name (CSEqField.solver)]; fgnConst = Some ({parse = parseGtN}); init = init; reset = reset; mark = mark; unwind = unwind} : CSManager.solver)
 end
