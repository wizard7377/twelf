...
module CSIneqField
  ...
  let rec gtNConDec d =
    ConDec
      (toString d ^ ">" ^ toString zero, None, 0, Normal, gt0 (constant d), Type)
  ...
  let rec geqN0 d =
    if d = zero then geq00 () else gtGeq (constant d, constant zero, gtNExp d)
  ...
  and restrict = function
    | pos, restr -> (
        let l = label pos in
        if dead l then unifyRestr (restr, geq00 ())
        else
          match restriction l with
    ...
    | pos, restr -> (
        let l = label pos in
        if dead l then unifyRestr (restr, geqN0 (const row))
        else
          match restriction l with
          | Some (Restr (_, proof', _)) -> unifyRestr (restr, proof')
          | None -> (
  ...
  and insertEqual (G, pos, sum) =
    ...
    let tag'' =
      tag (label (insertDecomp (decomp'', Exp (G, Sum (zero, [])))))
    in
  ...
  let rec isZeroExp U =
    match isConstantExp U with Some d -> d = zero | None -> false
  ...
  let rec solveGt = function
    | G, S, 0 -> (
        ...
        try
          if isZeroExp U2 then Some (solveGt0 U1)
          else
    ...
    | G, S, 0 -> (
        let rec solveGeq0 W =
          ...
          | None ->
              ...
              let _ = restrict (insert (G, (W, id)), Restr (G, proof, false)) in
              proof
        in
        let U1 = EClo (fst (S, id)) in
        let U2 = EClo (snd (S, id)) in
                    ...
                    ( "Y",
                      ...
                      pi
                        ( "Z",
                          number (),
                          arrow
                            ( gt (Root (BVar 3, Nil), Root (BVar 2, Nil)),
                              gt
