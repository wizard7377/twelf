...
struct
  ...
  local
    ...
    (* GEN BEGIN TAG OUTSIDE LET *) val gtID   = ref ~1 : cid ref (* GEN END TAG OUTSIDE LET *)
    ...
    (* GEN BEGIN TAG OUTSIDE LET *) val gtAddID  = ref ~1 : cid ref (* GEN END TAG OUTSIDE LET *)
    (* GEN BEGIN TAG OUTSIDE LET *) val geqAddID = ref ~1 : cid ref (* GEN END TAG OUTSIDE LET *)
    (* GEN BEGIN TAG OUTSIDE LET *) val gtGeqID = ref ~1 : cid ref (* GEN END TAG OUTSIDE LET *)
    (* GEN BEGIN TAG OUTSIDE LET *) val geq00ID = ref ~1 : cid ref (* GEN END TAG OUTSIDE LET *)
    ...
    *)
    ...
    (* GEN BEGIN TAG OUTSIDE LET *) val tableau =
          let
            (* GEN BEGIN TAG OUTSIDE LET *) val l = {owner = Exp (Null, Sum (zero, nil)), tag = ref 0,
    ...
    fun (* GEN BEGIN FUN FIRST *) ownerSum (Var (G, mon)) = Sum(zero, [mon]) (* GEN END FUN FIRST *)
                     ...
                     of SOME(pos) => (n, pos)
                      | NONE =>
                          let
                            ...
                            (* GEN BEGIN TAG OUTSIDE LET *) val l = {owner = Var (G, Mon (one, UsL)),
                                     tag = ref 0,
                                     restr = ref NONE,
    ...
    *)
    and insertEqual (G, pos, sum) =
          let
            ...
            (* GEN BEGIN TAG OUTSIDE LET *) val pos' = insertDecomp (decomp', Exp (G, Sum(zero, nil))) (* GEN END TAG OUTSIDE LET *)
    
            (* GEN BEGIN TAG OUTSIDE LET *) val decomp'' = unaryMinusDecomp (decomp') (* GEN END TAG OUTSIDE LET *)
            (* GEN BEGIN TAG OUTSIDE LET *) val tag'' = #tag(label (insertDecomp (decomp'', Exp (G, Sum(zero, nil))))) (* GEN END TAG OUTSIDE LET *)
          in
            (
    ...
    fun (* GEN BEGIN FUN FIRST *) solveGt (G, S, 0) =
          let
            fun solveGt0 (W) =
                  (case isConstantExp (W)
                     of SOME(d) =>
                      ...
                      | NONE =>
                          let
                            (* GEN BEGIN TAG OUTSIDE LET *) val proof = newEVar (G, gt0 (W)) (* GEN END TAG OUTSIDE LET *)
                            (* GEN BEGIN TAG OUTSIDE LET *) val _ = restrict (insert (G, (W, id)),
                                              Restr (G, gtGeq (W, constant (zero), proof), true)) (* GEN END TAG OUTSIDE LET *);
                          in
                            proof
                          end)
            ...
            (
              ...
              then SOME(solveGt0 (U1))
                ...
                let
                  ...
                  (* GEN BEGIN TAG OUTSIDE LET *) val proof = solveGt0 (W) (* GEN END TAG OUTSIDE LET *)
                in
                  SOME(gtAdd (W, constant (zero), U2, proof))
                end
            ) handle Error => NONE
          end (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) solveGt (G, S, n) = NONE (* GEN END FUN BRANCH *)
                     ...
                     of SOME(d) =>
                      ...
                      | NONE =>
                          let
                            ...
                            (* GEN BEGIN TAG OUTSIDE LET *) val _ = restrict (insert (G, (W, id)),
                                       Restr (G, proof, false)) (* GEN END TAG OUTSIDE LET *)
                          in
                            proof
                          end)
