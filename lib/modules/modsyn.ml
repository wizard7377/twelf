...
module ModSyn
  ...
  let rec mapExpConsts f U =
    ...
    let rec trExp = function
      ...
      | U -> FgnExpStd.Map.apply csfe trExp
  ...
  and mapConDecConsts = function
    ...
    | f, IntSyn.AbbrevDef (name, parent, i, U, V, L) ->
        IntSyn.AbbrevDef (name, parent, i, mapExpConsts f U, mapExpConsts f V, L)
  ...
  let rec installModule
    ...
    let rec mapConst cid =
      match IntTree.lookup constMap cid with None -> cid | Some cid' -> cid'
    in
    let rec doStruct (mid, StructInfo strdec) =
      ...
      in
      let _ =
        match nsOpt with Some ns -> Names.insertStruct (ns, mid') | _ -> ()
      in
      let _ =
        match parent with None -> Names.installStructName mid' | _ -> ()
      in
    ...
    let rec doConst (cid, ConstInfo (condec, fixity, namePrefOpt, origin)) =
      ...
      in
      let _ =
        match nsOpt with Some ns -> Names.insertConst (ns, cid') | _ -> ()
      in
      let _ =
        match parent with None -> Names.installConstName cid' | _ -> ()
      in
...
(*
  ...
  type transform = IntSyn.cid * IntSyn.ConDec -> IntSyn.ConDec
  *)
  val installStruct : IntSyn.strDec * module * Names.namespace option * (IntSyn.cid * (string * Paths.occConDec option) -> unit)(* action *)
 * bool -> unit
  val installSig : module * Names.namespace option * (IntSyn.cid * (string * Paths.occConDec option) -> unit)(* action *)
 * bool -> unit
  val instantiateModule : module * (Names.namespace -> (IntSyn.cid * IntSyn.conDec -> IntSyn.conDec))(* Names.namespace -> transform *)
 -> module
(* Extract some entries of the current global signature table in order
  ...
  *)
  ...
  val reset : unit -> unit
  val installSigDef : string * module -> unit
