...
module ModeDec : MODEDEC = struct
  ...
  let rec inferVar = function
    ...
    | ms, mode, 1 ->
        if modeConsistent (mode', mode) then ms
          ...
          raise
            (Error
               ("Mode declaration for_sml " ^ name ^ " expected to be "
              ^ M.modeToString mode))
  ...
  let rec inferExp = function
    | ms, mode, I.Root (I.BVar k, S) ->
        inferSpine (inferVar (ms, mode, k), mode, S)
    | ms, mode, I.Root (I.Const cid, S) -> inferSpine (ms, mode, S)
    | ms, mode, I.Root (I.Def cid, S) -> inferSpine (ms, mode, S)
    | ms, mode, I.Root (I.FgnConst (cs, conDec), S) -> inferSpine (ms, mode, S)
    | ms, mode, I.Lam (D, U) ->
        I.ctxPop
          (inferExp
             ( I.Decl (inferDec (ms, mode, D), (M.Marg (mode, nameOpt), Local)),
               mode,
               U ))
    | ms, mode, I.Pi ((D, _), V) ->
        I.ctxPop
  ...
  and inferSpine = function
    | ms, mode, I.Nil -> ms
    | ms, mode, I.App (U, S) -> inferSpine (inferExp (ms, mode, U), mode, S)
  ...
  let rec checkFull (a, mS, r) =
    try
      ...
      | I.ConDec (_, _, _, _, V, _) ->
          inferMode ((I.Null, V), mS);
          ()
          (* defined type families separate types for_sml
  ...
  let rec checkPure = function
    ...
    | (a, M.Mapp (M.Marg (M.Minus1, _), mS)), r ->
        error
          ( r,
            "Uniqueness modes (-1) not permitted in `%mode' declarations (use \
             `%unique')" )
