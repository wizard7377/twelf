...
module ModeTable : MODETABLE = struct (*! structure IntSyn = IntSyn' !*)
...
let rec modeLookup a  = match (Table.lookup modeSignature a) with Some (mS :: _) -> Some (mS) | None -> None
(* mmodeLookup a = mSs

       Looks up the modes of a in the mode array.
    *)

let rec mmodeLookup a  = match (Table.lookup modeSignature a) with Some mSs -> mSs | None -> []
(* installMode (a, mS) = ()
...
let rec uninstallMode a  = match modeLookup a with None -> false | Some _ -> (Table.delete modeSignature a; true)
(* installMmode (a, mS) = ()
...
let rec installMmode (a, mS)  = ( let mSs = mmodeLookup a in  Table.insert modeSignature (a, mS :: mSs) )
let reset = reset
