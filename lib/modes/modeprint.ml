...
module F = Formatter
module P = Print
let rec modeToString = function M.Plus -> "+" | M.Star -> "*" | M.Minus -> "-" | M.Minus1 -> "-1"
let rec argToString (M.Marg (m, _))  = modeToString m
let rec nameDec = function (I.Dec (_, V), M.Marg (_, name)) -> I.Dec (name, V) | (D, M.Marg (_, None)) -> D
let rec makeSpine (G)  = ( let rec makeSpine' = function (I.Null, _, S) -> S | (I.Decl (G, _), k, S) -> makeSpine' (G, k + 1, I.App (I.Root (I.BVar k, I.Nil), S)) in  makeSpine' (G, 1, I.Nil) )
