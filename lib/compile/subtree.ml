 ...
 *)
  ...
  let rec compatible (label, T, U, rho_t, rho_u) =
    let rec genExp = function
      | label, b, T, U ->
          ...
          T
      | label, b, T, U -> (
          if eqHeads (H1, H2) then I.Root (H1, genSpine (label, b, S1, S2))
          else
            match b with
            | Regular ->
      ...
      | label, b, I.Lam (D1, T1), I.Lam (D2, U2) ->
          I.Lam
            ( I.Dec (N, genExp (TypeLabel, Regular, A1, A2)),
              genExp (label, b, T1, U2) )
      | label, b, I.Pi (DD1, E1), I.Pi (DD2, E2) ->
          I.Pi
    ...
    let rec genTop = function
      | label, T, U ->
          if eqHeads (H1, H2) then I.Root (H1, genSpine (label, Regular, S1, S2))
          else raise (Generalization "Top-level function symbol not shared")
      | label, I.Lam (D1, T1), I.Lam (D2, U2) ->
          I.Lam
            (I.Dec (N, genExp (label, Regular, A1, A2)), genTop (label, T1, U2))
  ...
  let rec mkNode = function
    | Node (_, Children), sg, rho1, GR, rho2 ->
        let c = S.new_ () in
        S.insertList c [ (1, Node (rho1, Children)); (2, Leaf (rho2, G, RC)) ];
        Node (sg, c)
    | Leaf (_, G1, RC1), sg, rho1, GR, rho2 ->
        let c = S.new_ () in
        S.insertList c [ (1, Leaf (rho1, G1, RC1)); (2, Leaf (rho2, G2, RC2)) ];
        Node (sg, c)
  (* Insertion *)
  ...
  let rec compareChild (children, (n, child), nsub_t, nsub_e, GR) =
    ...
    | Some (sg, rho1, rho2) ->
        if isId rho1 then
          if isId rho2 then (* sg = nsub_t = nsub_e *)
            S.insertShadow children (n, mkNode (child, sg, rho1, GR, rho2))
          else (* sg = nsub_t and nsub_e = sg o rho2 *)
  ...
  and insert = function
    ...
    | N, nsub_e, GR -> (
        if noChildren children then (* initial *)
          (
  ...
  let rec assign (nvaronly, Glocal_u1, Us1, U2, nsub_goal, asub, csub, cnstr) =
    ...
    let rec assignHead (nvaronly, depth, Glocal_u1, Us1, U2, cnstr) =
      match (H1, H2) with
      ...
      | I.Def d1, _ ->
          assignExp (nvaronly, depth, Glocal_u1, Whnf.expandDef Us1, U2, cnstr)
    ...
    and assignExpW = function
      | nvaronly, depth, Glocal_u1, (I.Uni L1, s1), I.Uni L2, cnstr -> cnstr
      | nvaronly, depth, Glocal_u1, Us1, I.NVar n, cnstr ->
          S.insert nsub_goal (n, (Glocal_u1, (nvaronly, I.EClo Us1)));
          cnstr
      | Body, depth, Glocal_u1, Us1, U2, cnstr -> (
          match H2 with
          | I.BVar k2 -> (
              if k2 > depth then (
                (* BVar(k2) stands for_sml an existential variable *)
                (* S2 is an etaSpine by invariant *)
                S.insert asub
                  (k2 - I.ctxLength Glocal_u1, Assign (Glocal_u1, I.EClo Us1));
                cnstr)
              else
                match H1 with
                | I.BVar k1 ->
                    if k1 = k2 then
                      assignSpine (Body, depth, Glocal_u1, (S1, s1), S2, cnstr)
                    else raise (Assignment "Bound variable clash")
                | _ -> raise (Assignment "Head mismatch"))
          | _ -> assignHead (Body, depth, Glocal_u1, Us1, U2, cnstr))
      | TypeLabel, depth, Glocal_u1, Us1, U2, cnstr -> (
          match H2 with
          | I.BVar k2 -> (
              if k2 > depth then
                (* BVar(k2) stands for_sml an existential variable *)
                ...
                | I.BVar k1 ->
                    if k1 = k2 then
                      assignSpine
                        (TypeLabel, depth, Glocal_u1, (S1, s1), S2, cnstr)
                    else raise (Assignment "Bound variable clash")
                | _ -> raise (Assignment "Head mismatch"))
          | _ -> assignHead (TypeLabel, depth, Glocal_u1, Us1, U2, cnstr))
      | nvaronly, depth, Glocal_u1, Us1, U2, cnstr -> (
          if k2 > depth then (
            ...
            | Body -> (
                match Us1 with
                | I.EVar (r, _, V, Cnstr), s ->
                    let U2' = normalizeNExp (U2, csub) in
                    Eqn (Glocal_u1, I.EClo Us1, U2') :: cnstr
                | I.EClo (U, s'), s ->
                    assignExp
                      (Body, depth, Glocal_u1, (U, I.comp (s', s)), U2, cnstr)
                | I.FgnExp (_, ops), _ ->
                    let U2' = normalizeNExp (U2, csub) in
                    Eqn (Glocal_u1, I.EClo Us1, U2') :: cnstr))
      | nvaronly, depth, Glocal_u1, (I.Lam (D1, U1), s1), I.Lam (D2, U2), cnstr
        ->
    ...
    and assignSpine = function
      ...
      | nvaronly, depth, Glocal_u1, (I.SClo (S1, s1'), s1), S, cnstr ->
          assignSpine
            (nvaronly, depth, Glocal_u1, (S1, I.comp (s1', s1)), S, cnstr)
      | nvaronly, depth, Glocal_u1, (I.App (U1, S1), s1), I.App (U2, S2), cnstr
        ->
  ...
  let rec assignableEager (nsub, nsub_query, assignSub, cnstrSub, cnstr) =
    ...
    let rec assign' nsub_query nsub =
      ...
      in
      let _ =
        S.forall nsub_left (fun (nv, (nvaronly, U)) ->
  ...
  let rec unifyW = function
    | G, (X, I.Shift 0), Us2 -> r := Some (I.EClo Us2)
    | G, (X, s), Us2 ->
  ...
  let rec ctxToExplicitSub = function
    ...
    | i, Gquery, I.Decl (Gclause, I.Dec (_, A)), asub -> (
        ...
        let U' = I.newEVar (Gquery, I.EClo (A, s)) in
  ...
  let rec solveAuxG = function
    ...
    | C.UnifyEq (Glocal, e1, N, eqns), s, Gquery ->
        ...
        if unifiable (G, (N, s'), (e1, s')) then solveAuxG (eqns, s, Gquery)
        else false
  ...
  let rec solveResiduals
    ...
    in
    if success then sc ((ConjGoals, s), cid (* B *)) else ()
  ...
  let rec retrieveChild (num, Child, nsub_query, assignSub, cnstr, Gquery, sc) =
    let rec retrieve = function
      | Leaf (nsub, Gclause, Residuals), nsub_query, assignSub, cnstrSub, cnstr
        -> (
          match
          ...
          | Some (nsub_query', cnstrSub', cnstr') ->
              if isId nsub_query' (* cnstrSub' = empty? by invariant *) then
                (* LCO optimization *)
  ...
  let rec retrieval (n, STree, G, r, sc) =
    (* s = id *)
    ...
    let rec retrieve = function
      | ( Leaf (nsub, Gclause, Residuals),
          ...
          | Some (nsub_query', (nsub_left', cnstrSub'), cnstr') ->
              if isId nsub_query' then (* LCO optimization *)
                (
                i := !i + 1;
                S.insert candSet
                  ( !i,
  ...
  let rec retrieveCandidates (n, STree, Gquery, r, sc) =
    (* s = id *)
    let nsub_query, assignSub = (querySubId (), assignSubId ()) in
