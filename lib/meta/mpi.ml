...
module MTPi
  ...
  let History : StateSyn.state Ring.ring * StateSyn.state Ring.ring list ref =
    ref []
  ...
  let rec printFillResult (_, P) =
    let rec formatTuple (G, P) =
      let rec formatTuple' = function
        | F.Unit -> []
        | F.Inx (M, F.Unit) -> [ Print.formatExp (G, M) ]
        | F.Inx (M, P') ->
            Print.formatExp (G, M)
  ...
  let rec menuToString () =
    let rec menuToString' = function
      | k, [], (None, _) -> (Some k, "")
      | k, [], (kopt', _) -> (kopt', "")
      | k, Splitting O :: M, kOopt' ->
          let kOopt'' =
            if MTPSplitting.applicable O then (Some k, Some O) else kOopt'
          in
  ...
  let rec select k =
    let rec select' = function
      ...
      | 1, Splitting O :: _ ->
          ...
          let _ = delete () in
          let _ = map (fun S -> insert (MTPrint.nameState S)) S' in
          menu ();
          printMenu ()
      | 1, Recursion O :: _ ->
          ...
          let _ = pushHistory () in
          let _ = delete () in
          let _ = insert (MTPrint.nameState S') in
          menu ();
          printMenu ()
      | 1, Inference O :: _ ->
