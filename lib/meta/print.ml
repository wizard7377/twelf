...
let rec formatTag = function (G, S.Parameter l) -> [Fmt.String "<p>"] | (G, S.Lemma (S.Splits k)) -> [Fmt.String "<i"; Fmt.String (Int.toString k); Fmt.String ">"] | (G, S.Lemma (S.RL)) -> [Fmt.String "<i >"] | (G, S.Lemma (S.RLdone)) -> [Fmt.String "<i*>"]
...
let rec formatCtx = function (I.Null, B) -> [] | (I.Decl (I.Null, D), I.Decl (I.Null, T)) -> if ! Global.chatter >= 4 then [Fmt.HVbox (formatTag (I.Null, T) @ [Fmt.Break; Print.formatDec (I.Null, D)])] else [Print.formatDec (I.Null, D)] | (I.Decl (G, D), I.Decl (B, T)) -> if ! Global.chatter >= 4 then formatCtx (G, B) @ [Fmt.String ","; Fmt.Break; Fmt.Break] @ [Fmt.HVbox (formatTag (G, T) @ [Fmt.Break; Print.formatDec (G, D)])] else formatCtx (G, B) @ [Fmt.String ","; Fmt.Break] @ [Fmt.Break; Print.formatDec (G, D)]
(* formatState S = fmt'
