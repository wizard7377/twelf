  ...
  type state =
    | State of
        ...
        * (IntSyn.dctx (* Context of Hypothesis, in general not named *)
          * tag IntSyn.ctx)
        * (* Status information *)
        (FunSyn.for_sml * order)
        * (* Induction hypothesis, order       *)
          int
          (* length of meta context            *)
        * order (* Current order *)
        * int
        * FunSyn.for_sml list (* History of residual lemmas *)
        * FunSyn.for_sml
...
module StateSyn (Whnf : WHNF) (Conv : CONV) : STATESYN = struct
  ...
  let rec orderSub = function
    | Arg ((U, s1), (V, s2)), s -> Arg ((U, I.comp (s1, s)), (V, I.comp (s2, s)))
  ...
  let rec normalizeOrder = function
    | Arg (Us, Vs) -> Arg ((Whnf.normalize Us, I.id), (Whnf.normalize Vs, I.id))
