...
module UniqueSearch
  ...
  *)
              ...
              if isInstantiated X then sc (I.App (X', S), acc')
                ...
                solve
                  ( max,
                    ...
                    acc' )),
            acc )
    | max, depth, ps', (C.Exists (I.Dec (_, A), r), s), dp, sc, acc ->
        let X = I.newEVar (G, I.EClo (A, s)) in
        rSolve
          ( max,
            depth,
  ...
  and aSolve = function
    ...
    | (C.UnifyEq (G', e1, N, eqns), s), dp, cnstr, sc, acc ->
