  ...
  local
    ...
    *)
    ...
    (* purifyFor ((P, t), (Psi, F), s) = (t', Psi', s')
       ...
       and   Psi' |- s' : Psi1
    *)
    let rec purifyFor = function ((T.Unit, t), (Psi, T.True), s) -> (t, Psi, s)
...
*)
    ...
    (* purifyCtx (t, Psi) = (t', Psi', s')
       ...
       and   Psi' |- s' : Psi
    *)
    let rec purifyCtx = function (t as T.Shift k, Psi) -> (t, Psi, T.id)
    ...
    let rec coverageCheckPrg = function (W, Psi, T.Lam (D, P)) -> 
          coverageCheckPrg (W, I.Decl (Psi, D), P)
      | (W, Psi, T.New P) -> 
          coverageCheckPrg (W, Psi, P)
      | (W, Psi, T.PairExp (U, P)) -> 
          coverageCheckPrg (W, Psi, P)
      | (W, Psi, T.PairBlock (B, P)) -> 
          coverageCheckPrg (W, Psi, P)
