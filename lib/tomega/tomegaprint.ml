...
module TomegaPrint
    ...
    (Names : Names.NAMES)
    (Print : Print.PRINT) : Tomega.TOMEGAPRINT = struct
  ...
  let rec evarName n =
    let rec evarName' = function
      ...
      | Y :: L -> if Names.evarName (G, X) = n then Y else evarName' L
    in
    evarName' !evarList
  ...
  let rec formatFor' = function
    | Psi, T.All ((D, T.Explicit), F) -> (
        match D with
        | T.UDec D ->
            let G = T.coerceCtx Psi in
            let D' = Names.decName (G, D) in
            [
              Fmt.String "all {"; P.formatDec (G, D'); Fmt.String "}"; Fmt.Break;
            ]
...
*)
  ...
  *)
  ...
  and argsToSpine = function
    ...
    | T.Dot (T.Exp U, s), k, S -> argsToSpine (s, k - 1, T.AppExp (U, S))
    | T.Dot (T.Prg P, s), k, S -> argsToSpine (s, k - 1, T.AppPrg (P, S))

  and formatTuple (Psi, P) =
    let rec formatTuple' = function
      | T.Unit -> []
      | T.PairExp (M, T.Unit) -> [ Print.formatExp (T.coerceCtx Psi, M) ]
      | T.PairExp (M, P') ->
  ...
  and formatPrg3 = function
    ...
    | callname, (Psi, P) -> formatLet callname (Psi, P, [])
    | callname, (Psi, P) -> formatLet callname (Psi, P, [])
    | callname, (Psi, P) -> formatLet callname (Psi, P, [])
    | callname, (Psi, P) -> formatNew callname (Psi, P, [])
    | callname, (Psi, T.Redex (P, S)) -> formatRedex callname (Psi, P, S)
    | callname, (Psi, T.Lam (D, P)) ->
        Fmt.HVbox
          [
            Fmt.String "lam";
            Fmt.Space;
    ...
    | callname, (Psi, X) -> formatPrg3 callname (Psi, P)
    | callname, (Psi, X) -> Fmt.String (nameEVar X)
    | callname, (Psi, T.Case (T.Cases Cs)) ->
        Fmt.HVbox
          ((Fmt.String "case" :: Fmt.Break :: formatCases (1, Psi, Cs, callname))
          @ [ Fmt.String "." ])
  ...
  and formatNew = function
    | callname, (Psi, T.New (T.Lam (T.UDec D, P)), fmts) ->
        let G = T.coerceCtx Psi in
        let D' = Names.decName (G, D) in
        formatNew callname
          ( I.Decl (Psi, T.UDec D'),
  ...
  and formatHead callname (name, (max, index), Psi', s, Psi) =
    ...
    let S = argsToSpine (s, I.ctxLength Psi - max, T.Nil) in
    let Fspine = fmtSpine callname (Psi', S) in
    Fmt.Hbox [ Fmt.Space; Fmt.HVbox (Fmt.String name :: Fmt.Break :: Fspine) ]
  ...
  let rec formatPrg11 = function
    | name, (max, index), Psi, T.Lam (D, P), callname ->
        formatPrg11
          ( name,
            (max, index + 1),
            I.Decl (Psi, decName (T.coerceCtx Psi, D)),
  ...
  let rec formatPrg1 = function
    | name :: names, (max, index), Psi, T.PairPrg (P1, P2), callname ->
...
*)
  ...
  let rec formatPrg0 ((names, projs), T.Rec (D, P)) =
    (* number of ih. *)
    let max = 1 in
    Fmt.Vbox0
      ( 0,
...
*)
  ...
  let rec nameCtx = function
    ...
    | I.Decl (Psi, D) -> I.Decl (nameCtx Psi, D)
