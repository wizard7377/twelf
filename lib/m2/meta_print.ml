...
module MetaPrint
  ...
  let rec fmtPrefix GM =
    let rec fmtPrefix' = function
      | M.Prefix (I.Null, I.Null, I.Null), Fmt -> Fmt
      | ( M.Prefix
  ...
  let rec stateToString (M.State (name, GM, V)) =
    name ^ ":\n" ^ prefixToString GM ^ "\n--------------\n"
  ...
  let rec sgnToString = function
    | M.SgnEmpty -> ""
    | M.ConDec (e, S) ->
        (if !Global.chatter >= 4 (* use explicitly quantified form *) then
           Print.conDecToString e ^ "\n"
         else if
           !Global.chatter
           >= 3 (* use form without quantifiers, which is reparsable *)
