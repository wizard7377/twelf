  ...
  let rec convertOneFor cid =
    ...
    let rec convertFor' = function
      | I.Pi ((D, _), V), M.Mapp (M.Marg (M.Plus, _), mS), w1, w2, n ->
          ...
          in
          fun F ->
            ( T.All ((T.UDec (Weaken.strengthenDec (D, w1)), T.Explicit), F' F),
              F'' )
...
*)
  ...
  let rec menu () =
    ...
    | S.State (W, Psi, P, F) :: _ ->
        ...
        let F1 =
          map
            (fun (T.EVar (Psi, r, F, TC, TCs, X)) ->
              Names.varReset I.Null;
              S.Focus (T.EVar (TomegaPrint.nameCtx Psi, r, F, TC, TCs, X), W))
            Xs
        in
  ...
  let rec init names =
    ...
    let F =
      map
        ...
        Xs
    in
    let [ Ofix ] = map (fun f -> FixedPoint.expand (f, TC)) F in
