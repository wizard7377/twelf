...
module OSPath : OS_PATH = struct
  ...
  let fromString path =
    ...
    let isAbs =
      Stdlib.String.length path > 0 && Stdlib.String.get path 0 = '/'
    in
    let arcs =
      Stdlib.String.split_on_char '/' path
  ...
  let getParent path = match Filename.dirname path with "." -> "" | p -> p

  let splitDirFile path =
    { dir = Filename.dirname path; file = Filename.basename path }

  let joinDirFile { dir; file } =
    if dir = "" || dir = "." then file else Filename.concat dir file
  ...
  let splitBaseExt path =
    ...
    try
      ...
      if dot_idx = 0 then { base = base_name; ext = None }
      else
        let base = Stdlib.String.sub base_name 0 dot_idx in
        let ext =
          Stdlib.String.sub base_name (dot_idx + 1)
            (Stdlib.String.length base_name - dot_idx - 1)
        in
        { base; ext = Some ext }
    with Not_found -> { base = base_name; ext = None }

  let joinBaseExt { base; ext } =
    match ext with None -> base | Some e -> base ^ "." ^ e
  ...
  let mkAbsolute { path; relativeTo } =
    if Filename.is_relative path then Filename.concat relativeTo path else path
