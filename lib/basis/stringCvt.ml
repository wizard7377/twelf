...
module StringCvt : STRING_CVT = struct
  ...
  let padLeft c width s =
    ...
    if len >= width then s else Stdlib.String.make (width - len) c ^ s

  let padRight c width s =
    let len = Stdlib.String.length s in
    if len >= width then s else s ^ Stdlib.String.make (width - len) c
  ...
  let splitl pred reader state =
    let rec loop acc state =
      ...
      | Some (c, state') ->
          if pred c then loop (Stdlib.String.make 1 c :: acc) state'
          else (Stdlib.String.concat "" (Stdlib.List.rev acc), state)
    in
    loop [] state
  ...
  let skipWS reader state = dropl isSpace reader state
  ...
  let scanString scanner str =
    ...
    in
    match scanner reader 0 with None -> None | Some (result, _) -> Some result
end
