...
module Date : DATE = struct
  ...
  let date ~year ~month ~day ~hour ~minute ~second ~offset =
    if
      year < 0 || day < 1 || day > 31 || hour < 0 || hour > 23 || minute < 0
      || minute > 59 || second < 0 || second > 61
    then
      (* 61 allows for leap seconds *)
      raise Date
    else { year; month; day; hour; minute; second; offset }
  ...
  let toString d =
    Printf.sprintf "%04d-%s-%02d %02d:%02d:%02d" d.year
      (month_to_string d.month) d.day d.hour d.minute d.second
