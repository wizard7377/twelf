...
module CharArray : MONO_ARRAY = struct
  ...
  let array n c =
    if n < 0 || n > maxLen then raise (Invalid_argument "CharArray.array")
    else Bytes.make n c

  let fromList lst =
    let len = Stdlib.List.length lst in
    if len > maxLen then raise (Invalid_argument "CharArray.fromList")
    else
  ...
  let tabulate n f =
    if n < 0 || n > maxLen then raise (Invalid_argument "CharArray.tabulate")
    else Bytes.init n f
  ...
  let sub arr i =
    ...
    else Bytes.get arr i

  let update arr i c =
    if i < 0 || i >= Bytes.length arr then
      raise (Invalid_argument "CharArray.update")
    else Bytes.set arr i c

  let vector arr = Bytes.to_string arr
  ...
  let copy ~src ~dst ~di =
    ...
    else Bytes.blit src 0 dst di src_len
  ...
  let copyVec ~src ~dst ~di =
    ...
    else Bytes.blit_string src 0 dst di src_len
  ...
  let foldri f init arr =
    let rec loop i acc =
      if i < 0 then acc else loop (i - 1) (f (i, Bytes.get arr i, acc))
    in
  ...
  let foldr f init arr =
    let rec loop i acc =
      if i < 0 then acc else loop (i - 1) (f (Bytes.get arr i, acc))
    in
    loop (Bytes.length arr - 1) init
