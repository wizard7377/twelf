...
module CharVector : MONO_VECTOR = struct
  ...
  let appi f vec =
    for i = 0 to Stdlib.String.length vec - 1 do
      f (i, Stdlib.String.get vec i)
    done
  ...
  let foldli f init vec =
    let rec loop i acc =
      if i >= Stdlib.String.length vec then acc
  ...
  let exists pred vec =
    let rec loop i =
      if i >= Stdlib.String.length vec then false
      else if pred (Stdlib.String.get vec i) then true
      else loop (i + 1)
    in
    loop 0
  ...
  let all pred vec =
    let rec loop i =
      if i >= Stdlib.String.length vec then true
      else if not (pred (Stdlib.String.get vec i)) then false
      else loop (i + 1)
    in
    loop 0
