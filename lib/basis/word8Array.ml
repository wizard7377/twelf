...
module Word8Array : WORD8_ARRAY = struct
  ...
  let array n init =
    if n < 0 || n > maxLen then raise (Invalid_argument "Word8Array.array")
    else
      let arr =
  ...
  let fromList lst =
    ...
    if len > maxLen then raise (Invalid_argument "Word8Array.fromList")
    else
      let arr =
  ...
  let tabulate n f =
    if n < 0 || n > maxLen then raise (Invalid_argument "Word8Array.tabulate")
    else
      let arr =
        Bigarray.Array1.create Bigarray.int8_unsigned Bigarray.c_layout n
      in
  ...
  let sub arr i =
    ...
    else Bigarray.Array1.get arr i

  let update arr i v =
    if i < 0 || i >= Bigarray.Array1.dim arr then
      raise (Invalid_argument "Word8Array.update")
    else Bigarray.Array1.set arr i v

  let vector arr =
    let len = Bigarray.Array1.dim arr in
    let rec loop i acc =
      if i < 0 then acc else loop (i - 1) (Bigarray.Array1.get arr i :: acc)
    in
    loop (len - 1) []
  ...
  let copyVec ~src ~dst ~di =
    ...
    else Stdlib.List.iteri (fun i v -> Bigarray.Array1.set dst (di + i) v) src
  ...
  let foldri f init arr =
    let rec loop i acc =
      if i < 0 then acc else loop (i - 1) (f i (Bigarray.Array1.get arr i) acc)
    in
  ...
  let foldr f init arr =
    let rec loop i acc =
      if i < 0 then acc else loop (i - 1) (f (Bigarray.Array1.get arr i) acc)
    in
    loop (Bigarray.Array1.dim arr - 1) init
