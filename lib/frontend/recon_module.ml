...
(*! structure Paths : Paths.PATHS !*)
  ...
  type inst
  val coninst : (string list * string * Paths.region) * ExtSyn.term * Paths.region -> inst
  val strinst : (string list * string * Paths.region) * strexp * Paths.region -> inst
  type sigexp
  val thesig : sigexp
...
let rec strexp (ids, id, r) ()  = ( let qid = Names.Qid (ids, id) in  match Names.structLookup qid with None -> error (r, "Undeclared structure " ^ Names.qidToString (valOf (Names.structUndef qid))) | Some mid -> (mid, r) )
...
let rec coninst ((ids, id, r1), tm, r2) (ns, eqns)  = ( let qid = Names.Qid (ids, id) in  match Names.constLookupIn (ns, qid) with None -> error (r1, "Undeclared identifier " ^ Names.qidToString (valOf (Names.constUndefIn (ns, qid)))) | Some cid -> (cid, External tm(* this is wrong because constants in the sig being instantiated might incorrectly appear in tm -kw *)
, r2) :: eqns )
...
let rec applyEqns wherecl namespace  = ( let eqns = wherecl namespace in let table : eqnTable = IntTree.new_ (0) in let rec add (cid, Inst, r)  = (match IntTree.lookup table cid with None -> IntTree.insert table (cid, ref [(Inst, r)]) | Some rl -> rl := (Inst, r) :: (! rl)) in let _ = List.app add eqns in let rec doInst = function ((Internal cid, r), condec) -> (try ModSyn.strictify (ExtSyn.internalInst (condec, ModSyn.abbrevify (cid, IntSyn.sgnLookup cid), r)) with ExtSyn.Error msg -> raise (ExtSyn.Error (msg ^ "\nin instantiation generated for_sml " ^ Names.qidToString (Names.constQid cid)))) | ((External tm, r), condec) -> ModSyn.strictify (ExtSyn.externalInst (condec, tm, r)) in let rec transformConDec (cid, condec)  = (match IntTree.lookup table cid with None -> condec | Some ({ contents = l }) -> List.foldr doInst condec l) in  transformConDec )
let rec moduleWhere (module_, wherecl)  = ( (* val _ = IntSyn.resetFrom (mark, markStruct) *)
let (mark, markStruct) = IntSyn.sgnSize () in let module' = ModSyn.instantiateModule (module_, applyEqns wherecl) in let _ = Names.resetFrom (mark, markStruct) in  module' )
 end
