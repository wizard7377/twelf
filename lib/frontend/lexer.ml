...
module Lexer (Stream' : Stream.STREAM) : LEXER = struct
  ...
  *)
  ...
  let rec lex (inputFun : int -> string) =
    assert false (* The remaining functions do not access the state or *)
  ...
  let rec readNext () =
    ...
    if nextSize = 0 (* end of file? *) then (
      s := EOFString;
  ...
  and lexPragmaKey = function
    ...
    | ID (_, "mode"), r -> (MODE, r)
    | ID (_, "unique"), r -> (Unique.UNIQUE, r)
    | ID (_, "terminates"), r -> (TERMINATES, r)
    | ID (_, "block"), r -> (BLOCK, r)
    | ID (_, "worlds"), r -> (WORLDS, r)
    | ID (_, "covers"), r -> (Cover.COVERS, r)
    | ID (_, "total"), r -> (Total.TOTAL, r)
    | ID (_, "reduces"), r -> (Reduces.REDUCES, r)
    | ID (_, "tabled"), r -> (Tabled.Table.TABLED, r)
  ...
  and lexContinue'' = function
    | mt -> Stream.Cons (mt, lexContinueQualId j)
    | mt -> Stream.Cons (mt, lexContinue j)
  ...
  and lexContinueQualId' j =
    if char j = '.' then
      if isIdChar (char (j + 1)) then
        Stream.Cons ((Paths.PATHSEP, P.Reg (j, j + 1)), lexContinue (j + 1))
  ...
  let rec stringToNat s =
    ...
    let rec stn (i, n) =
      if i = l then n else stn (i + 1, (10 * n) + charToNat (String.sub (s, i)))
    in
    stn (0, 0)
