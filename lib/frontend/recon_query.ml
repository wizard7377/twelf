...
module ReconQuery
    ...
    (Print : Print.PRINT) : RECON_QUERY = struct
  ...
  *)
  ...
  let rec queryToQuery (Query (optName, tm), Paths.Loc (fileName, r)) =
    (* construct an external term for_sml the result of the query
    ...
    let _ =
      if freeVar (optName, Xs) then
        error (r, "Proof term variable " ^ valOf optName ^ " occurs in type")
      else ()
    in
    (V, optName, Xs)

  let rec finishDefine (Define (optName, tm, clsOpt), ((U, oc1), (V, oc2Opt), L))
      =
  ...
  *)
  ...
  let rec solveToSolve (defines, sol, Paths.Loc (fileName, r)) =
    ...
    let _ = T.checkErrors r in
    let _ =
      match L with IntSyn.Type -> () | _ -> error (r0, "Query was not a type")
    in
    let rec sc = function
      | M, [], _ -> (
