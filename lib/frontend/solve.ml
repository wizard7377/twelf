...
let rec exceeds = function (Some (n), Some (m)) -> (n >= m) | (Some (n), None) -> false | (None, _) -> true
(* boundEq : bound * bound -> bool *)

let rec boundEq = function (Some (n), Some (m)) -> (n = m) | (None, None) -> true | _ -> false
(* boundToString : bound -> string *)

let rec boundToString = function (Some (n)) -> Int.toString (n) | (None) -> "*"
(* boundMin : bound * bound -> bound *)

let rec boundMin = function (Some (n), Some (m)) -> Some (Int.min (n, m)) | (b, None) -> b | (None, b) -> b
...
let rec checkSolutions (expected, try_, solutions)  = if boundEq (boundMin (expected, try_), Some (solutions)) then () else raise (AbortQuery ("Query error -- wrong number of solutions: expected " ^ boundToString expected ^ " in " ^ boundToString try_ ^ " tries, but found " ^ Int.toString solutions))
...
let rec checkStages (try_, stages)  = if boundEq (try_, Some (stages)) then () else raise (AbortQuery ("Query error -- wrong number of stages: " ^ boundToString try_ ^ " tries, but terminated after  " ^ Int.toString stages))
...
let rec moreSolutions ()  = (print ("More? "); match String.sub (Compat.inputLine97 (TextIO.stdIn), 0) with 'y' -> true | 'Y' -> true | ';' -> true | 'q' -> raise (AbortQuery ("Query error -- explicit quit")) | 'Q' -> raise (AbortQuery ("Query error -- explicit quit")) | _ -> false)
...
let rec solve args  = match (! Compile.optimize) with (* solves A where program clauses are indexed using substitution trees
         and reconstructs the proof term X afterwards - bp
       *)
Compile.Indexing -> solveSbt args | Compile.LinearHeads -> solve' args | _ -> solve' args
(* %query <expected> <try> A or %query <expected> <try> X : A *)

let rec query' ((expected, try_, quy), Paths.Loc (fileName, r))  = ( (* optName = SOME(X) or NONE, Xs = free variables in query excluding X *)
(* times itself *)
...
let rec querySbt ((expected, try_, quy), Paths.Loc (fileName, r))  = ( (* optName = SOME(X) or NONE, Xs = free variables in query excluding X *)
(* times itself *)
(* Problem: we cannot give an answer substitution for_sml the variables
               in the printed query, since the new variables in this query
               may not necessarily have global scope.
...
let rec query args  = match (! Compile.optimize) with (* Execute query where program clauses are
          ...
          *)
Compile.Indexing -> querySbt args | Compile.LinearHeads -> query' args | _ -> query' args
...
let rec querytabled ((numSol, try_, quy), Paths.Loc (fileName, r))  = ( (* optName = SOME(X) or NONE, Xs = free variables in query excluding X *)
(* times itself *)
(* Problem: we cannot give an answer substitution for_sml the variables
        in the printed query, since the new variables in this query
        may not necessarily have global scope.
...
let _ = if ! Global.chatter >= 3 then Msg.message ("%querytabled " ^ boundToString numSol ^ " " ^ boundToString try_) else () in let (A, optName, Xs) = ReconQuery.queryToQuery (quy, Paths.Loc (fileName, r)) in let _ = if ! Global.chatter >= 4 then Msg.message (" ") else () in let _ = if ! Global.chatter >= 3 then Msg.message ("\n" ^ (Timers.time Timers.printing expToString) (IntSyn.Null, A) ^ ".\n") else () in let g = (Timers.time Timers.compiling Compile.compileGoal) (IntSyn.Null, A) in let solutions = ref 0 in let status = ref false in let solExists = ref false in let stages = ref 1 in let rec scInit O  = (solutions := ! solutions + 1; solExists := true; if ! Global.chatter >= 3 then (Msg.message ("\n---------- Solutions " ^ Int.toString (! solutions) ^ " ----------\n"); Msg.message ((Timers.time Timers.printing evarInstToString) Xs ^ " \n")) else if ! Global.chatter >= 1 then Msg.message "." else (); (match optName with None -> () | Some (name) -> (Msg.message (CompSyn.pskeletonToString O ^ "\n"); (Timers.time Timers.ptrecon PtRecon.solve) (O, (g, IntSyn.id), CompSyn.DProg (IntSyn.Null, IntSyn.Null), (fun (O, M) -> if ! Global.chatter >= 3 then Msg.message ((Timers.time Timers.printing evarInstToString) [(M, name)] ^ "\n") else ())))); (if ! Global.chatter >= 3 then (* Question: should we collect constraints in M? *)
match (Timers.time Timers.printing Print.evarCnstrsToStringOpt) Xs with None -> () | Some (str) -> Msg.message ("Remaining constraints:\n" ^ str ^ "\n") else ()); (if ! Global.chatter >= 1 then Msg.message "More solutions?\n" else ()); match numSol with None -> () | Some n -> (if (! solutions = n) then ((if ! Global.chatter >= 1 then Msg.message "Found enough solutions\n" else ()); raise (Done)) else ())) in let rec loop ()  = (if exceeds (Some (! stages - 1), try_) then ((if ! Global.chatter >= 1 then Msg.message ("\n ================= " ^ " Number of tries exceeds stages " ^ " ======================= \n") else ()); status := false; raise (Done)) else (); (if ! Global.chatter >= 1 then Msg.message ("\n ====================== Stage " ^ Int.toString (! stages) ^ " finished =================== \n") else ()); if exceeds (Some (! stages), try_) then (Msg.message ("\n ================= " ^ " Number of tries exceeds stages " ^ " ======================= \n"); status := false; raise (Done)) else (); if Tabled.nextStage () then (stages := (! stages) + 1; loop ()) else (* table did not change,
...
let (A, optName, Xs) = ReconQuery.queryToQuery (query, Paths.Loc ("stdIn", Paths.Reg (0, 0))) in let g = (Timers.time Timers.compiling Compile.compileGoal) (IntSyn.Null, A) in let rec scInit M  = ((if ! Global.chatter >= 1 then Msg.message ((Timers.time Timers.printing evarInstToString) Xs ^ "\n") else ()); match optName with None -> () | Some (name) -> if ! Global.chatter >= 3 then Msg.message ((Timers.time Timers.printing evarInstToString) [(M, name)] ^ "\n") else (); if ! Global.chatter >= 3(* Question: should we collect constraints from M *)
 then match (Timers.time Timers.printing Print.evarCnstrsToStringOpt) Xs with None -> () | Some (str) -> Msg.message ("Remaining constraints:\n" ^ str ^ "\n") else (); if moreSolutions () then () else raise (Done)) in let _ = if ! Global.chatter >= 3 then Msg.message "Solving...\n" else () in  try ((Timers.time Timers.solving AbsMachine.solve) ((g, IntSyn.id), CompSyn.DProg (IntSyn.Null, IntSyn.Null), scInit); (* scInit is timed into solving! *)
Msg.message "No more solutions\n") with Done -> (); (* Ignore s': parse one query at a time *)
qLoop () )
...
(* loops -- scinit will raise exception Done *)
let solExists = ref false in let (A, optName, Xs) = ReconQuery.queryToQuery (query, Paths.Loc ("stdIn", Paths.Reg (0, 0))) in let g = (Timers.time Timers.compiling Compile.compileGoal) (IntSyn.Null, A) in let _ = Tabled.reset () in let rec scInit O  = ((if ! Global.chatter >= 1 then Msg.message ((Timers.time Timers.printing evarInstToString) Xs ^ "\n") else ()); match optName with None -> () | Some (name) -> if ! Global.chatter >= 3 then Msg.message (" Sorry cannot reconstruct pskeleton proof terms yet \n") else (); if ! Global.chatter >= 3(* Question: should we collect constraints from M? *)
 then match (Timers.time Timers.printing Print.evarCnstrsToStringOpt) Xs with None -> () | Some (str) -> Msg.message ("Remaining constraints:\n" ^ str ^ "\n") else (); solExists := true; if moreSolutions () then () else raise (Done)) in let rec loop ()  = (if Tabled.nextStage () then loop () else (* table did not change,
                         * i.e. all solutions have been found
                         * we check for_sml *all* solutions
                         *)
raise (Completed)) in let _ = if ! Global.chatter >= 3 then Msg.message "Solving...\n" else () in  try ((Timers.time Timers.solving Tabled.solve) ((g, IntSyn.id), CompSyn.DProg (IntSyn.Null, IntSyn.Null), scInit); (* scInit is timed into solving! *)
(try loop () with Completed -> if ! solExists then Msg.message "No more solutions\n" else Msg.message "the query has no solution\n")) with Done -> (); (* Ignore s': parse one query at a time *)
qLoopT () )
 end
