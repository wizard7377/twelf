...
module type TWELF = sig
  ...
  module Table : sig
    ...
    (* strategy used for_sml %querytabled *)
    ...
    (* reset global table           *)
    val top : unit -> unit
    (* top-level for_sml interactive tabled queries *)
  end
  ...
  (* top-level for_sml interactive queries *)
  module Config : sig
    type config
...
(* TODO Fix module parameters with *)
module Twelf
    ...
    (Thm : Thm.THM)
    (ReconThm : Recon_thm.RECON_THM)
    (ThmPrint : Thm.Thmprint.THMPRINT)
    (TabledSyn : Tabled.Table.Tabledsyn.TABLEDSYN)
    (WorldSyn : Worldsyn.WORLDSYN)
    (Worldify : Worldify.WORLDIFY)
    (ModSyn : Modsyn.MODSYN)
    (ReconModule : Recon_module.RECON_MODULE)
    (MetaGlobal : Meta_global.METAGLOBAL)
    (Skolem : Skolem.SKOLEM)
    (Prover : Prover.PROVER)
    (ClausePrint : Clause_print.CLAUSEPRINT)
    (Trace : Trace.TRACE)
    (PrintTeX : Print.PRINT)
    (ClausePrintTeX : Clause_print.CLAUSEPRINT)
    (CSManager : Cs.Cs_manager.CS_MANAGER)
    (CSInstaller : Cs.Solvers.CS_INSTALLER)
    (Compat : Compat.COMPAT)
    (UnknownExn : Unknownexn.UNKNOWN_EXN)
    (Msg : Msg.MSG) : TWELF = struct
  ...
  let rec abortIO = function
    | fileName, { cause = OS.SysErr (m, _); function_ = f; name = n } ->
        msg ("IO Error on file " ^ fileName ^ ":\n" ^ m ^ "\n");
        ABORT
    | fileName, { function_ = f; _ } ->
        msg ("IO Error on file " ^ fileName ^ " from function " ^ f ^ "\n");
        ABORT
  ...
  let rec handleExceptions chlev fileName (f : 'a -> status) (x : 'a) =
    ...
    | Abstract.Error msg ->
        abortFileMsg chlev (fileName, msg)
  ...
  let rec installConDec fromCS (conDec, fileNameocOpt, r) =
    let _ =
  ...
  let rec installBlockDec fromCS (conDec, fileNameocOpt, r) =
  ...
  let rec installBlockDef fromCS (conDec, fileNameocOpt, r) =
    (* val _ = Origins.installOrigin (cid, fileNameocOpt) *)
    let cid = IntSyn.sgnAdd conDec in
    let _ =
      try
  ...
  let rec installStrDec (strdec, module_, r, isDef) =
    ...
    in
    let _ =
      try ModSyn.installStruct (strdec, module_, !context, installAction, isDef)
      with Names.Error msg -> raise (Names.Error (Paths.wrap (r, msg)))
    in
    ()
  ...
  let rec install1 = function
    | fileName, (Parser.ConDec condec, r) -> (
        try
          let optConDec, ocOpt =
            ReconConDec.condecToConDec (condec, Paths.Loc (fileName, r), false)
          in
          let rec icd = function
            | Some conDec ->
                (* allocate new cid. *)
                let cid =
                  installBlockDec IntSyn.Ordinary (conDec, (fileName, ocOpt), r)
                in
                ()
            | Some conDec ->
                (* allocate new cid. *)
                let cid =
                  installBlockDef IntSyn.Ordinary (conDec, (fileName, ocOpt), r)
                in
                ()
            | Some conDec ->
                  ...
                  if Subordinate.frozen [ a ] then
                    raise
                      (ModeTable.Error
                         (Paths.wrap
                            ( r,
                              "Cannot redeclare mode for_sml frozen constant "
            ...
            (fun mdec r ->
              try
                ...
                | _ ->
                    raise
                      (ModeTable.Error
                         "Cannot declare modes for_sml foreign constants")
...
*)
        ...
        let T, rrs = ReconThm.tdeclTotDecl lterm in
        let La = Thm.installTotal (T, rrs) in
        let _ = map Total.install La in
        let _ =
          try map Total.checkFam La with
        ...
        let _ =
          if !Global.chatter >= 3 then
            msg ("%total " ^ ThmPrint.tDeclToString T ^ ".\n")
          else ()
        in
        ()
    | fileName, (Parser.TerminatesDec lterm, _) ->
        ...
        let _ =
          if !Global.autoFreeze then (
            Subordinate.freeze La;
            ())
          else ()
        in
        let _ =
          if !Global.chatter >= 3 then
            msg ("%terminates " ^ ThmPrint.tDeclToString T ^ ".\n")
          else ()
        in
        ()
    | fileName, (Parser.ReducesDec lterm, _) ->
        ...
        (*
          ...
          *)
        (*  -bp6/12/99.   *)
        let R, rrs = ReconThm.rdeclTorDecl lterm in
        let (ThmSyn.RDecl (_, ThmSyn.Callpats callpats)) = R in
        let La = Thm.installReduces (R, rrs) in
        let _ =
          map (Timers.time Timers.terminate Reduces.checkFamReduction) La
        in
        let _ =
          if !Global.autoFreeze then (
            Subordinate.freeze La;
        ...
        let _ =
          if !Global.chatter >= 3 then
            msg ("%reduces " ^ ThmPrint.rDeclToString R ^ ".\n")
          else ()
        in
        ()
    | fileName, (Parser.TabledDec tdecl, _) ->
        ...
        let _ =
          if !Global.chatter >= 3 then
            msg ("%tabled " ^ ThmPrint.tabledDeclToString T ^ ".\n")
          else ()
        in
        ()
    | fileName, (Parser.KeepTableDec tdecl, _) ->
        ...
        let _ =
          if !Global.chatter >= 3 then
            msg ("%keeptabled " ^ ThmPrint.keepTableDeclToString T ^ ".\n")
          else ()
        in
        ()
    | fileName, (Parser.TheoremDec tdec, r) ->
        ...
        let _ = ReconTerm.checkErrors r in
        let GBs, E = ThmSyn.theoremDecToConDec (Tdec, r) in
        ...
        let _ =
          List.foldr
            (fun ((G1, G2), k) ->
              FunSyn.labelAdd
                (FunSyn.LabelDec
                   (Int.toString k, FunSyn.ctxToList G1, FunSyn.ctxToList G2)))
            0 GBs
        in
        ...
        let _ =
          if !Global.chatter >= 3 then
            msg ("%theorem " ^ Print.conDecToString E ^ "\n")
          else ()
        in
        ()
    | fileName, (Parser.ProveDec lterm, r) ->
        ...
        let _ =
          if !Global.chatter >= 3 then
            msg
              ("%prove " ^ Int.toString depth ^ " " ^ ThmPrint.tDeclToString T
             ^ ".\n")
          else ()
        in
        ...
        in
        let _ = if !Global.chatter >= 3 then msg "%Qed.QED\n" else () in
        Prover.install (fun E ->
    ...
    | fileName, (Parser.WorldDec wdecl, _) ->
        let ThmSyn.WDecl (qids, cp), rs = ReconThm.wdeclTowDecl wdecl in
        let _ =
          ListPair.app
            (fun ((a, _), r) ->
              if Subordinate.frozen [ a ] then
                  ...
                  (WorldSyn.Error
                     (Paths.wrapLoc
                        ( Paths.Loc (fileName, r),
                          "Cannot declare worlds for_sml frozen family "
                          ^ Names.qidToString (Names.constQid a) )))
              else ())
            (cpa, rs)
        in
    ...
    | fileName, declr -> install1WithSig (fileName, None, declr)
    | fileName, declr -> install1WithSig (fileName, None, declr)
    | fileName, declr -> install1WithSig (fileName, None, declr)
    | fileName, declr -> install1WithSig (fileName, None, declr)
    | fileName, (Parser.Use name, r) -> (
        match !context with
        | None -> Cs.CSManager.useSolver name
        | _ ->
            raise
              (ModSyn.Error
        ...
        let name =
          try
            ...
            | Some id ->
                ...
                id
            | None -> "_" (* anonymous *)
          with ModSyn.Error msg -> raise (ModSyn.Error (Paths.wrap (r, msg)))
        in
        let _ =
          if !Global.chatter >= 3 then msg ("%sig " ^ name ^ " = { ... }.\n")
          else ()
        in
        ()
  ...
  let rec installSubsig fileName s =
    ...
    let result =
      try
        ...
        let _ =
          if !Global.chatter >= 4 then msg "% end subsignature\n\n" else ()
        in
        Value (module_, s')
  ...
  let rec loadFile fileName =
    handleExceptions 0 fileName (withOpenIn fileName) (fun instream ->
  ...
  let rec topLoop () =
    match
      (handleExceptions 0 "stdIn" sLoop) () (* "stdIn" as fake fileName *)
    with
    | ABORT -> topLoop ()
    | OK -> ()
  ...
  let rec installCSMDec conDec optFixity mdecL =
    ...
    in
    let _ =
      if !Global.chatter >= 3 then msg (Print.conDecToString conDec ^ "\n")
      else ()
    in
    let _ =
  ...
  let _ = Cs.CSManager.setInstallFN installCSMDec
  ...
  module Config = struct
    ...
    let rec read config =
      ...
      let rec read' (sources, configs) config =
        withOpenIn config (fun instream ->
            let { dir = configDir; file = _ } = OS.Path.splitDirFile config in
            let rec parseItem (sources, configs) item =
              if isConfig item then
                if List.exists (fun config' -> item = config') configs then
                  (sources, configs) (* we have already read this one *)
                else read' (sources, item :: configs) item
              else if List.exists (fun source' -> item = source') sources then
                (sources, configs) (* we have already collected this one *)
              else (sources @ [ item ], configs)
            in
            let rec parseLine (sources, configs) line =
              if Substring.isEmpty line (* end of file *) then (sources, configs)
              else
            ...
            and parseLine' (sources, configs) line =
              ...
              then parseStream (sources, configs)
                ...
                let line' =
                  Substring.string (Substring.takel (not o Char.isSpace) line)
                in
      ...
      (pwdir, List.map ModFile.create (1 (read' ([], [ config ]) config)))
    (*
    ...
    and append (pwdir, sources) =
      ...
      let sources' =
        ...
        if pwdir = OS.FileSys.getDir () then sources
  ...
  module Print : sig
    ...
    let rec domains () = msg Cs.CSInstaller.version
  ...
  module Table : sig
    ...
    let rec top () =
      ...
      let rec topLoopT () =
        match
          (handleExceptions 0 "stdIn" sLoopT) () (* "stdIn" as fake fileName *)
        with
        | ABORT -> topLoopT ()
        | OK -> ()
      in
