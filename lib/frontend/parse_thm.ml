...
module ParseThm
  ...
  let rec parseOrderOpt = function
    ...
    | LS.Cons ((L.LBRACE, r), s') -> decideRBrace (r, parseOrders (LS.expose s'))
  ...
  let rec parseAssert' (LS.Cons ((L.ASSERT, r), s')) =
    ...
    (E.assert_ml (E.callpats callpats), f'')
  (* --------------------- *)
  ...
  let rec parsePredicate = function
    | LS.Cons ((L.ID (_, "<"), r), s') -> (E.predicate ("LESS", r), LS.expose s')
    | LS.Cons ((L.ID (_, "<="), r), s') -> (E.predicate ("LEQ", r), LS.expose s')
    | LS.Cons ((L.Eq, r), s') -> (E.predicate ("EQUAL", r), LS.expose s')
    | LS.Cons ((t, r), s') ->
        Parsing.error
