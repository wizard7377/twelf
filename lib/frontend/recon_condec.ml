  ...
  *)
  ...
  let rec condecToConDec = function
    | Condec (name, tm), Paths.Loc (fileName, r), abbFlag ->
        ...
        in
        let cd =
          Names.nameConDec (IntSyn.ConDec (name, None, i, IntSyn.Normal, V', L))
        in
    ...
    | Condef (optName, tm1, tm2Opt), Paths.Loc (fileName, r), abbFlag ->
        ...
        let (U, oc1), (V, oc2Opt), L =
          ...
          | ExtSyn.JOf ((U, oc1), (V, oc2), L) -> ((U, oc1), (V, Some oc2), L)
        in
        let _ = ExtSyn.checkErrors r in
        let i, (U'', V'') =
    ...
    | Blockdec (name, Lsome, Lblock), Paths.Loc (fileName, r), abbFlag ->
        ...
        let rec ctxToList = function
          | IntSyn.Null, acc -> acc
          | IntSyn.Decl (G, D), acc -> ctxToList (G, D :: acc)
        in
        let rec ctxAppend = function
          | G, IntSyn.Null -> G
          | G, IntSyn.Decl (G', D) -> IntSyn.Decl (ctxAppend (G, G'), D)
        in
