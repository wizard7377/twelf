...
(* GEN BEGIN SIGNATURE DECLARATION *) signature PARSER =
sig
  ...
  val parseStream: TextIO.instream -> (file_parse_result * Paths.region) Stream.stream
  val parseTerminalQ : string * string -> ExtQuery.query Stream.stream (* reads from std input *)

end (* GEN END SIGNATURE DECLARATION *);  (* signature PARSER *)
