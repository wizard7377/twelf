...
(* GEN BEGIN TAG INSIDE LET *) fun defSize x = (case x of 
   		       Sgn.DEF_TERM y => S.size_term y
		     | Sgn.DEF_TYPE y => S.size_tp y) (* GEN END TAG INSIDE LET *)

(* GEN BEGIN TAG INSIDE LET *) fun cidSize cid = (case I.sgnLookup cid of 
 ...
 | _ => 0) (* GEN END TAG INSIDE LET *)

(* GEN BEGIN TAG INSIDE LET *) fun o_cidSize cid = (case I.sgnLookup cid of 
...
(* GEN BEGIN TAG INSIDE LET *) val k : Reductio.eq_c option ref = ref NONE (* GEN END TAG INSIDE LET *)


exception Crap
(* GEN BEGIN TAG INSIDE LET *) fun sanityCheck cid = ((case I.sgnLookup cid of 
...
handle Syntax.Syntax _ => (print ("--> " ^ Int.toString cid); raise Match)) (* GEN END TAG INSIDE LET *)



(* GEN BEGIN TAG INSIDE LET *) fun gen_graph n autoCompress =
    let
