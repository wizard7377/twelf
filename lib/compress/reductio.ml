...
module Reductio = struct
  ...
  let rec tp_eq = function
    | TRoot (n, sp), TRoot (n', sp') -> type_const_head_eq (n, n', sp, sp')
    | TPi (m, a, b), TPi (m', a', b') -> m = m' && tp_eq (a, a') && tp_eq (b, b')
    | _ -> false
  ...
  and elt_eq' = function
    ...
    | Ascribe (t, a), Ascribe (t', a') -> ntm_eq (t, t') && tp_eq (a, a')
    | Omit, Omit -> true
    | _ -> false
  ...
  and atm_eq = function
    | tm, tm' -> const_head_eq (n, n', sp, sp', ATerm tm, ATerm tm')
  ...
  and ntm_eq' = function
    | tm, tm' -> const_head_eq (n, n', sp, sp', NTerm tm, NTerm tm')
  ...
  and const_head_eq (n, n', sp, sp', tm, tm') =
    ...
    in
    let rec redux t n sp = reduce (srTerm (t, typeOf (Sgn.classifier n)), sp) in
  ...
  let rec pp_shift (vs, shift) m =
    ...
    if m < len then (List.drop (vs, m), shift) else ([], m - len + shift)
  ...
  and pp_normalize' = function
    ...
    | TermDot (t, a, s) ->
        ...
        let v =
          ...
          with Strict.EtaContract -> raise Domain
        in
  ...
  let rec pp_invert vs shift =
    ...
    let rec search = function
      | n, [], (x : int) -> None
      | n, h :: tl, x -> if x = h then Some n else search (n + 1) tl x
    in
  ...
  and match_const_head (n, n', s, s', elt, elt', err) =
    ...
    let eq =
      ...
      | _, Sgn.DEF_TERM t, Sgn.DEF_TERM t' ->
  ...
  and constraint_gen' = function
    | G, ([], a, CG_CHECK a') -> ([ TypeC (a, a') ], [], None)
  ...
  and tp_constraint_gen = function
    | G, ([], Type) -> ([], [])
  ...
  and matching_succeeds G (p, q) =
    ...
    let p' = matching p in
    let _ =
      if check_equality_constraints p' then ()
      else raise (Matching "residual equality constraints failed")
    in
    let _ =
      if check_typing_constraints G q then ()
  ...
  and check_spinelt = function
    ...
    | G, Ascribe (t, a), a' -> tp_eq (a, a') && check (G, NTerm t, a)
  ...
  and synth = function
    ...
    | G, ARoot (Const n, s) ->
        ...
        let _ = matching_succeeds G (p, q) in
 ...
 *)
  ...
  let rec check_strictness_type = function
    | _, TRoot (n, s) -> true
