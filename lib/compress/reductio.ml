...
struct
	...
	type tp_c = term * tp
        ...
        (* equality checking *)
	(* GEN BEGIN TAG INSIDE LET *) fun tp_eq (TRoot (n, sp), TRoot(n', sp')) = type_const_head_eq(n, n', sp, sp')
	...
	(* GEN BEGIN TAG INSIDE LET *) fun eq_c_true (EltC(e,e')) = elt_eq(e, e')
	  | eq_c_true (SpineC(s,s')) = sp_eq(s, s')
	  | eq_c_true (TypeC(a,a')) = tp_eq(a, a') (* GEN END TAG INSIDE LET *)
	...
	(* GEN BEGIN TAG INSIDE LET *) fun pp_shift (vs,shift) m = 
	    let 
		val len = length vs
	    in
		if m < len
	...
	(* GEN BEGIN TAG INSIDE LET *) fun pp_nth (vs,shift) n = 
	    let 
		val len = length vs
	    in
		if n < len 
	...
	(* GEN BEGIN TAG INSIDE LET *) fun pp_o (pps, (vs, shift)) = 
	    let
	...
	(* GEN BEGIN TAG INSIDE LET *) fun pp_comp ppsl = foldr pp_o ([],0) ppsl (* GEN END TAG INSIDE LET *)
	...
	(* GEN BEGIN TAG INSIDE LET *) fun pp_normalize s = pp_normalize' s
	...
	and prepattern (s : subst) = pp_normalize s (* GEN END TAG INSIDE LET *)

	(* pp_ispat: is this ppsubst a pattern substitution? *)
	(* GEN BEGIN TAG INSIDE LET *) fun pp_ispat ([],shift) = true
	...
	(* GEN BEGIN TAG INSIDE LET *) fun makesubst ([],0) = Id
	  | makesubst ([],shift) = Shift (0, shift)
	  | makesubst (v::vs,shift) = VarOptDot (v, makesubst (vs,shift)) (* GEN END TAG INSIDE LET *)

        (* take in a ppsubst and return a substitution (which may involve VarOptDots) that is its inverse. *)
	(* GEN BEGIN TAG INSIDE LET *) fun pp_invert (vs,shift) =
	    let
	...
	(* GEN BEGIN TAG INSIDE LET *) fun flex_left ((r as ref NONE,a), s : subst, rhs) = 
	    let
	  ...
	  | flex_left _ = raise Error "evar invariant violated" (* GEN END TAG INSIDE LET *)
	...
	(* GEN BEGIN TAG INSIDE LET *) fun just_one c = [c]
	and just_one' c = [c]
	...
	(* GEN BEGIN TAG INSIDE LET *) fun matching (p) =  let
	    fun matching' (c::p,p') =
...
(*	fun ctxcons (a, G) = map (shift_tp 0) (a::G) *)
	(* GEN BEGIN TAG INSIDE LET *) fun ctxcons (a, G) = a::G (* GEN END TAG INSIDE LET *)
...
(* 	val constraint_gen : tp list -> spine * tp * cg_mode -> eq_c list * tp_c list
	...
	(* GEN BEGIN TAG INSIDE LET *) fun constraint_gen G (s, z, c) = constraint_gen' G (s, z, c)
...
(* DEBUG		 val _ = l3 := (p, q, aopt)::(!l3) *)
	...
	and elt_synth (G, AElt t) = synth (G, t)
	  ...
	  | elt_synth (G, Omit) = raise Error "trying to synthesize an omitted argument" (* GEN END TAG INSIDE LET *)

	(* GEN BEGIN TAG INSIDE LET *) fun check_plusconst_type t = check_type CON_PLUS ([], t) (* GEN END TAG INSIDE LET *)
	(* GEN BEGIN TAG INSIDE LET *) fun check_minusconst_type t = check_type CON_MINUS ([], t) (* GEN END TAG INSIDE LET *)
 ...
 *)
	(* GEN BEGIN TAG INSIDE LET *) fun check_strictness_type _ (TRoot(n, s)) = true
	  ...
	  | check_strictness_type plusconst (TPi(_,_,b)) = check_strictness_type plusconst b (* GEN END TAG INSIDE LET *)
							
	(* GEN BEGIN TAG INSIDE LET *) val check_plusconst_strictness = check_strictness_type true (* GEN END TAG INSIDE LET *)
	(* GEN BEGIN TAG INSIDE LET *) val check_minusconst_strictness = check_strictness_type false (* GEN END TAG INSIDE LET *)
end
