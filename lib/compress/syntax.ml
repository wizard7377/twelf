...
struct
	...
	(* GEN BEGIN TAG INSIDE LET *) fun EVarDotId ev = EVarDot (ev, [], Id) (* GEN END TAG INSIDE LET *)
...
(*	type ctx = decl list *)
	...
	(* GEN BEGIN TAG INSIDE LET *) fun termof (Elt t) = t
	  | termof (AElt t) = ATerm t
	  | termof (Ascribe(t, a)) = NTerm t
	  | termof (Omit) = raise Syntax "invariant violated: arguments to variables cannot be omitted" (* GEN END TAG INSIDE LET *)
	...
	(* GEN BEGIN TAG INSIDE LET *) fun curryfoldr sf sl x = foldr (fn (s,x') => sf s x') x sl (* GEN END TAG INSIDE LET *)
	...
	(* GEN BEGIN TAG INSIDE LET *) fun lower acc (a as TRoot _, []) = (a, acc)
...
(*
	...
	and substs_comp sl = foldr subst_compose Id sl (* GEN END TAG INSIDE LET *)

	(* GEN BEGIN TAG INSIDE LET *) fun elt_eroot_elim (AElt(t)) = eroot_elim_plus t
	  | elt_eroot_elim (Elt(ATerm(t))) = Elt(eroot_elim t)
	  | elt_eroot_elim x = x (* GEN END TAG INSIDE LET *)

	(* GEN BEGIN TAG INSIDE LET *) fun ntm_eroot_elim (Lam(ATerm(t))) = Lam(eroot_elim t)
	  | ntm_eroot_elim x = x (* GEN END TAG INSIDE LET *)



	(* GEN BEGIN TAG INSIDE LET *) fun ctxLookup (G, n) = subst_tp (Shift (0, n + 1)) (List.nth (G, n)) (* GEN END TAG INSIDE LET *)

	(* GEN BEGIN TAG INSIDE LET *) fun typeOf (tclass a) = a (* GEN END TAG INSIDE LET *)
	(* GEN BEGIN TAG INSIDE LET *) fun kindOf (kclass k) = k (* GEN END TAG INSIDE LET *)

	(* GEN BEGIN TAG INSIDE LET *) val sum = foldl op+ 0 (* GEN END TAG INSIDE LET *)
	(* GEN BEGIN TAG INSIDE LET *) fun size_term (NTerm (Lam t)) = 1 + (size_term t)
	...
	and size_knd (Type) = 1
	  | size_knd (KPi(_,a,b)) = 1 + size_tp a + size_knd b (* GEN END TAG INSIDE LET *)

     (* convert a kind to a context of all the pi-bound variables in it *) 
	(* GEN BEGIN TAG INSIDE LET *) fun explodeKind (Type) = []
	  | explodeKind (KPi(_,a,k)) = (explodeKind k) @ [a] (* GEN END TAG INSIDE LET *)
