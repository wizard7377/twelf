...
struct
  ...
  *)
  ...
  fun lookup dict x =
    let
      fun (* GEN BEGIN FUN FIRST *) lk (Empty) = false (* GEN END FUN FIRST *)
  ...
  *)
  ...
  fun insert (dict, x) =
    let
      ...
      fun (* GEN BEGIN FUN FIRST *) ins (Empty) = Red(x, Empty, Empty) (* GEN END FUN FIRST *)
  	| (* GEN BEGIN FUN BRANCH *) ins (Red(x1, left, right)) =
  	  (case Int.compare(x,x1)
  	     of EQUAL => Red(x, left, right)
  	      | LESS => Red(x1, ins left, right)
  	      | GREATER => Red(x1, left, ins right)) (* GEN END FUN BRANCH *)
  	| (* GEN BEGIN FUN BRANCH *) ins (Black(x1, left, right)) =
  	  (case Int.compare(x,x1)
  	     of EQUAL => Black(x, left, right)
  	      | LESS => restore_left (Black(x1, ins left, right))
  	      | GREATER => restore_right (Black(x1, left, ins right))) (* GEN END FUN BRANCH *)
    in
      case ins dict
  ...
  in
    ...
    fun foldl f a t =
        let fun (* GEN BEGIN FUN FIRST *) fo (Empty, r) = r (* GEN END FUN FIRST *)
              | (* GEN BEGIN FUN BRANCH *) fo (Red (x, left, right), r) =
                  	          fo (right, f (x, fo (left, r))) (* GEN END FUN BRANCH *)
              | (* GEN BEGIN FUN BRANCH *) fo (Black (x, left, right), r) =
                  		  fo (right, f (x, fo (left, r))) (* GEN END FUN BRANCH *)
    	in
    	  fo (t, a)
    	end
