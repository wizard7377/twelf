...
struct
  ...
  *)
  ...
  (* GEN BEGIN TAG INSIDE LET *) fun lookup dict x =
    let
      fun lk (Empty) = false
  	| lk (Red tree) = lk' tree
  ...
  *)
  ...
  (* GEN BEGIN TAG INSIDE LET *) fun insert (dict, x) =
    let
  ...
  in
    type intset = rbt
    (* GEN BEGIN TAG INSIDE LET *) val empty = Empty (* GEN END TAG INSIDE LET *)
    (* GEN BEGIN TAG INSIDE LET *) val insert = fn (x,t) => insert (t, x) (* GEN END TAG INSIDE LET *)
    (* GEN BEGIN TAG INSIDE LET *) val member = fn (x,t) => lookup t x (* GEN END TAG INSIDE LET *)
    (* GEN BEGIN TAG INSIDE LET *) fun foldl f a t =
