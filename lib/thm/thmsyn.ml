...
module ThmSyn (Abstract : ABSTRACT) (Whnf : WHNF) (Names' : NAMES) : THMSYN = struct (*! structure IntSyn = IntSyn !*)
...
type thDecl = ThDecl of IntSyn.dec IntSyn.ctx * IntSyn.dec IntSyn.ctx list * IntSyn.dec IntSyn.ctx * ModeSyn.mode IntSyn.ctx * int
(* Proof declaration *)
...
let rec theoremDecToConDec ((name, ThDecl (GBs, G, MG, i)), r)  = ( (* theoremToConDec' G V = V'
             ...
             and  . |-  V' : kind
          *)
let rec theoremToConDec' = function (I.Null, V) -> V | (I.Decl (G, D), V) -> if Abstract.closedDec (G, (D, I.id)) then theoremToConDec' (G, Abstract.piDepend ((Whnf.normalizeDec (D, I.id), I.Maybe), V)) else error (r, "Free variables in theorem declaration") in  (GBs, I.ConDec (name, None, i, I.Normal, theoremToConDec' (G, I.Uni (I.Type)), I.Kind)) )
...
let rec theoremDecToModeSpine ((name, ThDecl (GBs, G, MG, i)), r)  = ( let rec theoremToModeSpine' = function (I.Null, I.Null, mS) -> mS | (I.Decl (G, I.Dec (x, _)), I.Decl (MG, m), mS) -> theoremToModeSpine' (G, MG, M.Mapp (M.Marg (m, x), mS)) in  theoremToModeSpine' (G, MG, M.Mnil) )
