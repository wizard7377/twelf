...
module Timers (Timing' : TIMING) : TIMERS = struct module Timing = Timing'
...
let centers = [parsing; recon; abstract; checking; modes; subordinate; terminate; printing; compiling; solving; coverage; worlds; ptrecon; filling; filltabled; splitting; recursion; inference; delphin]
let total = Timing.sumCenter ("Total         ", centers)
let time = Timing.time
let rec reset ()  = List.app Timing.reset centers
let rec check ()  = (List.app (print o Timing.toString) centers; print (Timing.sumToString total); print "Remember that the success continuation counts into Solving!\n")
