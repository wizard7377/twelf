...
module Timing : TIMING = struct (* user and system time add up to total CPU time used *)
...
let rec minus ({usr = t1; sys = t2; gc = t3}, {usr = s1; sys = s2; gc = s3})  = {usr = Time.- (t1, s1); sys = Time.- (t2, s2); gc = Time.- (t3, s3)}
let rec plus ({usr = t1; sys = t2; gc = t3}, {usr = s1; sys = s2; gc = s3})  = {usr = Time.+ (t1, s1); sys = Time.+ (t2, s2); gc = Time.+ (t3, s3)}
...
let rec checkCPUAndGCTimer timer  = ( let {usr = usr; sys = sys} = Compat.Timer.checkCPUTimer timer in let gc = Compat.Timer.checkGCTime timer in  {usr = usr; sys = sys; gc = gc} )
let rec time (_, counters) (f : 'a -> 'b) (x : 'a)  = ( let realTimer = Timer.startRealTimer () in let CPUTimer = Timer.startCPUTimer () in let result = try Value (f x) with exn -> Exception (exn) in let evalCPUTime = checkCPUAndGCTimer (CPUTimer) in let evalRealTime = Timer.checkRealTimer (realTimer) in let (CPUTime, realTime) = ! counters in let _ = counters := (plus (CPUTime, evalCPUTime), Time.+ (realTime, evalRealTime)) in  match result with Value (v) -> v | Exception (e) -> raise (e) )
...
let rec timesToString (name, (CPUTime, realTime))  = name ^ ": " ^ "Real = " ^ stdTime (7, realTime) ^ ", " ^ "Run = " ^ stdTime (7, sum CPUTime) ^ " " ^ "(" ^ stdTime (7, t1) ^ " usr, " (* ^ stdTime (5, t2) ^ " sys, " ^ *)
 (* elide sys time *)
 ^ stdTime (6, t3) ^ " gc)" ^ "\n"
let rec toString (name, { contents = (CPUTime, realTime) })  = timesToString (name, (CPUTime, realTime))
let rec sumToString (name, centers)  = ( let rec sumup = function ([], (CPUTime, realTime)) -> timesToString (name, (CPUTime, realTime)) | ((_, { contents = (C, R) }) :: centers, (CPUTime, realTime)) -> sumup (centers, (plus (CPUTime, C), Time.+ (realTime, R))) in  sumup (centers, (zero, Time.zeroTime)) )
(* local ... *)
...
module Counting : TIMING = struct type 'a result = Value of 'a | Exception of exn
type center = string * int ref
type sum = string * center list
let rec init ()  = ()
let rec newCenter (name)  = (name, ref 0)
let rec reset (_, counters)  = (counters := 0)
let rec time (_, counters) (f : 'a -> 'b) (x : 'a)  = ( let _ = counters := ! counters + 1 in  f x )
let rec sumCenter (name, l)  = (name, l)
let rec toString' (name, n)  = name ^ ": " ^ Int.toString n ^ "\n"
let rec toString (name, { contents = n })  = toString' (name, n)
let rec sumToString (name, centers)  = ( let rec sumup = function ([], total) -> toString' (name, total) | ((_, { contents = n }) :: centers, total) -> sumup (centers, total + n) in  sumup (centers, 0) )
 end
