...
%************************************************************************
*)
functor (* GEN BEGIN FUNCTOR DECL *) Formatter(): FORMATTER =
   struct
(*
\subsection{Setting default values}
*)
      (* GEN BEGIN TAG OUTSIDE LET *) val Indent  = ref 3
      and Skip    = ref 1
      and Blanks  = ref 1
      and Pagewidth = ref 80 (* GEN END TAG OUTSIDE LET *)

      (* GEN BEGIN TAG OUTSIDE LET *) val Bailout = ref true (* GEN END TAG OUTSIDE LET *)
      (* GEN BEGIN TAG OUTSIDE LET *) val BailoutIndent = ref 0 (* GEN END TAG OUTSIDE LET *)
      (* GEN BEGIN TAG OUTSIDE LET *) val BailoutSpot = ref 40 (* GEN END TAG OUTSIDE LET *)
(*
%************************************************************************
...
*)
      local
         fun (* GEN BEGIN FUN FIRST *) Spaces' 0 s = s (* GEN END FUN FIRST *)
          |  (* GEN BEGIN FUN BRANCH *) Spaces' n s = Spaces' (n-1) (s^" ") (* GEN END FUN BRANCH *)
         fun Spaces n = if n>0 then Spaces' n "" else ""
         fun (* GEN BEGIN FUN FIRST *) Newlines' 0 s = s (* GEN END FUN FIRST *)
          |  (* GEN BEGIN FUN BRANCH *) Newlines' n s = Newlines' (n-1) (s^"\n") (* GEN END FUN BRANCH *)
         fun Newlines n = if n>0 then Newlines' n "" else ""
      in
        (* GEN BEGIN TAG OUTSIDE LET *) val Sp = Spaces (* GEN END TAG OUTSIDE LET *) (* return a number of spaces *)
        fun Spmod n = Spaces (n mod (!Pagewidth))
        (* GEN BEGIN TAG OUTSIDE LET *) val Nl = Newlines (* GEN END TAG OUTSIDE LET *) (* return a number of newlines *)
        fun Np() = "\n\012\n" (* CTRL_L == "\012" *)
      end
...
*)
fun (* GEN BEGIN FUN FIRST *) Width0(m,   b,i, Str(n,_)) = (n,n) (* GEN END FUN FIRST *)
 |  (* GEN BEGIN FUN BRANCH *) Width0(Hori,b,i, Brk(m,_)) = (m,m) (* GEN END FUN BRANCH *)
 |  (* GEN BEGIN FUN BRANCH *) Width0(Vert,b,i, Brk(_,n)) = (n,n) (* GEN END FUN BRANCH *)
 |  (* GEN BEGIN FUN BRANCH *) Width0(Hori,b,i, Dbk)      = (b,b) (* GEN END FUN BRANCH *)
 |  (* GEN BEGIN FUN BRANCH *) Width0(Vert,b,i, Dbk)      = (i,i) (* GEN END FUN BRANCH *)
 |  (* GEN BEGIN FUN BRANCH *) Width0(m,b,i, Ebk)      = (0,0) (* GEN END FUN BRANCH *)
 |  (* GEN BEGIN FUN BRANCH *) Width0(m,b,i, Vbx((min,max),_,_,_))      = (min,max) (* GEN END FUN BRANCH *)
 |  (* GEN BEGIN FUN BRANCH *) Width0(m,b,i, Hbx((min,max),_,_))        = (min,max) (* GEN END FUN BRANCH *)
 |  (* GEN BEGIN FUN BRANCH *) Width0(m,b,i, Hvx(((min,max),_),_,_,_,_)) = (min,max) (* GEN END FUN BRANCH *)
 |  (* GEN BEGIN FUN BRANCH *) Width0(m,b,i, Hov(((min,max),_),_,_,_,_)) = (min,max) (* GEN END FUN BRANCH *)

fun Width fmt = Width0(Hori,!Blanks,!Indent,fmt)

(* GEN BEGIN TAG OUTSIDE LET *) val Unused = ~9999 (* GEN END TAG OUTSIDE LET *)  (* a bad value to mark unused arguments of Width0 *)
(*
{\bf Caution:}
...
*)
local
   fun (* GEN BEGIN FUN FIRST *) vlistWidth'(i,nil,(totmin,totmax),(tmmin,tmmax)) =
                  (Max(totmin,tmmin), Max(totmax,tmmax)) (* GEN END FUN FIRST *)
    |  (* GEN BEGIN FUN BRANCH *) vlistWidth'(i,Dbk::t, (totmin,totmax), (tmmin,tmmax)) =
                    vlistWidth'(i,t, (Max(totmin,tmmin),Max(totmax,tmmax)),
                                Width0(Vert,Unused,i,Dbk)) (* GEN END FUN BRANCH *)
    |  (* GEN BEGIN FUN BRANCH *) vlistWidth'(i,(b as (Brk(_)))::t, (totmin,totmax), (tmmin,tmmax)) =
                    vlistWidth'(i,t, (Max(totmin,tmmin),Max(totmax,tmmax)),
                                Width0(Vert,Unused,i,b)) (* GEN END FUN BRANCH *)
    |  (* GEN BEGIN FUN BRANCH *) vlistWidth'(i,x::t, (totmin,totmax), (tmmin,tmmax)) =
                    vlistWidth'(i,t,(totmin,totmax),
                                sumpair((Width0(Vert,Unused,i,x)),
                                         (tmmin,tmmax))) (* GEN END FUN BRANCH *)
in
...
fun hlistWidth(l,blanks) =
              List.foldr ((* GEN BEGIN FUNCTION EXPRESSION *) fn (fmt,(x,y)) =>
                        sumpair(Width0(Hori,blanks,Unused,fmt),(x,y)) (* GEN END FUNCTION EXPRESSION *))
                    (0,0) l
(*
...
fun hovlistWidth(l,blanks,indent) =
              let (* GEN BEGIN TAG OUTSIDE LET *) val (vmin,vmax) = vlistWidth(l,indent)
                  and (hmin,hmax) = hlistWidth(l,blanks) (* GEN END TAG OUTSIDE LET *)
                  (* GEN BEGIN TAG OUTSIDE LET *) val (min,mmode) = if vmin<hmin then (vmin,Vert)
                                                 else (hmin,Hori) (* GEN END TAG OUTSIDE LET *)
                in
                 ( (min,hmax), (mmode,Hori) )
              end
(*
...
*)
(* GEN BEGIN TAG OUTSIDE LET *) val hvlistWidth = hovlistWidth (* GEN END TAG OUTSIDE LET *)
...
(* GEN BEGIN TAG OUTSIDE LET *) val Break    = Dbk (* GEN END TAG OUTSIDE LET *)
fun Break0 b i = Brk(b,i)
fun String s = Str(size s,s)
fun String0 i s = Str(i, s)
(* GEN BEGIN TAG OUTSIDE LET *) val Space    = Str(1, Sp 1) (* GEN END TAG OUTSIDE LET *)
...
*)
  fun summaxwidth l =
      (List.foldr ((* GEN BEGIN FUNCTION EXPRESSION *) fn (fmt,ysum) =>
              let (* GEN BEGIN TAG OUTSIDE LET *) val (_,y) = Width0(Hori,Unused,Unused,fmt) (* GEN END TAG OUTSIDE LET *)
              in y + ysum end (* GEN END FUNCTION EXPRESSION *))
             0
             l)
(*
...
fun (* GEN BEGIN FUN FIRST *) gh(nil,nil,_) = nil (* GEN END FUN FIRST *)
  | (* GEN BEGIN FUN BRANCH *) gh(cg,nil,res) = rev ((summaxwidth cg,cg,Ebk)::res) (* GEN END FUN BRANCH *)
  | (* GEN BEGIN FUN BRANCH *) gh(cg,(Dbk::t),res) = gh(nil,t,(summaxwidth cg,cg,Dbk)::res) (* GEN END FUN BRANCH *)
  | (* GEN BEGIN FUN BRANCH *) gh(cg,((b as (Brk(_,_)))::t),res) =
                  gh(nil,t,(summaxwidth cg,cg,b)::res) (* GEN END FUN BRANCH *)
  | (* GEN BEGIN FUN BRANCH *) gh(cg,(h::t),res) = gh(cg@[h],t,res) (* GEN END FUN BRANCH *)
(*
...
\end{itemize}
*)
fun (* GEN BEGIN FUN FIRST *) pphv(mw,li,bl,is,ss,mp,ch,lb,nil,res)= (Max(mp,ch),res) (* GEN END FUN FIRST *)
 |  (* GEN BEGIN FUN BRANCH *) pphv(mw,li,bl,is,ss,mp,ch,lb,((gpwdth,flist,brk)::t),res) =
    let (* GEN BEGIN TAG OUTSIDE LET *) val (ch1,s1,mp) =
        (* horizontal width, string to print, max print width *)
            ...
            then (* OK - group fits within page or has to fit:
                    horizontal break *)
            let (* GEN BEGIN TAG OUTSIDE LET *) val (n,s)=print'p(mw,li,bl,is,ss,Hori,lb,res) (* GEN END TAG OUTSIDE LET *)
            in (ch+n,s,mp) end
            else (* group will not fit: vertical break.
                    Was last line of maximum width? *)
            let (* GEN BEGIN TAG OUTSIDE LET *) val (n,s)=print'p(mw,li,bl,is,ss,Vert,lb,res) (* GEN END TAG OUTSIDE LET *) in
                (n,s,Max(mp,ch))
            end (* GEN END TAG OUTSIDE LET *)
 (* Now print the elements of the group using default for horizontal tabs *)
       (* GEN BEGIN TAG OUTSIDE LET *) val (n2,s2) = pph(mw,(li+ch1),bl,is,ss,flist,0,s1) (* GEN END TAG OUTSIDE LET *)
       (* Now print rest of horizontal-vertical box *)
    in pphv(mw,li,bl,is,ss,mp,(ch1+n2),brk,t,s2) end (* GEN END FUN BRANCH *)
(*
%{\bf Improvements:}
...
\end{itemize}
*)
and (* GEN BEGIN FUN FIRST *) ppv(mw,li,ci,bl,is,ss,max,gw,nil,res) =
                          (Max(max,gw),res) (* GEN END FUN FIRST *)
  | (* GEN BEGIN FUN BRANCH *) ppv(mw,li,ci,bl,is,ss,max,gw, Dbk::t,res) =
        let (* GEN BEGIN TAG OUTSIDE LET *) val (n,s)   = print'p(mw,li,bl,is,ss,Vert,Dbk,res) (* GEN END TAG OUTSIDE LET *)
        in ppv(mw,li,(li+n),bl,is,ss,Max(max,gw), n,t, s) end (* GEN END FUN BRANCH *)
  |  (* GEN BEGIN FUN BRANCH *) ppv(mw,li,ci,bl,is,ss,max,gw,(b as (Brk(_,_)))::t,res) =
        let (* GEN BEGIN TAG OUTSIDE LET *) val (n,s)   = print'p(mw,li,bl,is,ss,Vert,b,res) (* GEN END TAG OUTSIDE LET *)
        in ppv(mw,li,(li+n),bl,is,ss,Max(max,gw), n,t,s) end (* GEN END FUN BRANCH *)
  |  (* GEN BEGIN FUN BRANCH *) ppv(mw,li,ci,bl,is,ss,max,gw,h::t,res) =
          let (* GEN BEGIN TAG OUTSIDE LET *) val (n,s)   = print'p(mw,ci,bl,is,ss,Vert,h,res) (* GEN END TAG OUTSIDE LET *)
          in ppv(mw,li,(ci+n),bl,is,ss,max,(gw+n),t,s) end (* GEN END FUN BRANCH *)
...
\end{itemize}
*)
and (* GEN BEGIN FUN FIRST *) pph(mw,id,bl,is,ss,nil,nres,sres) = (nres,sres) (* GEN END FUN FIRST *)
  | (* GEN BEGIN FUN BRANCH *) pph(mw,id,bl,is,ss, h::t,nres,sres) =
          let (* GEN BEGIN TAG OUTSIDE LET *) val (n,s)   = print'p(mw,id,bl,is,ss,Hori,h,sres) (* GEN END TAG OUTSIDE LET *)
          in pph(mw,(id+n),bl,is,ss,t,n+nres,s) end (* GEN END FUN BRANCH *)
(*
...
*)
and (* GEN BEGIN FUN FIRST *) print'p(mw,id,bl,is,ss,mo,  Str(n,s),res) = (n,s::res) (* GEN END FUN FIRST *)
 |  (* GEN BEGIN FUN BRANCH *) print'p(mw,id,bl,is,ss,Hori,Brk(b,i),res) =
           (b, (if (!Bailout) then Spmod(b) else Sp(b))::res) (* GEN END FUN BRANCH *)
 |  (* GEN BEGIN FUN BRANCH *) print'p(mw,id,bl,is,ss,Vert, Brk(b,i), res) =
           (i, (if (!Bailout) then Spmod(id+i) else Sp(id+i))::(Nl(ss))::res) (* GEN END FUN BRANCH *)
 |  (* GEN BEGIN FUN BRANCH *) print'p(mw,id,bl,is,ss,Hori, Dbk, res) =
           (bl, (if (!Bailout) then Spmod(bl) else Sp(bl))::res) (* GEN END FUN BRANCH *)
 |  (* GEN BEGIN FUN BRANCH *) print'p(mw,id,bl,is,ss,Vert, Dbk, res)      =
           (is,(if (!Bailout) then Spmod(id+is) else Sp(id+is))::(Nl(ss))::res) (* GEN END FUN BRANCH *)
 |  (* GEN BEGIN FUN BRANCH *) print'p(mw,id,bl,is,ss,mo,   Ebk,res)      = (0,res) (* GEN END FUN BRANCH *)
 |  (* GEN BEGIN FUN BRANCH *) print'p(mw,id,bl,is,ss,mo,   Hbx((min,max),blanks,l),res) =
            ...
            else pph(mw,id,blanks,is,ss,l,0,res) (* GEN END FUN BRANCH *)
 |  (* GEN BEGIN FUN BRANCH *) print'p(mw,id,bl,is,ss,mo,   Vbx((min,max),indent,skip,l), res) =
           if (!Bailout) andalso (id+min) >= mw
               andalso (id mod (!Pagewidth) >= !BailoutSpot)
           then let (* GEN BEGIN TAG OUTSIDE LET *) val id = mw+(!BailoutIndent) (* GEN END TAG OUTSIDE LET *)
                in ppv(mw+(!Pagewidth),id,id,bl,indent,skip,0,0,l,(Nl(ss))::res) end
           else ppv(mw,id,id,bl,indent,skip,0,0,l,res) (* GEN END FUN BRANCH *)
 |  (* GEN BEGIN FUN BRANCH *) print'p(mw,id,bl,is,ss,mo,   Hvx(((min,max),(nmode,xmode)),blanks,indent,skip,l), res) =
            let (* GEN BEGIN TAG OUTSIDE LET *) val gl=gh(nil,l,nil) (* GEN END TAG OUTSIDE LET *) in
            if (!Bailout) andalso (id+min) >= mw
               andalso (id mod (!Pagewidth) >= !BailoutSpot)
 ...
 |  (* GEN BEGIN FUN BRANCH *) print'p(mw,id,bl,is,ss,mo,   Hov(((min,max),(nmode,xmode)),blanks,indent,skip,l), res) =
             if (max<=(mw-id))
             then if xmode=Hori
                  ...
                  then if nmode=Hori
                       then pph(mw+(!Pagewidth),mw + (!BailoutIndent),
                                  blanks,is,ss,l,0,(Nl(ss))::res)
                       else let (* GEN BEGIN TAG OUTSIDE LET *) val id = mw + (!BailoutIndent) (* GEN END TAG OUTSIDE LET *)
                            in ppv(mw+(!Pagewidth),id,id,
                                  blanks,indent,skip,0,0,l,(Nl(ss))::res) end
                  else if nmode=Hori
                       then pph(mw,id,blanks,is,ss,l,0,res)
                       else ppv(mw,id,id,blanks,indent,skip,0,0,l,res) (* GEN END FUN BRANCH *)
(*
%{\bf Improvements:}
...
*)
      ...
      fun print_fmt fm =
          List.foldr ((* GEN BEGIN FUNCTION EXPRESSION *) fn (s,_) => print s (* GEN END FUNCTION EXPRESSION *))
                ()
...
*)
      ...
      fun output_fmt(Formatstream outs,fm) =
          List.foldr ((* GEN BEGIN FUNCTION EXPRESSION *) fn (s,_) => TextIO.output(outs, s) (* GEN END FUNCTION EXPRESSION *))
                ()
                (snd(print'p(!Pagewidth,0,!Blanks,!Indent,!Skip,Hori,fm,nil)))
      ...
      fun file_open_fmt filename =
         let (* GEN BEGIN TAG OUTSIDE LET *) val fmt_stream = open_fmt(TextIO.openOut filename) (* GEN END TAG OUTSIDE LET *)
             (* GEN BEGIN TAG OUTSIDE LET *) val close_func = (* GEN BEGIN FUNCTION EXPRESSION *) fn () => ( TextIO.closeOut(close_fmt(fmt_stream)) ) (* GEN END FUNCTION EXPRESSION *) (* GEN END TAG OUTSIDE LET *)
          in (close_func, fmt_stream) end

      fun with_open_fmt filename func =
         let (* GEN BEGIN TAG OUTSIDE LET *) val (close_func, fmt_stream) = file_open_fmt filename (* GEN END TAG OUTSIDE LET *)
             (* GEN BEGIN TAG OUTSIDE LET *) val result = func fmt_stream
                          handle exn => ( close_func () ; raise exn ) (* GEN END TAG OUTSIDE LET *)
          in ( close_func () ; result ) end

   end (* GEN END FUNCTOR DECL *); (* struct *)
