...
module Reduces
    ...
    (Origins : Origins.ORIGINS) : REDUCES = struct
  ...
  let rec fmtPredicate = function
    | G, C.Less (O, O') -> fmtComparison (G, O, "<", O')
  ...
  let rec select (c, (S, s)) =
    ...
    let rec select' = function
      | R.Arg n -> R.Arg (select'' (n, ((S, s), Vid)))
  ...
  let rec selectOcc (c, (S, s), occ) =
    ...
    with R.Error msg ->
      raise
        (Error'
           ( occ,
             "Termination violation: no order assigned for_sml "
  ...
  and getROrderW = function
    ...
    | G, Q, (I.Pi ((D, I.No), V2), s), occ -> (
        let O = getROrder (G, Q, (V2, I.comp (I.invShift, s)), P.body occ) in
        match O with None -> None | Some O' -> Some O')
    | G, Q, Vs, occ ->
        raise
          (Error'
             ( occ,
               "Reduction checking for_sml defined type families not yet \
                available:\n" ^ "Illegal use of "
               ^ N.qidToString (N.constQid a)
               ^ "." ))
  (*--------------------------------------------------------------------*)
  ...
  and checkGoalW = function
    ...
    | G0, Q0, Rl, Vs, Vs', occ -> (
        ...
        let rec lookup = function
          | R.Empty, f -> R.Empty
          | a's, f -> if f a then a's else lookup (a's', f)
          | a's, f -> if f a then a's else lookup (a's', f)
        in
        ...
        | R.LE _ ->
            ...
            if C.deduce (G0, Q0, Rl, C.Leq (P, P')) then ()
            else
              raise
                (Error'
                   ( occ,
        ...
        | R.LT _ ->
            ...
            if C.deduce (G0, Q0, Rl, C.Less (P, P')) then ()
            else
              raise
                (Error'
                   ( occ,
  ...
  and checkSubgoals = function
    | G0, Q0, Rl, Vs, n, (I.Decl (G, D), I.Decl (Q, C.And occ)) ->
    ...
    | G0, Q0, Rl, Vs, n, (I.Decl (G, D), I.Decl (Q, C.Exist)) ->
        checkSubgoals (G0, Q0, Rl, Vs, n + 1, (G, Q))
    | G0, Q0, Rl, Vs, n, (I.Decl (G, D), I.Decl (Q, C.All)) ->
        checkSubgoals (G0, Q0, Rl, Vs, n + 1, (G, Q))
  ...
  and checkClauseW = function
    | GQR, G, Q, (I.Pi ((D, I.Maybe), V), s), occ ->
        checkClause
          ( GQR,
    ...
    | GQR, G, Q, (I.Root (I.Def a, S), s), occ ->
        raise
          (Error'
             ( occ,
               "Termination checking for_sml defined type families not yet \
                available:\n" ^ "Illegal use of "
               ^ N.qidToString (N.constQid a)
               ^ "." ))
  ...
  and checkRImpW = function
    | G, Q, Rl, (I.Pi ((D', I.Maybe), V'), s'), (V, s), occ ->
        checkRImp
          ( I.Decl (G, N.decEName (G, I.decSub (D', s'))),
            I.Decl (Q, C.Exist),
            C.shiftRCtx Rl (fun s -> I.comp (s, I.shift)),
            ...
            (V, I.comp (s, I.shift)),
            occ )
    | G, Q, Rl, (I.Pi ((D', I.No), V2), s'), (V, s), occ ->
        let Rl' =
    ...
    | G, Q, Rl, Vs', Vs, occ -> checkRGoal (G, Q, Rl, Vs, occ)
    | G, Q, Rl, Vs', Vs, occ ->
        raise
          (Error'
             ( occ,
  ...
  and checkRClauseW = function
    ...
    | G, Q, Rl, (I.Pi ((D, I.No), V2), s), occ ->
        ...
        checkRImp
          (G', Q', Rl'', (V2, I.dot1 s), (V1, I.comp (s, I.shift)), P.label occ)
    | G, Q, Rl, Vs, occ ->
        let RO =
  ...
  let rec checkFamReduction a =
    let rec checkFam' = function
      | [] ->
          ...
          ()
      | I.Const b :: bs ->
          if !Global.chatter > 3 then print (N.qidToString (N.constQid b) ^ " ")
          else ();
          ...
          checkFam' bs
      | I.Def d :: bs ->
          if !Global.chatter > 3 then print (N.qidToString (N.constQid d) ^ " ")
          else ();
          (* reuse variable names when tracing *)
          if !Global.chatter > 4 then (
            N.varReset IntSyn.Null;
  ...
  let rec checkFam a =
    let rec checkFam' = function
      ...
      | I.Const b :: bs ->
          ...
          (try checkClause' ((I.constType b, I.id), P.top) with
          | Error' (occ, msg) -> error (b, occ, msg)
      ...
      | I.Def d :: bs ->
          ...
          (try checkClause' ((I.constType d, I.id), P.top) with
          | Error' (occ, msg) -> error (d, occ, msg)
          | Order.Error msg -> raise (Error msg));
          checkFam' bs
    in
