...
module Print
    ...
    (Symbol : Symbol.SYMBOL) : PRINT = struct
  ...
  let rec lookuplvar l =
    ...
    let _ =
      if List.exists (fun r -> r = l) !lvars then ()
      else lvars := !lvars @ [ l ]
    in
    ...
    let rec sTS = function
      | I.Shift k, S ->
          if k < depth then sTS (I.Dot (I.Idx (k + 1), I.Shift (k + 1)), S)
            ...
            S
      | I.Dot (I.Idx k, s), S -> sTS (s, I.App (I.Root (I.BVar k, I.Nil), S))
      | I.Dot (I.Exp U, s), S -> sTS (s, I.App (U, S))
    in
    sTS (s, I.Nil)
  ...
  *)
  ...
  let rec dropImp = function
    ...
    | i, I.SClo (S, s), n -> sclo' (dropImp (i, S, n), s)
    | i, I.Nil, n -> TooFew
  ...
  *)
  ...
  let rec constQid cid =
    if !noShadow then Names.conDecQid (I.sgnLookup cid) else Names.constQid cid
  ...
  *)
  ...
  let rec fmtCon = function
    | G, I.BVar n -> Str0 (Symbol.bvar (Names.bvarName (G, n)))
    ...
    | G, H -> Str0 (Symbol.const (projName (G, H)))
    | G, H ->
        let n = lookuplvar r in
        (* LVar fixed Sun Dec  1 11:36:55 2002 -cs *)
        fmtConstPath (fun l0 ->
            (Symbol.const ("#[" ^ l0 ^ n ^ "]" ^ projName (G, H)), constQid cid))
    | G, I.FgnConst (cs, conDec) -> (
        ...
        match (Names.constLookup (Names.Qid ([], name)), !noShadow) with
        | Some _, false ->
  ...
  *)
  ...
  let rec fmtExpW = function
    ...
    | G, d, ctx, (I.Pi ((D, P), V2), s) ->
        let D' = Names.decLUName (G, D) in
        fmtLevel
          (I.Decl (G, D'), d, ctx, (braces (G, d, ((D', V2), s)), I.dot1 s))
    | G, d, ctx, (I.Pi ((D, P), V2), s) ->
    ...
    | G, d, ctx, (U, s) -> fmtOpArgs (G, d, ctx, opargs (G, d, R), s)
    | G, d, ctx, (I.Lam (D, U), s) ->
        let D' = Names.decLUName (G, D) in
        fmtLevel
          ( I.Decl (G, D') (* I.decSub (D', s) *),
  ...
  and opargs (G, d, R) =
    if !implicit then
      if !printInfix then opargsImplicitInfix (G, d, R)
      else opargsImplicit (G, d, R)
    else opargsExplicit (G, d, R)

  and fmtOpArgs = function
    | G, d, ctx, oa, s ->
        if isNil S' then aa (ctx, List.hd opFmts) (* opFmts = [fmtCon(G,C)] *)
  ...
  and fmtSub'' = function
    | G, d, l, I.Shift k -> [ Str ("^" ^ Int.toString k); Str "]" ]
    | G, d, l, I.Dot (I.Idx k, s) ->
        Str (Names.bvarName (G, k))
        :: Str "." :: F.Break
        :: fmtSub' (G, d, l + 1, s)
    | G, d, l, I.Dot (I.Exp U, s) ->
        fmtExp (G, d + 1, noCtxt, (U, I.id))
        :: Str "." :: F.Break
        :: fmtSub' (G, d, l + 1, s)

  and fmtExp (G, d, ctx, (U, s)) =
    if exceeded (d, !printDepth) then sym "%%"
    else fmtExpW (G, d, ctx, Whnf.whnf (U, s))

  and fmtSpine = function
    | G, d, l, (I.Nil, _) -> []
    | G, d, l, (I.SClo (S, s'), s) -> fmtSpine (G, d, l, (S, I.comp (s', s)))
    | G, d, l, (I.App (U, S), s) ->
        if elide l then [] (* necessary? *)
        else if addots l then [ ldots ]
        else fmtExp (G, d + 1, appCtxt, (U, s)) :: fmtSpine' (G, d, l, (S, s))
  ...
  and fmtLevel = function
    | G, d, Ctxt (fixity', accum, l), (OpArgs (fixity, fmts, S), s) ->
    ...
    | G, d, Ctxt (fixity', accum, l), (OpArgs (fixity, fmts, S), s) ->
        ...
        let rhs =
          if accMore && elide l then []
          else if accMore && addots l then fmts @ [ ldots ]
          else
            fmts
            @ [
                fmtExp
                  (G, d + 1, Ctxt (FX.Infix (p, FX.None), [], 0), snd (S, s));
              ]
        in
        if accMore then
          fmtExp (G, d, Ctxt (fixity, rhs @ accum, l + 1), fst (S, s))
        else
    ...
    | G, d, Ctxt (fixity', accum, l), (OpArgs (fixity, fmts, S), s) ->
        ...
        let lhs =
          if accMore && elide l then []
          else if accMore && addots l then [ ldots ] @ fmts
          else
            [
              fmtExp (G, d + 1, Ctxt (FX.Infix (p, FX.None), [], 0), fst (S, s));
            ]
            @ fmts
        in
        if accMore then
          fmtExp (G, d, Ctxt (fixity, accum @ lhs, l + 1), snd (S, s))
        else
          let both = fmtExp (G, d, Ctxt (fixity, lhs, 0), snd (S, s)) in
          addAccum (parens ((fixity', fixity), both), fixity', accum)
    | G, d, Ctxt (fixity', accum, l), (OpArgs (fixity, fmts, S), s) ->
        let lhs = fmtExp (G, d + 1, Ctxt (fixity, [], 0), fst (S, s)) in
        let rhs = fmtExp (G, d + 1, Ctxt (fixity, [], 0), snd (S, s)) in
        addAccum
    ...
    | G, d, Ctxt (fixity', accum, l), (OpArgs (fixity, fmts, S), s) ->
        ...
        let pfx =
          if accMore && elide l then []
          else if accMore && addots l then [ ldots; F.Break ]
          else fmts
        in
        if accMore then
          fmtExp (G, d, Ctxt (fixity, accum @ pfx, l + 1), fst (S, s))
        else
    ...
    | G, d, Ctxt (fixity', accum, l), (OpArgs (fixity, fmts, S), s) ->
        ...
        let pfx =
          if accMore && elide l then []
          else if accMore && addots l then [ F.Break; ldots ]
          else fmts
        in
        if accMore then
          fmtExp (G, d, Ctxt (fixity, pfx @ accum, l + 1), fst (S, s))
        else
          let whole = fmtExp (G, d, Ctxt (fixity, pfx, 0), fst (S, s)) in
          addAccum (parens ((fixity', fixity), whole), fixity', accum)
  ...
  and fmtDec = function
    ...
    | G, d, (I.BDec (x, (cid, t)), s) ->
        let Gsome, Gblock = I.constBlock cid in
        F.HVbox
          ([ Str0 (Symbol.const (nameOf x)); sym ":" ]
          @ fmtDecList' (G, (Gblock, I.comp (t, s))))
    | G, d, (I.ADec (x, _), s) ->
        F.HVbox [ Str0 (Symbol.bvar (nameOf x)); sym ":_" ]
    | G, d, (I.NDec (Some name), s) -> F.HVbox [ sym name ]

  and fmtDecList' = function
    | G0, ([], s) -> []
    | G0, (D :: [], s) -> [ sym "{"; fmtDec (G0, 0, (D, s)); sym "}" ]
    | G0, (D :: L, s) ->
        sym "{"
        :: fmtDec (G0, 0, (D, s))
        :: sym "}" :: F.Break
  ...
  let rec fmtDecList = function
    | G0, [] -> []
    | G0, D :: [] -> [ sym "{"; fmtDec (G0, 0, (D, I.id)); sym "}" ]
    | G0, D :: L ->
        sym "{"
        :: fmtDec (G0, 0, (D, I.id))
        :: sym "}" :: F.Break
  ...
  let rec fmtConDec = function
    ...
    | hide, condec ->
        ...
        F.HVbox
          ...
          @ (formatWorlds (T.Worlds W) :: [ sym "." ]))
    | hide, condec ->
        (* val _ = Names.varReset () *)
        let qid = Names.conDecQid condec in
        let _ = Names.varReset IntSyn.Null in
        let G, V, U =
          if hide then skipI2 (imp, I.Null, V, U) else (I.Null, V, U)
        in
        let Vfmt = fmtExp (G, 0, noCtxt, (V, I.id)) in
...
*)
  ...
  let rec fmtCnstr = function
    ...
    | I.FgnCnstr csfc ->
        let rec fmtExpL = function
          ...
          | (G, U) :: [] -> [ fmtExp (Names.ctxLUName G, 0, noCtxt, (U, I.id)) ]
          | (G, U) :: expL ->
              [
  ...
  *)
  ...
  let rec evarCnstrsToStringOpt Xnames =
    ...
    match cnstrL with [] -> None | _ -> Some (cnstrsToString cnstrL)
