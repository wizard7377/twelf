...
local
  ...
  *)
  fun (* GEN BEGIN FUN FIRST *) fmtCon (G, I.BVar(n)) =
      let
        (* GEN BEGIN TAG OUTSIDE LET *) val I.Dec (SOME n, _) = I.ctxDec (G, n) (* GEN END TAG OUTSIDE LET *)
      in
        sexp [Str ("<Var name = \"" ^ n ^ "\"/>")]
      end (* GEN END FUN FIRST *)
    | (* GEN BEGIN FUN BRANCH *) fmtCon (G, I.Const(cid)) = sexp [Str "<Const name=\"", Str (I.conDecName (I.sgnLookup cid)), Str "\"/>"] (* GEN END FUN BRANCH *)
    | (* GEN BEGIN FUN BRANCH *) fmtCon (G, I.Def(cid)) = sexp [Str "<Def>", F.Break, Integer cid, Str "</Def>"] (* GEN END FUN BRANCH *)
    | (* GEN BEGIN FUN BRANCH *) fmtCon (G, I.FgnConst (csid, condec)) = sexp [Str "FngConst"] (* GEN END FUN BRANCH *)  (* FIX -cs Fri Jan 28 17:45:35 2005*)
    (* I.Skonst, I.FVar cases should be impossible *)

  (* fmtUni (L) = "L" *)
  fun (* GEN BEGIN FUN FIRST *) fmtUni (I.Type) = Str "<Type/>" (* GEN END FUN FIRST *)
    | (* GEN BEGIN FUN BRANCH *) fmtUni (I.Kind) = Str "<Kind/>" (* GEN END FUN BRANCH *)
  ...
  (* fmtExpW (G, (U, s)) = fmt
     ...
     Invariants:
       ...
       (U,s) in whnf
  *)
  fun (* GEN BEGIN FUN FIRST *) fmtExpW (G, (I.Uni(L), s)) = sexp [Str "<Uni>", F.Break, fmtUni L, Str "</Uni>"] (* GEN END FUN FIRST *)
      ...
      (case (fmtSpine (G, (S, s)))
         of NONE =>  fmtCon (G, H)
          | SOME fmts =>  F.HVbox [Str "<App>", fmtCon (G, H),
               F.Break, sexp (fmts), Str "</App>"]) (* GEN END FUN BRANCH *)
    ...
    | (* GEN BEGIN FUN BRANCH *) fmtExpW (G, (I.FgnExp (csid, F), s)) = sexp [Str "FgnExp"] (* GEN END FUN BRANCH *) (* FIX -cs Fri Jan 28 17:45:43 2005 *)
  ...
  *)
  and (* GEN BEGIN FUN FIRST *) fmtSpine (G, (I.Nil, _)) = NONE (* GEN END FUN FIRST *)
    ...
    | (* GEN BEGIN FUN BRANCH *) fmtSpine (G, (I.App(U, S), s)) =
      (case (fmtSpine (G, (S, s)))
         of NONE => SOME [fmtExp (G, (U, s))]
          | SOME fmts => SOME ([fmtExp (G, (U, s)), F.Break] @ fmts)) (* GEN END FUN BRANCH *)

  and (* GEN BEGIN FUN FIRST *) fmtDec (G, (I.Dec (NONE, V), s)) =
        sexp [Str "<Dec>", F.Break, fmtExp (G, (V, s)), Str "</Dec>"] (* GEN END FUN FIRST *)
    | (* GEN BEGIN FUN BRANCH *) fmtDec (G, (I.Dec (SOME(x), V), s)) =
        sexp [Str "<Dec name =", Name x,  Str ">", F.Break, fmtExp (G, (V, s)), Str "</Dec>"] (* GEN END FUN BRANCH *)


  and (* GEN BEGIN FUN FIRST *) fmtDec' (G, (I.Dec (NONE, V), s)) =
        sexp [fmtExp (G, (V, s))] (* GEN END FUN FIRST *)
    | (* GEN BEGIN FUN BRANCH *) fmtDec' (G, (I.Dec (SOME(x), V), s)) =
        sexp [fmtExp (G, (V, s))] (* GEN END FUN BRANCH *)
  ...
  *)
  fun (* GEN BEGIN FUN FIRST *) fmtConDec (I.ConDec (name, parent, imp, _, V, L)) =
    ...
    | (* GEN BEGIN FUN BRANCH *) fmtConDec (I.BlockDec (name, _, _, _)) =
      Str ("<! Skipping Skolem constant " ^ name ^ ">") (* GEN END FUN BRANCH *)
...
(*  fun formatSpine (G, S) = sexp (fmtSpine (G, (S, I.id))) *)
  ...
  fun printSgnToFile path filename =
      let
        (* GEN BEGIN TAG OUTSIDE LET *) val file = TextIO.openOut (path ^ filename) (* GEN END TAG OUTSIDE LET *)
        (* GEN BEGIN TAG OUTSIDE LET *) val _ = TextIO.output (file, "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<!-- nsgmls ex.xml -->\n<!DOCTYPE Signature SYSTEM \"lf.dtd\">\n<Signature>") (* GEN END TAG OUTSIDE LET *)
        ...
        (* GEN BEGIN TAG OUTSIDE LET *) val _ = TextIO.output (file, "</Signature>") (* GEN END TAG OUTSIDE LET *)
