...
module Normalize (Whnf : Whnf.WHNF) : Normalize.NORMALIZE = struct
  ...
  let rec whnfFor = function
    ...
    | Ft -> Ft
    | Ft -> Ft
    | T.FClo (F, t1), t2 -> whnfFor (F, T.comp (t1, t2))
    | Ft -> Ft
    | Ft -> Ft
  ...
  let rec normalizePrg = function
    ...
    | T.Unit, _ -> T.Unit
    | T.EVar (_, { contents = Some P }, _), t -> T.PClo (P, t)
    | P, t -> T.PClo (P, t)
    | T.PClo (P, t), t' -> normalizePrg (P, T.comp (t, t'))

  and normalizeSpine = function
    | T.Nil, t -> T.Nil
    | T.AppExp (U, S), t ->
        T.AppExp (Whnf.normalize (U, T.coerceSub t), normalizeSpine (S, t))
    | T.AppPrg (P, S), t -> T.AppPrg (normalizePrg (P, t), normalizeSpine (S, t))
    | T.AppBlock (B, S), t ->
        T.AppBlock (I.blockSub (B, T.coerceSub t), normalizeSpine (S, t))
    | T.SClo (S, t1), t2 -> normalizeSpine (S, T.comp (t1, t2))
  (*
...
*)
  ...
  let rec normalizeSub = function
    ...
    | T.Dot (T.Block k, s) -> T.Dot (T.Block k, normalizeSub s)
    | T.Dot (T.Idx k, s) -> T.Dot (T.Idx k, normalizeSub s)
