...
module IntSyn (Global : GLOBAL) : INTSYN = struct type cid = int
(* Constant identifier        *)
...
let rec ctxLookup = function (Decl (G', D), 1) -> D | (Decl (G', _), k') -> ctxLookup (G', k' - 1)
...
let rec ctxLength G  = ( let rec ctxLength' = function (Null, n) -> n | (Decl (G, _), n) -> ctxLength' (G, n + 1) in  ctxLength' (G, 0) )
type fgnExp = exn
...
module FgnExpStd = struct module ToInternal = FgnOpnTable
module Map = FgnOpnTable
module App = FgnOpnTable
module EqualTo = FgnOpnTable
module UnifyWith = FgnOpnTable
let rec fold csfe f b  = ( let r = ref b in let rec g U  = r := f (U, ! r) in  App.apply csfe g; ! r )
 end
module FgnCnstrStd = struct module ToInternal = FgnOpnTable
module Awake = FgnOpnTable
module Simplify = FgnOpnTable
 end
let rec conDecName = function (ConDec (name, _, _, _, _, _)) -> name | (ConDef (name, _, _, _, _, _, _)) -> name | (AbbrevDef (name, _, _, _, _, _)) -> name | (SkoDec (name, _, _, _, _)) -> name | (BlockDec (name, _, _, _)) -> name | (BlockDef (name, _, _)) -> name
let rec conDecParent = function (ConDec (_, parent, _, _, _, _)) -> parent | (ConDef (_, parent, _, _, _, _, _)) -> parent | (AbbrevDef (_, parent, _, _, _, _)) -> parent | (SkoDec (_, parent, _, _, _)) -> parent | (BlockDec (_, parent, _, _)) -> parent | (BlockDef (_, parent, _)) -> parent
(* conDecImp (CD) = k
...
let rec conDecImp = function (ConDec (_, _, i, _, _, _)) -> i | (ConDef (_, _, i, _, _, _, _)) -> i | (AbbrevDef (_, _, i, _, _, _)) -> i | (SkoDec (_, _, i, _, _)) -> i | (BlockDec (_, _, _, _)) -> 0
(* watch out -- carsten *)

let rec conDecStatus = function (ConDec (_, _, _, status, _, _)) -> status | _ -> Normal
(* conDecType (CD) =  V
...
let rec conDecType = function (ConDec (_, _, _, _, V, _)) -> V | (ConDef (_, _, _, _, V, _, _)) -> V | (AbbrevDef (_, _, _, _, V, _)) -> V | (SkoDec (_, _, _, V, _)) -> V
...
let rec conDecUni = function (ConDec (_, _, _, _, _, L)) -> L | (ConDef (_, _, _, _, _, L, _)) -> L | (AbbrevDef (_, _, _, _, _, L)) -> L | (SkoDec (_, _, _, _, L)) -> L
...
let sgnStructArray = (Array.array (maxMid + 1, StrDec ("", None)) : strDec Array.array)
let nextMid = ref (0)
(* Invariants *)
...
let rec sgnClean (i)  = if i >= ! nextCid then () else (Array.update (sgnArray, i, dummyEntry); sgnClean (i + 1))
let rec sgnReset ()  = ((* Fri Dec 20 12:04:24 2002 -fp *)
(* this circumvents a space leak *)
sgnClean (0); nextCid := 0; nextMid := 0)
let rec sgnSize ()  = (! nextCid, ! nextMid)
let rec sgnAdd (conDec)  = ( let cid = ! nextCid in  if cid > maxCid then raise (Error ("Global signature size " ^ Int.toString (maxCid + 1) ^ " exceeded")) else (Array.update (sgnArray, cid, conDec); nextCid := cid + 1; cid) )
(* 0 <= cid < !nextCid *)

let rec sgnLookup (cid)  = Array.sub (sgnArray, cid)
let rec sgnApp (f)  = ( let rec sgnApp' (cid)  = if cid = ! nextCid then () else (f cid; sgnApp' (cid + 1)) in  sgnApp' (0) )
let rec sgnStructAdd (strDec)  = ( let mid = ! nextMid in  if mid > maxMid then raise (Error ("Global signature size " ^ Int.toString (maxMid + 1) ^ " exceeded")) else (Array.update (sgnStructArray, mid, strDec); nextMid := mid + 1; mid) )
(* 0 <= mid < !nextMid *)

let rec sgnStructLookup (mid)  = Array.sub (sgnStructArray, mid)
(* A hack used in Flit - jcreed 6/05 *)

let rec rename (cid, new_)  = ( let newConDec = match sgnLookup cid with ConDec (n, m, i, s, e, u) -> ConDec (new_, m, i, s, e, u) | ConDef (n, m, i, e, e', u, a) -> ConDef (new_, m, i, e, e', u, a) | AbbrevDef (n, m, i, e, e', u) -> AbbrevDef (new_, m, i, e, e', u) | BlockDec (n, m, d, d') -> BlockDec (new_, m, d, d') | SkoDec (n, m, i, e, u) -> SkoDec (new_, m, i, e, u) in  Array.update (sgnArray, cid, newConDec) )
let rec constDef (d)  = (match sgnLookup (d) with ConDef (_, _, _, U, _, _, _) -> U | AbbrevDef (_, _, _, U, _, _) -> U)
...
let rec constStatus (c)  = (match sgnLookup (c) with ConDec (_, _, _, status, _, _) -> status | _ -> Normal)
...
let rec comp = function (Shift (0), s) -> s | (s, Shift (0)) -> s | (Shift (n), Dot (Ft, s)) -> comp (Shift (n - 1), s) | (Shift (n), Shift (m)) -> Shift (n + m) | (Dot (Ft, s), s') -> Dot (frontSub (Ft, s'), comp (s, s'))
and bvarSub = function (1, Dot (Ft, s)) -> Ft | (n, Dot (Ft, s)) -> bvarSub (n - 1, s) | (n, Shift (k)) -> Idx (n + k)
and blockSub = function (Bidx k, s) -> (match bvarSub (k, s) with Idx k' -> Bidx k' | Block B -> B) | (LVar ({ contents = (Some B) }, sk, _), s) -> blockSub (B, comp (sk, s)) | (LVar (r, sk, (l, t)), s) -> LVar (r, comp (sk, s), (l, t)) | (L, s') -> Inst (map (fun U -> EClo (U, s')) ULs)
and frontSub = function (Idx (n), s) -> bvarSub (n, s) | (Exp (U), s) -> Exp (EClo (U, s)) | (Undef, s) -> Undef | (Block (B), s) -> Block (blockSub (B, s))
(* decSub (x:V, s) = D'
...
let rec decSub = function (Dec (x, V), s) -> Dec (x, EClo (V, s)) | (NDec x, s) -> NDec x | (BDec (n, (l, t)), s) -> BDec (n, (l, comp (t, s)))
(* dot1 (s) = s'
...
let rec ctxDec' = function (Decl (G', Dec (x, V')), 1) -> Dec (x, EClo (V', Shift (k))) | (Decl (G', BDec (n, (l, s))), 1) -> BDec (n, (l, comp (s, Shift (k)))) | (Decl (G', _), k') -> ctxDec' (G', k' - 1) in  ctxDec' (G, k) )
(* blockDec (G, v, i) = V
...
let rec blockDec (G, v, i)  = ( (* G |- s : Gsome *)
let BDec (_, (l, s)) = ctxDec (G, k) in let (Gsome, Lblock) = conDecBlock (sgnLookup l) in let rec blockDec' = function (t, D :: L, 1, j) -> decSub (D, t) | (t, _ :: L, n, j) -> blockDec' (Dot (Exp (Root (Proj (v, j), Nil)), t), L, n - 1, j + 1) in  blockDec' (s, Lblock, i, 1) )
...
let rec headOpt = function (Root (H, _)) -> Some (H) | (Lam (_, U)) -> headOpt U | _ -> None
let rec ancestor' = function (None) -> Anc (None, 0, None) | (Some (Const (c))) -> Anc (Some (c), 1, Some (c)) | (Some (Def (d))) -> (match sgnLookup (d) with ConDef (_, _, _, _, _, _, Anc (_, height, cOpt)) -> Anc (Some (d), height + 1, cOpt)) | (Some _) -> Anc (None, 0, None)
(* ancestor(U) = ancestor info for_sml d = U *)

let rec ancestor (U)  = ancestor' (headOpt U)
(* defAncestor(d) = ancestor of d, d must be defined *)

let rec defAncestor (d)  = (match sgnLookup (d) with ConDef (_, _, _, _, _, _, anc) -> anc)
...
let rec targetHeadOpt = function (Root (H, _)) -> Some (H) | (Pi (_, V)) -> targetHeadOpt V | (Redex (V, S)) -> targetHeadOpt V | (Lam (_, V)) -> targetHeadOpt V | (EVar ({ contents = (Some (V)) }, _, _, _)) -> targetHeadOpt V | (EClo (V, s)) -> targetHeadOpt V | _ -> None
...
let rec targetFamOpt = function (Root (Const (cid), _)) -> Some (cid) | (Pi (_, V)) -> targetFamOpt V | (Root (Def (cid), _)) -> targetFamOpt (constDef cid) | (Redex (V, S)) -> targetFamOpt V | (Lam (_, V)) -> targetFamOpt V | (EVar ({ contents = (Some (V)) }, _, _, _)) -> targetFamOpt V | (EClo (V, s)) -> targetFamOpt V | _ -> None
(* Root(Bvar _, _), Root(FVar _, _), Root(FgnConst _, _),
         EVar(ref NONE,..), Uni, FgnExp _
      *)
