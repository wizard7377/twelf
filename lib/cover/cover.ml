...
functor (* GEN BEGIN FUNCTOR DECL *) Cover
...
struct
  ...
  local
    ...
    *)
    fun createEVar (G, V) =
        let (* G |- V : L *)
          ...
          (* GEN BEGIN TAG OUTSIDE LET *) val X' = Whnf.newLoweredEVar (G', (V, iw)) (* GEN END TAG OUTSIDE LET *) (* G' |- X' : V[iw] *)
    ...
    *)
    fun (* GEN BEGIN FUN FIRST *) inCoverInst (M.Mnil) = Cnil (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) inCoverInst (M.Mapp (M.Marg (M.Plus, x), ms')) =
          Match (inCoverInst ms') (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) inCoverInst (M.Mapp (M.Marg (M.Minus, x), ms')) =
          Skip (inCoverInst ms') (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) inCoverInst (M.Mapp (M.Marg (M.Star, x), ms')) =
          Skip (inCoverInst ms') (* GEN END FUN BRANCH *)

    (* outCoverInst (ms) = ci
       converts mode spine ms to cover instructions ci for output coverage
    *)
    fun (* GEN BEGIN FUN FIRST *) outCoverInst (M.Mnil) = Cnil (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) outCoverInst (M.Mapp (M.Marg (M.Plus, x), ms')) =
          Skip (outCoverInst ms') (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) outCoverInst (M.Mapp (M.Marg (M.Minus, x), ms')) =
          Match (outCoverInst ms') (* GEN END FUN BRANCH *)
    ...
    fun (* GEN BEGIN FUN FIRST *) labToString (Top) = "^" (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) labToString (Child(lab, n)) = labToString(lab) ^ "." ^ Int.toString(n) (* GEN END FUN BRANCH *)
    ...
    fun (* GEN BEGIN FUN FIRST *) pluralize (1, s) = s (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) pluralize (n, s) = s ^ "s" (* GEN END FUN BRANCH *)
    ...
    fun (* GEN BEGIN FUN FIRST *) abbrevCGoal (G, V, 0, ci) = (G, abbrevCGoal' (G, V, ci)) (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) abbrevCGoal (G, I.Pi((D, P), V), p, ci) = (* p > 0 *)
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val D' = N.decEName (G, D) (* GEN END TAG OUTSIDE LET *)
        in
          abbrevCGoal (I.Decl (G, D'), V, p-1, ci)
        end (* GEN END FUN BRANCH *)
    and (* GEN BEGIN FUN FIRST *) abbrevCGoal' (G, I.Pi((D, P), V), ci) =
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val D' = N.decUName (G, D) (* GEN END TAG OUTSIDE LET *)
        in
          I.Pi ((D', P), abbrevCGoal' (I.Decl (G, D'), V, ci))
        end (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) abbrevCGoal' (G, I.Root (a, S), ci) =
          I.Root (a, abbrevCSpine (S, ci)) (* GEN END FUN BRANCH *)
      (* other cases are impossible by CGoal invariant *)

    fun formatCGoal (V, p, ci) =
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val _ = N.varReset I.Null (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val (G, V') = abbrevCGoal (I.Null, V, p, ci) (* GEN END TAG OUTSIDE LET *)
        in
          F.HVbox [Print.formatCtx (I.Null, G), F.Break, F.String "|-",
                   F.Space, Print.formatExp (G, V')]
        end

    fun (* GEN BEGIN FUN FIRST *) formatCGoals ((V,p)::nil, ci) = [formatCGoal (V, p, ci)] (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) formatCGoals ((V,p)::Vs, ci) =
          formatCGoal (V, p, ci) :: F.String "," :: F.Break :: formatCGoals (Vs, ci) (* GEN END FUN BRANCH *)
    ...
    fun showSplitVar (V, p, k, ci) =
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val _ = N.varReset I.Null (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val (G, V') = abbrevCGoal (I.Null, V, p, ci) (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val I.Dec (SOME(x), _) = I.ctxLookup (G, k) (* GEN END TAG OUTSIDE LET *)
        in
    ...
    *)
    ...
    fun (* GEN BEGIN FUN FIRST *) buildSpine 0 = I.Nil (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) buildSpine n = (* n > 0 *)
        (* Eta-long invariant violation -kw *)
        I.App (I.Root (I.BVar(n), I.Nil), buildSpine (n-1)) (* GEN END FUN BRANCH *)
    ...
    (* initCGoal' (a, 0, ., V) = ({x1:V1}...{xn:Vn} a x1...xn, n)
       ...
       G |- V : type
    *)
    fun (* GEN BEGIN FUN FIRST *) initCGoal' (a, k, G, I.Pi ((D, P), V)) =
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val D' = N.decEName (G, D) (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val (V', p) = initCGoal' (a, k+1, I.Decl(G, D'), V) (* GEN END TAG OUTSIDE LET *)
        in
          (I.Pi ((D', I.Maybe), V'), p)
        end (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) initCGoal' (a, k, G, I.Uni (I.Type)) =
          (I.Root (a, buildSpine k), k) (* GEN END FUN BRANCH *)
    ...
    *)
    ...
    fun equationToString (Eqn (G, Us1, Us2)) =
        let (* GEN BEGIN TAG OUTSIDE LET *) val G' = Names.ctxLUName G (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val fmt =
              F.HVbox [Print.formatCtx (I.Null, G'), F.Break,
                       F.String "|-", F.Space,
                       Print.formatExp (G', I.EClo (Us1)), F.Break,
                       F.String "=", F.Space,
                       Print.formatExp (G', I.EClo (Us2))] (* GEN END TAG OUTSIDE LET *)
        in
          F.makestring_fmt (fmt)
        end

    fun (* GEN BEGIN FUN FIRST *) eqnsToString (nil) = ".\n" (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) eqnsToString (eqn::eqns) =
          equationToString (eqn) ^ ",\n"
          ^ eqnsToString (eqns) (* GEN END FUN BRANCH *)
    ...
    *)
    ...
    fun (* GEN BEGIN FUN FIRST *) candsToString (Fail) = "Fail" (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) candsToString (Cands(ks)) = "Cands [" ^ List.foldl ((* GEN BEGIN FUNCTION EXPRESSION *) fn (k,str) => Int.toString k ^ "," ^ str (* GEN END FUNCTION EXPRESSION *)) "]" ks (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) candsToString (Eqns(eqns)) = "Eqns [\n" ^ eqnsToString eqns ^ "]" (* GEN END FUN BRANCH *)

    (* fail () = Fail
       indicate failure without splitting candidates
     *)
    fun fail (msg) =
        (chatter 7 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => msg ^ "\n" (* GEN END FUNCTION EXPRESSION *));
         Fail)

    (* failAdd (k, cands) = cands'
       indicate failure, but add k as splitting candidate
    *)
    fun (* GEN BEGIN FUN FIRST *) failAdd (k, Eqns _) = Cands (k::nil) (* GEN END FUN FIRST *) (* no longer matches *)
      | (* GEN BEGIN FUN BRANCH *) failAdd (k, Cands (ks)) = Cands (k::ks) (* GEN END FUN BRANCH *) (* remove duplicates? *)
      | (* GEN BEGIN FUN BRANCH *) failAdd (k, Fail) = Fail (* GEN END FUN BRANCH *)

    (* addEqn (e, cands) = cands'
       indicate possible match if equation e can be solved
    *)
    fun (* GEN BEGIN FUN FIRST *) addEqn (e, Eqns (es)) = Eqns (e::es) (* GEN END FUN FIRST *) (* still may match: add equation *)
      | (* GEN BEGIN FUN BRANCH *) addEqn (e, cands as Cands (ks)) = (* already failed: ignore new constraints *)
          cands (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) addEqn (e, Fail) = Fail (* GEN END FUN BRANCH *)         (* already failed without candidates *)
    ...
    *)
    fun (* GEN BEGIN FUN FIRST *) matchEqns (nil) = true (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) matchEqns (Eqn (G, Us1, Us2 as (U2, s2))::es) =
        ...
        andalso matchEqns (es) (* GEN END FUN BRANCH *)
    ...
    *)
    fun (* GEN BEGIN FUN FIRST *) resolveCands (Eqns (es)) =
      ...
      | (* GEN BEGIN FUN BRANCH *) resolveCands (Cands (ks)) = Cands (ks) (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) resolveCands (Fail) = Fail (* GEN END FUN BRANCH *)
    ...
    *)
    fun (* GEN BEGIN FUN FIRST *) collectConstraints (nil) = nil (* GEN END FUN FIRST *)
      ...
      | (* GEN BEGIN FUN BRANCH *) collectConstraints (I.EVar (_, _, _, ref constrs)::Xs) =
          ...
          constrs @ collectConstraints Xs (* GEN END FUN BRANCH *)
    ...
    *)
    ...
    fun (* GEN BEGIN FUN FIRST *) checkConstraints (G, Qs, Cands (ks)) = Cands (ks) (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) checkConstraints (G, Qs, Fail) = Fail (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) checkConstraints (G, Qs, Eqns _) = (* _ = nil *)
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val Xs = Abstract.collectEVars (G, Qs, nil) (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val constrs = collectConstraints Xs (* GEN END TAG OUTSIDE LET *)
        in
          case constrs
            of nil => Eqns (nil)
    ...
    *)
    fun (* GEN BEGIN FUN FIRST *) addKs (ccs as Cands(ks), CandList (klist)) = CandList (ccs::klist) (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) addKs (ces as Eqns(nil), CandList (klist)) = Covered (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) addKs (cfl as Fail, CandList (klist)) = CandList (cfl::klist) (* GEN END FUN BRANCH *)
    ...
    *)
    ...
    and (* GEN BEGIN FUN FIRST *) matchExpW (G, d, Us1 as (I.Root (H1, S1), s1), Us2 as (I.Root (H2, S2), s2), cands) =
        (case (H1, H2)
           ...
           of (I.BVar (k1), I.BVar(k2)) =>
            ...
            | (I.Proj (I.Bidx(k1), i1), I.Proj(I.LVar(r2, I.Shift(k2), (l2, t2)), i2)) =>
              let
                (* GEN BEGIN TAG OUTSIDE LET *) val I.BDec (bOpt, (l1, t1)) = I.ctxDec (G, k1) (* GEN END TAG OUTSIDE LET *)
              in
                if l1 <> l2 orelse i1 <> i2
                  then fail ("block index / block variable clash")
                else let (* GEN BEGIN TAG OUTSIDE LET *) val cands2 = matchSub (G, d, t1, I.comp(t2,I.Shift(k2)), cands) (* GEN END TAG OUTSIDE LET *)
                         ...
                         (* GEN BEGIN TAG OUTSIDE LET *) val _ = Unify.instantiateLVar (r2, I.Bidx (k1-k2)) (* GEN END TAG OUTSIDE LET *)
                     in
      ...
      | (* GEN BEGIN FUN BRANCH *) matchExpW (G, d, (I.Lam (D1, U1), s1), (I.Lam (D2, U2), s2), cands) =
           matchExp (I.Decl (G, I.decSub (D1, s1)), d+1, (U1, I.dot1 s1), (U2, I.dot1 s2), cands) (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) matchExpW (G, d, (I.Lam (D1, U1), s1), (U2, s2), cands) =
           (* eta-expand *)
           matchExp (I.Decl (G, I.decSub (D1, s1)), d+1,
                     ...
                     (I.Redex (I.EClo (U2, I.shift),
                      ...
                      I.dot1 s2),
                     cands) (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) matchExpW (G, d, (U1, s1), (I.Lam (D2, U2), s2), cands) =
           (* eta-expand *)
           matchExp (I.Decl (G, I.decSub (D2, s2)), d+1,
                     ...
                     (U2, I.dot1 s2),
                     cands) (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) matchExpW (G, d, Us1, Us2 as (I.EVar _, s2), cands) =
           addEqn (Eqn (G, Us1, Us2), cands) (* GEN END FUN BRANCH *)
    ...
    and (* GEN BEGIN FUN FIRST *) matchSpine (G, d, (I.Nil, _), (I.Nil, _), cands) = cands (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) matchSpine (G, d, (I.SClo (S1, s1'), s1), Ss2, cands) =
          matchSpine (G, d, (S1, I.comp (s1', s1)), Ss2, cands) (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) matchSpine (G, d, Ss1, (I.SClo (S2, s2'), s2), cands) =
          matchSpine (G, d, Ss1, (S2, I.comp (s2', s2)), cands) (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) matchSpine (G, d, (I.App (U1, S1), s1), (I.App (U2, S2), s2), cands) =
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val cands' = matchExp (G, d, (U1, s1), (U2, s2), cands) (* GEN END TAG OUTSIDE LET *)
        in
    ...
    *)
    ...
    and (* GEN BEGIN FUN FIRST *) matchSub (G, d, I.Shift(n1), I.Shift(n2), cands) = cands (* GEN END FUN FIRST *)
        (* by invariant *)
      | (* GEN BEGIN FUN BRANCH *) matchSub (G, d, I.Shift(n), s2 as I.Dot _, cands) =
          matchSub (G, d, I.Dot(I.Idx(n+1), I.Shift(n+1)), s2, cands) (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) matchSub (G, d, s1 as I.Dot _, I.Shift(m), cands) =
          matchSub (G, d, s1, I.Dot(I.Idx(m+1), I.Shift(m+1)), cands) (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) matchSub (G, d, I.Dot(Ft1,s1), I.Dot(Ft2,s2), cands) =
        let (* GEN BEGIN TAG OUTSIDE LET *) val cands1 =
           (case (Ft1, Ft2) of
             (I.Idx (n1), I.Idx (n2)) =>
               if n1 = n2
                 then cands
           ...
           | (I.Idx (n1), I.Exp (U2)) =>
                 matchExp (G, d, (I.Root (I.BVar (n1), I.Nil), I.id), (U2, I.id), cands)) (* GEN END TAG OUTSIDE LET *)
        in
    ...
    *)
    ...
    and (* GEN BEGIN FUN FIRST *) matchTopW (G, d, (I.Root (I.Const (c1), S1), s1),
        ...
        else fail ("type family clash") (* GEN END FUN FIRST *) (* fails, with no candidates since type families don't match *)
      | (* GEN BEGIN FUN BRANCH *) matchTopW (G, d, (I.Pi ((D1,_), V1), s1),
        ...
        (* Sat Dec 22 23:53:44 2001 -fp !!! *)
          matchTopW (I.Decl (G, D1), d+1, (V1, I.dot1 s1), (V2, I.dot1 s2), ci, cands) (* GEN END FUN BRANCH *)
    and (* GEN BEGIN FUN FIRST *) matchTopSpine (G, d, (I.Nil, _), (I.Nil, _), Cnil, cands) = cands (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) matchTopSpine (G, d, (I.SClo (S1, s1'), s1), Ss2, ci, cands) =
          matchTopSpine (G, d, (S1, I.comp (s1', s1)), Ss2, ci, cands) (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) matchTopSpine (G, d, Ss1, (I.SClo (S2, s2'), s2), ci, cands) =
          matchTopSpine (G, d, Ss1, (S2, I.comp (s2', s2)), ci, cands) (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) matchTopSpine (G, d, (I.App (U1, S1), s1), (I.App (U2, S2), s2),
                       Match (ci'), cands) =
      ...
      | (* GEN BEGIN FUN BRANCH *) matchTopSpine (G, d, (I.App (U1, S1), s1), (I.App (U2, S2), s2),
        ...
        (* an argument that need not be covered (Skip) *)
           matchTopSpine (G, d, (S1, s1), (S2, s2), ci', cands) (* GEN END FUN BRANCH *)
    ...
    *)
    fun (* GEN BEGIN FUN FIRST *) matchClause (G, ps', qs as (I.Root (_, _), s), ci) =
          checkConstraints (G, qs, resolveCands (matchTop (G, 0, ps', qs, ci, Eqns (nil)))) (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) matchClause (G, ps', (I.Pi ((I.Dec(_, V1), _), V2), s), ci) =
        let
          ...
          (* GEN BEGIN TAG OUTSIDE LET *) val X1 = Whnf.newLoweredEVar (G, (V1, s)) (* GEN END TAG OUTSIDE LET *)
    ...
    (* matchSig (G, (a @ S, s), ccs, ci, klist) = klist'
       ...
       klist <> Covered
    *)
    fun (* GEN BEGIN FUN FIRST *) matchSig (G, ps', nil, ci, klist) = klist (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) matchSig (G, ps', V::ccs', ci, klist) =
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val cands = CSManager.trail
                      ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => matchClause (G, ps', (V, I.id), ci) (* GEN END FUNCTION EXPRESSION *)) (* GEN END TAG OUTSIDE LET *)
        in
    ...
    *)
    and (* GEN BEGIN FUN FIRST *) matchSig' (G, ps', ccs, ci, Covered) = Covered (* GEN END FUN FIRST *) (* already covered: return *)
      | (* GEN BEGIN FUN BRANCH *) matchSig' (G, ps', ccs, ci, CandList (klist)) = (* not yet covered: continue to search *)
          matchSig (G, ps', ccs, ci, CandList (klist)) (* GEN END FUN BRANCH *)
     ...
     *)
    fun (* GEN BEGIN FUN FIRST *) matchBlocks (G, s', nil, V, k, i, ci, klist) = klist (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) matchBlocks (G, s', I.Dec (_, V')::piDecs, V, k, i, ci, klist) =
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val cands = CSManager.trail
                      ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => matchClause (G, (V, I.id), (V', s'), ci) (* GEN END FUNCTION EXPRESSION *)) (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val s'' = I.Dot (I.Exp (I.Root (I.Proj (I.Bidx k, i), I.Nil)), s') (* GEN END TAG OUTSIDE LET *)
        in
          matchBlocks' (G, s'', piDecs, V, k, i+1, ci, addKs (cands, klist))
        end (* GEN END FUN BRANCH *)
    and (* GEN BEGIN FUN FIRST *) matchBlocks' (G, s', piDecs, V, k, i, ci, Covered) = Covered (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) matchBlocks' (G, s', piDecs, V, k, i, ci, klist) =
          matchBlocks (G, s', piDecs, V, k, i, ci, klist) (* GEN END FUN BRANCH *) (* klist <> Covered *)
    ...
    *)
    fun (* GEN BEGIN FUN FIRST *) matchCtx (G, s', I.Null, V, k, ci, klist) = klist (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) matchCtx (G, s', I.Decl(G'', I.Dec(_, V')), V, k, ci, klist) =
        (* will always fail for input coverage *)
        let
          (*  G'', V' |- ^ : G''
              G |- s' : G'', V'
          *)
          (* GEN BEGIN TAG OUTSIDE LET *) val s'' = I.comp (I.shift, s') (* GEN END TAG OUTSIDE LET *)
          (*  G |- ^ o s' : G'' *)
          (* GEN BEGIN TAG OUTSIDE LET *) val cands = CSManager.trail
                      ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => matchClause (G, (V, I.id), (V', s''), ci) (* GEN END FUNCTION EXPRESSION *)) (* GEN END TAG OUTSIDE LET *)
        in
      ...
      | (* GEN BEGIN FUN BRANCH *) matchCtx (G, s', I.Decl(G'', I.BDec(_, (cid, s))), V, k, ci, klist) =
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val (Gsome, piDecs) = I.constBlock cid (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val s'' = I.comp (I.shift, s') (* GEN END TAG OUTSIDE LET *)
          (* G'' |- s : Gsome,
             G |- s'' : G''
             G |- s o s'' : Gsome
             Gsome |- piDecs : ctx
          *)
          (* GEN BEGIN TAG OUTSIDE LET *) val klist' = matchBlocks (G, I.comp (s, s''), piDecs, V, k, 1, ci, klist) (* GEN END TAG OUTSIDE LET *)
        in
          matchCtx' (G, s'', G'', V, k+1, ci, klist')
        end (* GEN END FUN BRANCH *)

    and (* GEN BEGIN FUN FIRST *) matchCtx' (G, s', G', V, k, ci, Covered) = Covered (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) matchCtx' (G, s', G', V, k, ci, CandList (klist)) =
          matchCtx (G, s', G', V, k, ci, CandList (klist)) (* GEN END FUN BRANCH *)

    (* as matchClause *)
    fun (* GEN BEGIN FUN FIRST *) matchOut (G, V, ci, (V', s'), 0) =
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val cands = matchTop (G, 0, (V, I.id), (V', s'), ci, Eqns(nil)) (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val cands' = resolveCands (cands) (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val cands'' = checkConstraints (G, (V', s'), cands') (* GEN END TAG OUTSIDE LET *)
        in
          addKs (cands'', CandList (nil))
        end (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) matchOut (G, V, ci, (V' as I.Pi ((I.Dec(_, V1'), _), V2'), s'), p) = (* p > 0 *)
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val X1 = Whnf.newLoweredEVar (G, (V1', s')) (* GEN END TAG OUTSIDE LET *)
    ...
    (* match (., V, p, ci, I/O(ccs)) = klist
       ...
       G |- V : type
    *)
    fun (* GEN BEGIN FUN FIRST *) match (G, V as I.Root (I.Const (a), S), 0, ci, Input(ccs)) =
          matchCtx' (G, I.id, G, V, 1, ci,
                     matchSig (G, (V, I.id), ccs, ci, CandList (nil))) (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) match (G, V, 0, ci, Output (V', q)) =
          matchOut (G, V, ci, (V', I.Shift (I.ctxLength G)), q) (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) match (G, I.Pi ((D, _), V'), p, ci, ccs) =
          match (I.Decl (G, D), V', p-1, ci, ccs) (* GEN END FUN BRANCH *)
    ...
    *)
    fun (* GEN BEGIN FUN FIRST *) insert (k, nil) = ((k, 1)::nil) (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) insert (k, ksn as (k', n')::ksn') =
        (case Int.compare (k, k')
           of LESS => (k, 1)::ksn
            | EQUAL => (k', n'+1)::ksn'
            | GREATER => (k', n')::insert (k, ksn')) (* GEN END FUN BRANCH *)

    (* join (ks, ksn) = ksn'
       ksn is as in function insert
    *)
    fun (* GEN BEGIN FUN FIRST *) join (nil, ksn) = ksn (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) join (k::ks, ksn) = join (ks, insert (k, ksn)) (* GEN END FUN BRANCH *)
    ...
    *)
    fun (* GEN BEGIN FUN FIRST *) selectCand (Covered) = NONE (* GEN END FUN FIRST *)     (* success: case is covered! *)
      | (* GEN BEGIN FUN BRANCH *) selectCand (CandList (klist)) = selectCand' (klist, nil) (* GEN END FUN BRANCH *)

    and (* GEN BEGIN FUN FIRST *) selectCand' (nil, ksn) = SOME(ksn) (* GEN END FUN FIRST *) (* failure: case G,V is not covered! *)
      | (* GEN BEGIN FUN BRANCH *) selectCand' (Fail::klist, ksn) = (* local failure (clash) and no candidates *)
          selectCand' (klist, ksn) (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) selectCand' (Cands(nil)::klist, ksn) = (* local failure but no candidates *)
          selectCand' (klist, ksn) (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) selectCand' (Cands(ks)::klist, ksn) = (* found candidates ks <> nil *)
          selectCand' (klist, join (ks, ksn)) (* GEN END FUN BRANCH *)
    ...
    *)
    ...
    and (* GEN BEGIN FUN FIRST *) instEVarsW (Vs, 0, XsRev) = (Vs, XsRev) (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) instEVarsW ((I.Pi ((I.Dec (xOpt, V1), _), V2), s), p, XsRev) =
        let (* p > 0 *)
          (* GEN BEGIN TAG OUTSIDE LET *) val X1 = Whnf.newLoweredEVar (I.Null, (V1, s)) (* GEN END TAG OUTSIDE LET *) (* all EVars are global *)
      ...
      | (* GEN BEGIN FUN BRANCH *) instEVarsW ((I.Pi ((I.BDec (_, (l, t)), _), V2), s), p, XsRev) =
        ...
        (* . |- s : G0 *)
        let (* p > 0 *)
          (* GEN BEGIN TAG OUTSIDE LET *) val L1 = I.newLVar (I.Shift(0), (l, I.comp(t, s))) (* GEN END TAG OUTSIDE LET *)
    ...
    *)
    local
      (* GEN BEGIN TAG OUTSIDE LET *) val caseList : (I.exp * int) list ref = ref nil (* GEN END TAG OUTSIDE LET *)
    in
    ...
    fun (* GEN BEGIN FUN FIRST *) createEVarSub (I.Null) = I.id (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) createEVarSub (I.Decl(G', D as I.Dec (_, V))) =
        let
          ...
          (* GEN BEGIN TAG OUTSIDE LET *) val X = Whnf.newLoweredEVar (I.Null, (V, s)) (* GEN END TAG OUTSIDE LET *)
    ...
    *)
    fun blockCases (G, Vs, cid, (Gsome, piDecs), sc) =
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val t = createEVarSub Gsome (* GEN END TAG OUTSIDE LET *)
          (* . |- t : Gsome *)
          ...
          (* GEN BEGIN TAG OUTSIDE LET *) val lvar = I.newLVar (sk, (cid, t)) (* GEN END TAG OUTSIDE LET *)
    ...
    fun (* GEN BEGIN FUN FIRST *) lowerSplitW (X as I.EVar (_, G, V, _), W, sc) =
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val sc' = (* GEN BEGIN FUNCTION EXPRESSION *) fn U =>  if Unify.unifiable (G, (X, I.id), (U, I.id))
                                then sc ()
                              else () (* GEN END FUNCTION EXPRESSION *) (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val _ = paramCases (G, (V, I.id), I.ctxLength G, sc') (* GEN END TAG OUTSIDE LET *) (* will trail *)
          (* GEN BEGIN TAG OUTSIDE LET *) val _ = worldCases (G, (V, I.id), W, sc') (* GEN END TAG OUTSIDE LET *) (* will trail *)
          (* GEN BEGIN TAG OUTSIDE LET *) val _ = constCases (G, (V, I.id), Index.lookup (I.targetFam V), sc') (* GEN END TAG OUTSIDE LET *) (* will trail *)
        in
          ()
        end (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) lowerSplitW (I.Lam (D, U), W, sc) =
          lowerSplitW (U, W, sc) (* GEN END FUN BRANCH *)
...
(*        val _ = print ("Consider C cases for "  ^ Print.expToString (G, I.EClo Vs) ^ "\n") *)
   ...
   fun splitEVar ((X as I.EVar (_, GX, V, _)), W, sc, print) = (* GX = I.Null *)
    ...
    fun abstract (V, s) =
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val (i, V') = Abstract.abstractDecImp (I.EClo (V, s)) (* GEN END TAG OUTSIDE LET *)
    ...
    *)
    fun splitVar (V, p, k, (W, ci)) =
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val _ = chatter 6 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => showSplitVar (V, p, k, ci) ^ "\n" (* GEN END FUNCTION EXPRESSION *)) (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val ((V1, s), XsRev) = instEVars ((V, I.id), p, nil) (* GEN END TAG OUTSIDE LET *)
          (* split on k'th variable, counting from innermost *)
          (* GEN BEGIN TAG OUTSIDE LET *) val SOME(X) = List.nth (XsRev, k-1) (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val _ = resetCases () (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val _ = splitEVar (X, W, (* GEN BEGIN FUNCTION EXPRESSION *) fn () => addCase (abstract (V1, s)) (* GEN END FUNCTION EXPRESSION *)) (* GEN END TAG OUTSIDE LET *) (* may raise Constraints.Error *)
        in
          SOME (getCases ())
        end
        (* Constraints.Error could be raised by abstract *)
        handle Constraints.Error (constrs) =>
          (chatter 7 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => "Inactive split:\n" ^ Print.cnstrsToString (constrs) ^ "\n" (* GEN END FUNCTION EXPRESSION *));
           NONE)
    ...
    *)
    ...
    fun (* GEN BEGIN FUN FIRST *) occursInExp (k, I.Uni _) = false (* GEN END FUN FIRST *)
      ...
      | (* GEN BEGIN FUN BRANCH *) occursInExp (k, I.Root (H, S)) = occursInHead (k, H) orelse occursInSpine (k, S) (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) occursInExp (k, I.Lam (D, V)) = occursInDec (k, D) orelse occursInExp (k+1, V) (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) occursInExp (k, I.FgnExp (cs, ops)) = false (* GEN END FUN BRANCH *)
    ...
    and (* GEN BEGIN FUN FIRST *) occursInHead (k, I.BVar (k')) = (k = k') (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) occursInHead (k, _) = false (* GEN END FUN BRANCH *)
    ...
    (* occursInMatchPos (k, U, ci) = true
       ...
       instructions ci
    *)
    fun (* GEN BEGIN FUN FIRST *) occursInMatchPos (k, I.Pi (DP, V), ci) =
          occursInMatchPos (k+1, V, ci) (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) occursInMatchPos (k, I.Root (H, S), ci) =
          occursInMatchPosSpine (k, S, ci) (* GEN END FUN BRANCH *)
    and (* GEN BEGIN FUN FIRST *) occursInMatchPosSpine (k, I.Nil, Cnil) = false (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) occursInMatchPosSpine (k, I.App(U, S), Match(ci)) =
          occursInExp (k, U) orelse occursInMatchPosSpine (k, S, ci) (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) occursInMatchPosSpine (k, I.App(U, S), Skip(ci)) =
          occursInMatchPosSpine (k, S, ci) (* GEN END FUN BRANCH *)
    ...
    *)
    ...
    and (* GEN BEGIN FUN FIRST *) InstEVarsSkipW (Vs, 0, XsRev, ci) = (Vs, XsRev) (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) InstEVarsSkipW ((I.Pi ((I.Dec (xOpt, V1), _), V2), s), p, XsRev, ci) =
        let (* p > 0 *)
          (* GEN BEGIN TAG OUTSIDE LET *) val X1 = Whnf.newLoweredEVar (I.Null, (V1, s)) (* GEN END TAG OUTSIDE LET *) (* all EVars are global *)
          (* was val X1 = I.newEVar (I.Null, I.EClo (V1, s)) (* all EVars are global *)
             Mon Feb 28 15:25:42 2011 --cs *)
          (* GEN BEGIN TAG OUTSIDE LET *) val EVarOpt = if occursInMatchPos (1, V2, ci)
                          then SOME(X1)
                        else NONE (* GEN END TAG OUTSIDE LET *)
        in
          instEVarsSkip ((V2, I.Dot (I.Exp (X1), s)), p-1, EVarOpt::XsRev, ci)
        end (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) InstEVarsSkipW ((I.Pi ((I.BDec (_, (l, t)), _), V2), s), p, XsRev, ci) =
        (* G0 |- t : Gsome *)
        (* . |- s : G0 *)
        let (* p > 0 *)
          (* GEN BEGIN TAG OUTSIDE LET *) val L1 = I.newLVar (I.Shift(0), (l, I.comp(t, s))) (* GEN END TAG OUTSIDE LET *)
    ...
    fun (* GEN BEGIN FUN FIRST *) targetBelowEq (a, I.EVar (ref(NONE), GY, VY, ref nil)) =
          Subordinate.belowEq (a, I.targetFam VY) (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) targetBelowEq (a, I.EVar (ref(NONE), GY, VY, ref (_::_))) =
    ...
    *)
    fun (* GEN BEGIN FUN FIRST *) recursive (X as I.EVar (ref(SOME(U)), GX, VX, _)) =
        let (* GX = I.Null*)
            (* is this always true? --cs!!!*)
          (* GEN BEGIN TAG OUTSIDE LET *) val a = I.targetFam VX (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val Ys = Abstract.collectEVars (GX, (X, I.id), nil) (* GEN END TAG OUTSIDE LET *)
          (* LVars are ignored here.  OK because never splittable? *)
          (* Sat Dec 15 22:42:10 2001 -fp !!! *)
          (* GEN BEGIN TAG OUTSIDE LET *) val recp = List.exists ((* GEN BEGIN FUNCTION EXPRESSION *) fn Y => targetBelowEq (a, Y) (* GEN END FUNCTION EXPRESSION *)) Ys (* GEN END TAG OUTSIDE LET *)
        in
          recp
        end (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) recursive (I.Lam (D, U)) = recursive U (* GEN END FUN BRANCH *)

    local
      (* GEN BEGIN TAG OUTSIDE LET *) val counter = ref 0 (* GEN END TAG OUTSIDE LET *)
    in
    ...
    *)
    fun finitary1 (X, k, W, f, cands) =
        ( resetCount () ;
          chatter 7 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => "Trying " ^ Print.expToString (I.Null, X) ^ " : "
                     ^ ".\n" (* GEN END FUNCTION EXPRESSION *)) ;
          ( splitEVar (X, W, (* GEN BEGIN FUNCTION EXPRESSION *) fn () => (f (); if recursive X
                                        then raise NotFinitary
                                      else incCount ()) (* GEN END FUNCTION EXPRESSION *)) ;
            chatter 7 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => "Finitary with " ^ Int.toString (getCount ()) ^ " candidates.\n" (* GEN END FUNCTION EXPRESSION *));
    
            (k, getCount ())::cands )
           handle NotFinitary => ( chatter 7 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => "Not finitary.\n" (* GEN END FUNCTION EXPRESSION *));
                                   cands )
                 | Constraints.Error (constrs) =>
                                 ( chatter 7 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => "Inactive finitary split.\n" (* GEN END FUNCTION EXPRESSION *));
                                   cands )
        )
    ...
    *)
    fun (* GEN BEGIN FUN FIRST *) finitarySplits (nil, k, W, f, cands) = cands (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) finitarySplits (NONE::Xs, k, W, f, cands) =
        (* parameter blocks can never be split *)
          finitarySplits (Xs, k+1, W, f, cands) (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) finitarySplits (SOME(X)::Xs, k, W, f, cands) =
          finitarySplits (Xs, k+1, W, f, CSManager.trail ((* GEN BEGIN FUNCTION EXPRESSION *) fn () =>  finitary1 (X, k, W, f,cands) (* GEN END FUNCTION EXPRESSION *))) (* GEN END FUN BRANCH *)
    ...
    *)
    ...
    fun finitary (V, p, W, ci) =
        let
          ...
          (* GEN BEGIN TAG OUTSIDE LET *) val ((V1, s), XsRev) = instEVarsSkip ((V, I.id), p, nil, ci) (* GEN END TAG OUTSIDE LET *)
        in
          finitarySplits (XsRev, 1, W, (* GEN BEGIN FUNCTION EXPRESSION *) fn () => (abstract (V1, s)) (* GEN END FUNCTION EXPRESSION *), nil)
        end
    ...
    *)
    fun (* GEN BEGIN FUN FIRST *) eqInpSpine (ms, (I.SClo(S1,s1'),s1), Ss2) =
          eqInpSpine (ms, (S1, I.comp(s1',s1)), Ss2) (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) eqInpSpine (ms, Ss1, (I.SClo(S2,s2'),s2)) =
          eqInpSpine (ms, Ss1, (S2, I.comp(s2',s2))) (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) eqInpSpine (M.Mnil, (I.Nil,s), (I.Nil,s')) = true (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) eqInpSpine (M.Mapp(M.Marg(M.Plus,_), ms'),
                  (I.App(U,S),s), (I.App(U',S'),s')) =
          eqExp ((U,s), (U',s'))
          andalso eqInpSpine (ms', (S,s), (S',s')) (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) eqInpSpine (M.Mapp(_, ms'), (I.App(U,S),s), (I.App(U',S'),s')) =
          (* ignore Star, Minus, Minus1 *)
          eqInpSpine (ms', (S,s), (S',s')) (* GEN END FUN BRANCH *)
    ...
    *)
    fun (* GEN BEGIN FUN FIRST *) eqInp (I.Null, k, a, Ss, ms) = nil (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) eqInp (I.Decl(G', I.Dec(_, I.Root (I.Const(a'), S'))), k, a, Ss, ms) =
        (* defined type families disallowed here *)
        if a = a' andalso eqInpSpine (ms, (S', I.Shift(k)), Ss)
          then k::eqInp (G', k+1, a, Ss, ms)
        else eqInp (G', k+1, a, Ss, ms) (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) eqInp (I.Decl(G', I.Dec(_, I.Pi _)), k, a, Ss, ms) =
          eqInp (G', k+1, a, Ss, ms) (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) eqInp (I.Decl(G', I.NDec _), k, a, Ss, ms) =
          eqInp (G', k+1, a, Ss, ms) (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) eqInp (I.Decl(G', I.BDec(_, (b, t))), k, a, Ss, ms) =
          eqInp (G', k+1, a, Ss, ms) (* GEN END FUN BRANCH *)
    ...
    *)
    fun (* GEN BEGIN FUN FIRST *) contractionCands (I.Null, k) = nil (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) contractionCands (I.Decl(G', I.Dec(_, I.Root (I.Const(a), S))), k) =
        (* defined type families disallowed here *)
            ...
            | SOME(ms) =>
              case eqInp (G', k+1, a, (S, I.Shift(k)), ms)
                of nil => contractionCands (G', k+1)
                 | ns => (k::ns)::contractionCands (G', k+1)) (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) contractionCands (I.Decl(G', I.Dec(_, I.Pi _)), k) =
          (* ignore Pi --- contraction cands unclear *)
          contractionCands (G', k+1) (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) contractionCands (I.Decl(G', I.NDec _), k) =
          contractionCands (G', k+1) (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) contractionCands (I.Decl(G', I.BDec(_, (b, t))), k) =
    ...
    (* isolateSplittable ((G0, {{G1}}V, p) = ((G0@G1), V) where |G1| = p
       ...
       coverage goal ({{G}}V, p)
    *)
    fun (* GEN BEGIN FUN FIRST *) isolateSplittable (G, V, 0) = (G, V) (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) isolateSplittable (G, I.Pi((D,_), V'), p) =
          isolateSplittable (I.Decl(G, D), V', p-1) (* GEN END FUN BRANCH *)
    ...
    (* unifyUOutSpine (ms, S1[s1], S2[s2]) = true
       ...
       Effect: EVars in S1[s1], S2[s2] are instantianted, both upon
          failure and success
    *)
    fun (* GEN BEGIN FUN FIRST *) unifyUOutSpine (ms, (I.SClo(S1,s1'),s1), Ss2) =
          unifyUOutSpine (ms, (S1, I.comp(s1', s1)), Ss2) (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) unifyUOutSpine (ms, Ss1, (I.SClo(S2,s2'),s2)) =
          unifyUOutSpine (ms, Ss1, (S2, I.comp(s2',s2))) (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) unifyUOutSpine (M.Mnil, (I.Nil,s1), (I.Nil,s2)) = true (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) unifyUOutSpine (M.Mapp(M.Marg(M.Minus1,_),ms'), (I.App(U1,S1),s1), (I.App(U2,S2),s2)) =
          Unify.unifiable (I.Null, (U1,s1), (U2,s2)) (* will have effect! *)
          andalso unifyUOutSpine (ms', (S1,s1), (S2,s2)) (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) unifyUOutSpine (M.Mapp(_,ms'), (I.App(U1,S1),s1), (I.App(U2,S2), s2)) =
          (* if mode = + already equal by invariant; otherwise ignore *)
          unifyUOutSpine (ms', (S1,s1), (S2,s2)) (* GEN END FUN BRANCH *)
    ...
    *)
    ...
    and unifyUOutTypeW ((I.Root(I.Const(a1),S1),s1), (I.Root(I.Const(a2),S2),s2)) =
        (* a1 = a2 by invariant *)
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val SOME(ms) = UniqueTable.modeLookup a1 (* GEN END TAG OUTSIDE LET *) (* must succeed by invariant *)
        in
    ...
    *)
    fun (* GEN BEGIN FUN FIRST *) unifyUOut1 (XsRev, nil) = true (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) unifyUOut1 (XsRev, k1::nil) = true (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) unifyUOut1 (XsRev, k1::k2::ks) =
          unifyUOut2 (XsRev, k1, k2)
          andalso unifyUOut1 (XsRev, k2::ks) (* GEN END FUN BRANCH *)

    (* unifyOut ([X1,...,Xp], [[k11,...,k1{n1}],...,[km1,...,km{nm}]]) = true
       if unifyOut1 ([X1,...,Xp], [kj1,...,kj{nj}]) for each j
    *)
    fun (* GEN BEGIN FUN FIRST *) unifyUOut (XsRev, nil) = true (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) unifyUOut (XsRev, ks::kss) =
          unifyUOut1 (XsRev, ks)
          andalso unifyUOut (XsRev, kss) (* GEN END FUN BRANCH *)
    ...
    *)
    fun contractAll (V, p, ucands) =
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val ((V1, s), XsRev) = instEVars ((V, I.id), p, nil) (* GEN END TAG OUTSIDE LET *) (* as in splitVar *)
        in
    ...
    *)
    fun contract (V, p, ci, lab) =
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val (G, _) = isolateSplittable (I.Null, V, p) (* GEN END TAG OUTSIDE LET *) (* ignore body of coverage goal *)
          (* GEN BEGIN TAG OUTSIDE LET *) val ucands = contractionCands (G, 1) (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val n = List.length ucands (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val _ = if n > 0
                    then chatter 6 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => "Found " ^ Int.toString n ^ " contraction "
                                    ^ pluralize (n, "candidate") ^ "\n" (* GEN END FUNCTION EXPRESSION *))
                  else () (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val VpOpt' = if n > 0
                         then (contractAll (V, p, ucands)
                               handle Constraints.Error _ =>
                                      ( chatter 6 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => "Contraction failed due to constraints\n" (* GEN END FUNCTION EXPRESSION *));
                                        SOME(V, p) ))
                                        (* no progress if constraints remain *)
                       else SOME(V, p) (* GEN END TAG OUTSIDE LET *)  (* no candidates, no progress *)
          (* GEN BEGIN TAG OUTSIDE LET *) val _ = case VpOpt'
                    of NONE => chatter 6 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => "Case impossible: conflicting unique outputs\n" (* GEN END FUNCTION EXPRESSION *))
                     | SOME(V',p') => chatter 6 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => showPendingGoal (V', p', ci, lab) ^ "\n" (* GEN END FUNCTION EXPRESSION *)) (* GEN END TAG OUTSIDE LET *)
        in
          VpOpt'
        end
    ...
    *)
    ...
    and (* GEN BEGIN FUN FIRST *) findMin' ((k0,n0), nil) = (k0,n0) (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) findMin' ((k0,n0), (k',n')::kns) =
        if n' < n0
          then findMin' ((k',n'), kns)
        else findMin' ((k0,n0), kns) (* GEN END FUN BRANCH *)
    ...
    *)
    fun cover (V, p, wci as (W, ci), ccs, lab, missing) =
        ( chatter 6 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => showPendingGoal (V, p, ci, lab) ^ "\n" (* GEN END FUNCTION EXPRESSION *));
          cover' (contract(V, p, ci, lab), wci, ccs, lab, missing) )

    and (* GEN BEGIN FUN FIRST *) cover' (SOME(V, p), wci as (W, ci), ccs, lab, missing) =
          split (V, p, selectCand (match (I.Null, V, p, ci, ccs)), wci, ccs, lab, missing) (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) cover' (NONE, wci, ccs, lab, missing) =
        (* V is covered by unique output inconsistency *)
        ( chatter 6 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => "Covered\n" (* GEN END FUNCTION EXPRESSION *));
          missing ) (* GEN END FUN BRANCH *)

    and (* GEN BEGIN FUN FIRST *) split (V, p, NONE, wci, ccs, lab, missing) =
        (* V is covered: return missing patterns from other cases *)
        ( chatter 6 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => "Covered\n" (* GEN END FUNCTION EXPRESSION *));
          missing ) (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) split (V, p, SOME(nil), wci as (W, ci), ccs, lab, missing) =
        (* no strong candidates: check for finitary splitting candidates *)
        ( chatter 6 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => "No strong candidates---calculating weak candidates\n" (* GEN END FUNCTION EXPRESSION *));
          splitWeak (V, p, finitary (V, p, W, ci), wci, ccs, lab, missing) ) (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) split (V, p, SOME((k,_)::ksn), wci as (W, ci), ccs, lab, missing) =
        ...
        (case splitVar (V, p, k, wci)
           of SOME(cases) => covers (cases, wci, ccs, lab, missing)
            | NONE => (* splitting variable k generated constraints *)
              split (V, p, SOME (ksn), wci, ccs, lab, missing)) (* GEN END FUN BRANCH *) (* try other candidates *)

    and (* GEN BEGIN FUN FIRST *) splitWeak (V, p, nil, wci, ccs, lab, missing) =
        ( chatter 6 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => "No weak candidates---case " ^ labToString(lab) ^ " not covered\n" (* GEN END FUNCTION EXPRESSION *));
          (V,p)::missing ) (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) splitWeak (V, p, ksn, wci, ccs, lab, missing) = (* ksn <> nil *)
        (* commit to the minimal candidate, since no constraints can arise *)
          split (V, p, SOME(findMin ksn::nil), wci, ccs, lab, missing) (* GEN END FUN BRANCH *)

    and covers (cases, wci, ccs, lab, missing) =
        ( chatter 6 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => "Found " ^ Int.toString (List.length cases)
                              ^ pluralize (List.length cases, " case") ^ "\n" (* GEN END FUNCTION EXPRESSION *));
          covers' (cases, 1, wci, ccs, lab, missing) )

    and (* GEN BEGIN FUN FIRST *) covers' (nil, n, wci, ccs, lab, missing) =
        ( chatter 6 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => "All subcases of " ^ labToString(lab) ^ " considered\n" (* GEN END FUNCTION EXPRESSION *));
          missing ) (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) covers' ((V,p)::cases', n, wci, ccs, lab, missing) =
          covers' (cases', n+1, wci, ccs, lab, cover (V, p, wci, ccs, Child(lab, n), missing)) (* GEN END FUN BRANCH *)
    ...
    *)
    fun (* GEN BEGIN FUN FIRST *) constsToTypes (nil) = nil (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) constsToTypes (I.Const(c)::cs') = I.constType(c)::constsToTypes(cs') (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) constsToTypes (I.Def(d)::cs') = I.constType(d)::constsToTypes(cs') (* GEN END FUN BRANCH *)
    ...
    (* createCoverClause (G, V, 0) = ({{G}} V, |G|)
       where {{G}} V is in NF
    *)
    fun (* GEN BEGIN FUN FIRST *) createCoverClause (I.Decl (G, D), V, p) =
          createCoverClause (G, I.Pi ((D, I.Maybe), V), p+1) (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) createCoverClause (I.Null, V, p) =
          (Whnf.normalize (V, I.id), p) (* GEN END FUN BRANCH *)
    ...
    *)
    ...
    and (* GEN BEGIN FUN FIRST *) createCoverGoalW (G, (I.Pi ((D1,P1), V2), s), 0, ms) =
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val D1' = I.decSub (D1, s) (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val V2' = createCoverGoal (I.Decl (G, D1'), (V2, I.dot1 s), 0, ms) (* GEN END TAG OUTSIDE LET *)
        in
          I.Pi ((D1',P1), V2')
        end (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) createCoverGoalW (G, (I.Pi ((D as I.Dec (_, V1), _), V2), s), p, ms) =
        let (* p > 0, G = I.Null *)
          (* GEN BEGIN TAG OUTSIDE LET *) val X = Whnf.newLoweredEVar (G, (V1, s)) (* GEN END TAG OUTSIDE LET *)
          (* was  val X = I.newEVar (G, I.EClo (V1, s))  Mon Feb 28 15:33:52 2011 -cs *)
        in
          createCoverGoal (G, (V2, I.Dot (I.Exp (X), s)), p-1, ms)
        end (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) createCoverGoalW (G, (I.Root (a as I.Const(cid), S), s), p, ms) = (* s = id, p >= 0 *)
        I.Root (a, createCoverSpine (G, (S, s), (I.constType (cid), I.id), ms)) (* GEN END FUN BRANCH *)

    and (* GEN BEGIN FUN FIRST *) createCoverSpine (G, (I.Nil, s), _, M.Mnil) = I.Nil (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) createCoverSpine (G, (I.App (U1, S2), s), (I.Pi ((I.Dec (_, V1), _), V2), s'),
                          M.Mapp (M.Marg (M.Minus, x), ms')) =
        (* replace output argument by new variable *)
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val X = createEVar (G, I.EClo (V1, s')) (* GEN END TAG OUTSIDE LET *) (* strengthen G based on subordination *)
          (* GEN BEGIN TAG OUTSIDE LET *) val S2' = createCoverSpine (G, (S2, s), (V2, I.Dot (I.Exp (X), s')), ms') (* GEN END TAG OUTSIDE LET *)
        in
          I.App (X, S2')
        end (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) createCoverSpine (G, (I.App (U1, S2), s), (I.Pi (_, V2), s'), M.Mapp (_, ms')) =
        (* leave input ( + ) arguments as they are, ignore ( * ) impossible *)
          I.App (I.EClo (U1, s),
                 createCoverSpine (G, (S2, s), Whnf.whnf (V2, I.Dot (I.Exp (I.EClo (U1, s)), s')),
                                   ms')) (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) createCoverSpine (G, (I.SClo (S, s'), s), Vs, ms) =
          createCoverSpine (G, (S, I.comp (s', s)), Vs, ms) (* GEN END FUN BRANCH *)
  ...
  in
    ...
    *)
    fun checkCovers (a, ms) =
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val _ = chatter 4 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => "Input coverage checking family " ^ N.qidToString (N.constQid a)
                             ^ "\n" (* GEN END FUNCTION EXPRESSION *)) (* GEN END TAG OUTSIDE LET *)
          ...
          (* GEN BEGIN TAG OUTSIDE LET *) val (V0, p) = initCGoal (a) (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val _ = if !Global.doubleCheck
                    then TypeCheck.typeCheck (I.Null, (V0, I.Uni (I.Type)))
                  else () (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val _ = CSManager.reset () (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val cIn = inCoverInst ms (* GEN END TAG OUTSIDE LET *)      (* convert mode spine to cover instructions *)
          (* GEN BEGIN TAG OUTSIDE LET *) val cs = Index.lookup a (* GEN END TAG OUTSIDE LET *)       (* lookup constants defining a *)
          (* GEN BEGIN TAG OUTSIDE LET *) val ccs = constsToTypes cs (* GEN END TAG OUTSIDE LET *)    (* calculate covering clauses *)
          (* GEN BEGIN TAG OUTSIDE LET *) val W = W.lookup a (* GEN END TAG OUTSIDE LET *)            (* world declarations for a; must be defined *)
          (* GEN BEGIN TAG OUTSIDE LET *) val V0 = createCoverGoal (I.Null, (V0, I.id), p, ms) (* GEN END TAG OUTSIDE LET *) (* replace output by new EVars *)
          (* GEN BEGIN TAG OUTSIDE LET *) val (V0, p) = abstract (V0, I.id) (* GEN END TAG OUTSIDE LET *)    (* abstract will double-check *)
          (* GEN BEGIN TAG OUTSIDE LET *) val missing = cover (V0, p, (W, cIn), Input(ccs), Top, nil) (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val _ = case missing
    ...
    *)
    fun checkOut (G, (V, s)) =
        let
          ...
          (* GEN BEGIN TAG OUTSIDE LET *) val cOut = outCoverInst ms (* GEN END TAG OUTSIDE LET *)    (* determine cover instructions *)
          (* GEN BEGIN TAG OUTSIDE LET *) val (V', q) = createCoverClause (G, I.EClo(V, s), 0) (* GEN END TAG OUTSIDE LET *) (* abstract all variables in G *)
          (* GEN BEGIN TAG OUTSIDE LET *) val _ = if !Global.doubleCheck
                    then TypeCheck.typeCheck (I.Null, (V', I.Uni (I.Type)))
                  else () (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val V0 = createCoverGoal (I.Null, (V', I.id), q, ms) (* GEN END TAG OUTSIDE LET *) (* replace output by new EVars *)
          (* GEN BEGIN TAG OUTSIDE LET *) val (V0', p) = abstract (V0, I.id) (* GEN END TAG OUTSIDE LET *)    (* abstract will double-check *)
          (* GEN BEGIN TAG OUTSIDE LET *) val W = W.lookup a (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val missing = cover (V0', p, (W, cOut), Output(V',q), Top, nil) (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val _ = case missing
                    of nil => ()
    ...
    (* newEVarSubst (G, G') = s
       Invariant:   If G = xn:Vn,...,x1:V1
                  then s = X1...Xn.^k
                     G |- s : G'
    *)
    fun (* GEN BEGIN FUN FIRST *) newEVarSubst (G, I.Null) = I.Shift(I.ctxLength(G)) (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) newEVarSubst (G, I.Decl(G', D as I.Dec (_, V))) =
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val s' = newEVarSubst (G, G') (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val X = Whnf.newLoweredEVar (G, (V, s')) (* GEN END TAG OUTSIDE LET *)
          (* was val V' = I.EClo (V, s')
                 val X = I.newEVar (G, V') Mon Feb 28 15:34:31 2011 -cs *)
        in
          I.Dot (I.Exp (X), s')
        end (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) newEVarSubst (G, I.Decl(G', D as I.NDec _)) =
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val s' = newEVarSubst (G, G') (* GEN END TAG OUTSIDE LET *)
        in
          I.Dot (I.Undef, s')
        end (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) newEVarSubst (G, I.Decl(G', D as I.BDec (_, (b, t)))) =
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val s' = newEVarSubst (G, G') (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val L1 = I.newLVar (s', (b, t)) (* GEN END TAG OUTSIDE LET *)
    ...
    *)
    (* This ignores LVars, because collectEVars does *)
    (* Why is that OK?  Sun Dec 16 09:01:40 2001 -fp !!! *)
    fun (* GEN BEGIN FUN FIRST *) checkConstraints (G, (Si, ti), Cands (ks)) = Cands (ks) (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) checkConstraints (G, (Si, ti), Fail) = Fail (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) checkConstraints (G, (Si, ti), Eqns _) = (* _ = nil *)
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val Xs = Abstract.collectEVarsSpine (G, (Si, ti), nil) (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val constrs = collectConstraints Xs (* GEN END TAG OUTSIDE LET *)
        in
          case constrs
            of nil => Eqns (nil)
    ...
    *)
    fun matchClause (CGoal (G, S), (Si, ti)) =
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val cands1 = matchSpine (G, 0, (S, I.id), (Si, ti), Eqns (nil)) (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val cands2 = resolveCands cands1 (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val cands3 = checkConstraints (G, (Si, ti), cands2) (* GEN END TAG OUTSIDE LET *)
        in
          cands3
        end
    ...
    *)
    fun (* GEN BEGIN FUN FIRST *) matchClauses (cg, nil, klist) = klist (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) matchClauses (cg as CGoal(G, S), (CClause (Gi, Si)::ccs), klist) =
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val ti = newEVarSubst (G, Gi) (* GEN END TAG OUTSIDE LET *) (* G |- ti : Gi *)
          (* GEN BEGIN TAG OUTSIDE LET *) val cands = CSManager.trail ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => matchClause (cg, (Si, ti)) (* GEN END FUNCTION EXPRESSION *)) (* GEN END TAG OUTSIDE LET *)
        in
          matchClauses' (cg, ccs, addKs (cands, klist))
        end (* GEN END FUN BRANCH *)
    and (* GEN BEGIN FUN FIRST *) matchClauses' (cg, ccs, Covered) = Covered (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) matchClauses' (cg, ccs, klist as CandList _) =
          matchClauses (cg, ccs, klist) (* GEN END FUN BRANCH *)
    ...
    *)
    fun abstractSpine (S, s) =
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val (G', S') = Abstract.abstractSpine (S, s) (* GEN END TAG OUTSIDE LET *)
    
          (* GEN BEGIN TAG OUTSIDE LET *) val namedG' = N.ctxName G' (* GEN END TAG OUTSIDE LET *) (* for printing purposes *)
          (* GEN BEGIN TAG OUTSIDE LET *) val _ = if !Global.doubleCheck
    ...
    *)
    fun (* GEN BEGIN FUN FIRST *) kthSub (I.Dot (I.Exp(X), s), 1) = X (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) kthSub (I.Dot (_, s), k) = kthSub (s, k-1) (* GEN END FUN BRANCH *)

    (* subToXsRev (X1...Xn.^0) = [Xiopt,...,Xnopt]
       Invariant: Xi are either EVars (translate to SOME(Xi))
                  or not (translate to NONE)
    *)
    fun (* GEN BEGIN FUN FIRST *) subToXsRev (I.Shift(0)) = nil (* GEN END FUN FIRST *) (* n = 0 *)
      | (* GEN BEGIN FUN BRANCH *) subToXsRev (I.Dot (I.Exp(X), s)) =
          SOME(X)::subToXsRev (s) (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) subToXsRev (I.Dot (_, s)) =
          NONE::subToXsRev (s) (* GEN END FUN BRANCH *)
    ...
    *)
    local
      (* GEN BEGIN TAG OUTSIDE LET *) val caseList : cover_goal list ref = ref nil (* GEN END TAG OUTSIDE LET *)
    in
      fun resetCases () = (caseList := nil)
    ...
    *)
    fun splitVar (cg as CGoal (G, S), k, w) =
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val _ = chatter 6 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => showSplitVar (cg, k) ^ "\n" (* GEN END FUNCTION EXPRESSION *)) (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val s = newEVarSubst (I.Null, G) (* GEN END TAG OUTSIDE LET *) (* for splitting, EVars are always global *)
          (* G = xn:V1,...,x1:Vn *)
          (* s = X1....Xn.^0, where . |- s : G *)
          (* GEN BEGIN TAG OUTSIDE LET *) val X = kthSub (s, k) (* GEN END TAG OUTSIDE LET *) (* starts with k = 1 (a la deBruijn) *)
          (* GEN BEGIN TAG OUTSIDE LET *) val _ = resetCases () (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val _ = splitEVar (X, w, (* GEN BEGIN FUNCTION EXPRESSION *) fn () => addCase (abstractSpine (S, s)) (* GEN END FUNCTION EXPRESSION *)) (* GEN END TAG OUTSIDE LET *)
        in
          SOME (getCases ())
        end
        (* Constraints.Error could be raised by abstract *)
    ...
    *)
    fun finitary (CGoal (G, S), w) =
        let
          ...
          (* GEN BEGIN TAG OUTSIDE LET *) val s = newEVarSubst (I.Null, G) (* GEN END TAG OUTSIDE LET *) (* for splitting, EVars are always global *)
          (* s = X1...Xn.^0,  . |- S : G *)
          (* GEN BEGIN TAG OUTSIDE LET *) val XsRev = subToXsRev (s) (* GEN END TAG OUTSIDE LET *)
          (* XsRev = [SOME(X1),...,SOME(Xn)] *)
        in
          finitarySplits (XsRev, 1, w, (* GEN BEGIN FUNCTION EXPRESSION *) fn () => abstractSpine (S, s) (* GEN END FUNCTION EXPRESSION *), nil)
        end
    ...
    fun contractAll (CGoal(G,S), ucands) =
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val s = newEVarSubst (I.Null, G) (* GEN END TAG OUTSIDE LET *) (* for unif, EVars are always global *)
          (* GEN BEGIN TAG OUTSIDE LET *) val XsRev = subToXsRev (s) (* GEN END TAG OUTSIDE LET *)
        in
          if unifyUOut (XsRev, ucands)
    ...
    fun contract (cg as CGoal (G, S), lab) =
        let
          ...
          (* GEN BEGIN TAG OUTSIDE LET *) val cgOpt' = if n > 0
                         then (contractAll (cg, ucands)
                               handle Constraints.Error _ =>
                                 ( chatter 6 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => "Contraction failed due to constraints\n" (* GEN END FUNCTION EXPRESSION *));
                                  SOME(cg) )) (* no progress if constraints remain *)
                       else SOME(cg) (* GEN END TAG OUTSIDE LET *)    (* no candidates, no progress *)
          (* GEN BEGIN TAG OUTSIDE LET *) val _ = case cgOpt'
                    of NONE => chatter 6 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => "Case impossible: conflicting unique outputs\n" (* GEN END FUNCTION EXPRESSION *))
                     | SOME(cg') => chatter 6 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => showPendingCGoal (cg', lab) ^ "\n" (* GEN END FUNCTION EXPRESSION *)) (* GEN END TAG OUTSIDE LET *)
        in
          cgOpt'
        end
    ...
    *)
    fun cover (cg, w, ccs,  lab, missing) =
        ( chatter 6 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => showPendingCGoal (cg, lab) ^ "\n" (* GEN END FUNCTION EXPRESSION *));
          cover' (contract(cg, lab), w, ccs, lab, missing) )
    and (* GEN BEGIN FUN FIRST *) cover' (SOME(cg), w, ccs, lab, missing) =
        let (* GEN BEGIN TAG OUTSIDE LET *) val cands = match (cg, ccs) (* GEN END TAG OUTSIDE LET *) (* determine splitting candidates *)
            (* GEN BEGIN TAG OUTSIDE LET *) val cand = selectCand cands (* GEN END TAG OUTSIDE LET *) (* select one candidate *)
        in split (cg, cand, w, ccs, lab, missing) end (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) cover' (NONE, w, ccs, lab, missing) =
        (* cg is covered by unique output inconsistency *)
        ( chatter 6 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => "Covered\n" (* GEN END FUNCTION EXPRESSION *));
          missing ) (* GEN END FUN BRANCH *)
    and (* GEN BEGIN FUN FIRST *) split (cg, NONE, w, ccs, lab, missing) =
        (* cg is covered: return missing patterns from other cases *)
        ( chatter 6 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => "Covered\n" (* GEN END FUNCTION EXPRESSION *));
          missing ) (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) split (cg, SOME(nil), w, ccs, lab, missing) =
        (* no strong candidates: check for finitary splitting candidates *)
        ( chatter 6 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => "No strong candidates --- calculating weak candidates\n" (* GEN END FUNCTION EXPRESSION *));
          splitWeak (cg, finitary (cg, w), w, ccs, lab, missing) ) (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) split (cg, SOME((k,_)::ksn), w, ccs, lab, missing) =
        (* some candidates: split first candidate, ignoring multiplicities *)
        (* candidates are in reverse order, so non-index candidates are split first *)
        (* splitVar shows splitting as it happens *)
        ( chatter 6 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => "Splitting on " ^ Int.toString (k) ^ "\n" (* GEN END FUNCTION EXPRESSION *));
          case splitVar (cg, k, w)
            of SOME(cases) => covers (cases, w, ccs, lab, missing)
             | NONE =>
              ( chatter 6 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => "Splitting failed due to generated constraints\n" (* GEN END FUNCTION EXPRESSION *));
                split (cg, SOME(ksn), w, ccs, lab, missing))) (* GEN END FUN BRANCH *)

    and (* GEN BEGIN FUN FIRST *) splitWeak (cg, nil, w, ccs, lab, missing) =
        ( chatter 6 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => "No weak candidates---case " ^ labToString(lab) ^ " not covered\n" (* GEN END FUNCTION EXPRESSION *));
          cg::missing ) (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) splitWeak (cg, ksn, w, ccs, lab, missing) = (* ksn <> nil *)
          split (cg, SOME(findMin ksn::nil), w, ccs, lab, missing) (* GEN END FUN BRANCH *)

    and covers (cases, w, ccs, lab, missing) =
        ( chatter 6 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => "Found " ^ Int.toString (List.length cases)
                     ^ pluralize (List.length cases, " case") ^ "\n" (* GEN END FUNCTION EXPRESSION *));
          covers' (cases, 1, w, ccs, lab, missing) )
    and (* GEN BEGIN FUN FIRST *) covers' (nil, n, w, ccs, lab, missing) =
        ( chatter 6 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => "All subcases of " ^ labToString(lab) ^ " considered\n" (* GEN END FUNCTION EXPRESSION *));
          missing ) (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) covers' (cg::cases', n, w, ccs, lab, missing) =
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val missing1 = cover (cg, w, ccs, Child(lab, n), missing) (* GEN END TAG OUTSIDE LET *)
        in
    ...
    *)
    fun (* GEN BEGIN FUN FIRST *) substToSpine' (I.Shift(n), I.Null, T) = T (* GEN END FUN FIRST *)
      ...
      | (* GEN BEGIN FUN BRANCH *) substToSpine' (I.Dot (_, s) , I.Decl (G, I.BDec (_, (L, t))), T) =
    ...
    *)
    fun (* GEN BEGIN FUN FIRST *) purify' (I.Null) = (I.Null, I.id) (* GEN END FUN FIRST *)
      | (* GEN BEGIN FUN BRANCH *) purify' (I.Decl (G, I.NDec _)) =
        let (* GEN BEGIN TAG OUTSIDE LET *) val (G', s) = purify' G (* GEN END TAG OUTSIDE LET *)
          (* G' |- s : G *)
        in
          (G', I.Dot (I.Undef, s))
          (* G' |- _.s : G,_ *)
        end (* GEN END FUN BRANCH *)
      | (* GEN BEGIN FUN BRANCH *) purify' (I.Decl (G, D as I.Dec _)) =
        let (* GEN BEGIN TAG OUTSIDE LET *) val (G', s) = purify' G (* GEN END TAG OUTSIDE LET *)
          (* G' |- s : G *)
          (* G |- D : type *)
        in
      ...
      *)
      | (* GEN BEGIN FUN BRANCH *) purify' (I.Decl (G, D as I.BDec _)) =
        let (* GEN BEGIN TAG OUTSIDE LET *) val (G', s) = purify' G (* GEN END TAG OUTSIDE LET *)
          (* G' |- s : G *)
        in
          (G', I.Dot (I.Undef, s))
    ...
    *)
    fun coverageCheckCases (w, Cs, G) =
        let
          (* GEN BEGIN TAG OUTSIDE LET *) val _ = chatter 4 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => "[Tomega coverage checker..." (* GEN END FUNCTION EXPRESSION *)) (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val _ = chatter 4 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => "\n" (* GEN END FUNCTION EXPRESSION *)) (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val ccs = List.map ((* GEN BEGIN FUNCTION EXPRESSION *) fn (Gi, si) => CClause (Gi, substToSpine (si, G)) (* GEN END FUNCTION EXPRESSION *)) Cs (* GEN END TAG OUTSIDE LET *)
          (* Question: are all the Gi's above named already? *)
          (* GEN BEGIN TAG OUTSIDE LET *) val _ = chatter 6 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => "[Begin covering clauses]\n" (* GEN END FUNCTION EXPRESSION *)) (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val _ = List.app ((* GEN BEGIN FUNCTION EXPRESSION *) fn cc => chatter 6 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => showCClause cc ^ "\n" (* GEN END FUNCTION EXPRESSION *)) (* GEN END FUNCTION EXPRESSION *)) ccs (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val _ = chatter 6 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => "[End covering clauses]\n" (* GEN END FUNCTION EXPRESSION *)) (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val pureG = purify (G) (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val namedG = N.ctxLUName (pureG) (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val R0 = substToSpine (I.id, namedG) (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val cg0 = CGoal (namedG, R0) (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val missing = cover (cg0, w, ccs, Top, nil) (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val _ = case missing
                    of nil => ()        (* all cases covered *)
                     | _::_ => raise Error ("Coverage error") (* GEN END TAG OUTSIDE LET *)
          (* GEN BEGIN TAG OUTSIDE LET *) val _ = chatter 4 ((* GEN BEGIN FUNCTION EXPRESSION *) fn () => "]\n" (* GEN END FUNCTION EXPRESSION *)) (* GEN END TAG OUTSIDE LET *)
        in
          ()
        end
...
end (* GEN END FUNCTOR DECL *);  (* functor Cover *)
