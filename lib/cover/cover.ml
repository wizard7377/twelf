...
module Cover
  ...
  and abbrevCGoal' = function
    ...
    | G, I.Root (a, S), ci -> I.Root (a, abbrevCSpine S ci)
  ...
  let rec initCGoal' = function
    | a, k, G, I.Pi ((D, P), V) ->
        ...
        (I.Pi ((D', I.Maybe), V'), p)
    | a, k, G, I.Uni I.Type -> (I.Root (a, buildSpine k), k)
  ...
  let rec resolveCands = function
    | Eqns es -> if matchEqns (List.rev es) then Eqns [] else Fail
    | Cands ks -> Cands ks
    | Fail -> Fail
  ...
  let rec addKs = function
    | ccs, CandList klist -> CandList (ccs :: klist)
    | ces, CandList klist -> Covered
    | cfl, CandList klist -> CandList (cfl :: klist)
  ...
  and matchExpW = function
    | G, d, Us1, Us2, cands -> (
        ...
        | I.BVar k1, I.BVar k2 ->
            ...
            else if k1 > d then failAdd (k1 - d, cands)
        ...
        | I.Def d1, I.Def d2 ->
            if d1 = d2 (* because of strictness *) then
    ...
    | G, d, (I.Lam (D1, U1), s1), (I.Lam (D2, U2), s2), cands ->
        matchExp
          ( I.Decl (G, I.decSub (D1, s1)),
            ...
            (U2, I.dot1 s2),
            cands )
    | G, d, (I.Lam (D1, U1), s1), (U2, s2), cands ->
        matchExp
          ( I.Decl (G, I.decSub (D1, s1)),
            d + 1,
            (U1, I.dot1 s1),
            ( I.Redex
              ...
              I.dot1 s2 ),
            cands )
    | G, d, (U1, s1), (I.Lam (D2, U2), s2), cands ->
        matchExp
          ( I.Decl (G, I.decSub (D2, s2)),
            ...
            (U2, I.dot1 s2),
            cands )
    | G, d, Us1, Us2, cands -> addEqn (Eqn (G, Us1, Us2), cands)

  and matchSpine = function
    | G, d, (I.Nil, _), (I.Nil, _), cands -> cands
    | G, d, (I.SClo (S1, s1'), s1), Ss2, cands ->
        matchSpine (G, d, (S1, I.comp (s1', s1)), Ss2, cands)
    | G, d, Ss1, (I.SClo (S2, s2'), s2), cands ->
        matchSpine (G, d, Ss1, (S2, I.comp (s2', s2)), cands)
    | G, d, (I.App (U1, S1), s1), (I.App (U2, S2), s2), cands ->
  ...
  and matchSub = function
    | G, d, I.Shift n1, I.Shift n2, cands -> cands
    | G, d, I.Shift n, s2, cands ->
        matchSub (G, d, I.Dot (I.Idx (n + 1), I.Shift (n + 1)), s2, cands)
    | G, d, s1, I.Shift m, cands ->
        matchSub (G, d, s1, I.Dot (I.Idx (m + 1), I.Shift (m + 1)), cands)
    | G, d, I.Dot (Ft1, s1), I.Dot (Ft2, s2), cands ->
        let cands1 =
          match (Ft1, Ft2) with
          | I.Idx n1, I.Idx n2 ->
              if n1 = n2 then cands
              else if n1 > d then failAdd (n1 - d, cands)
              else
                fail "local variable / local variable clash in block instance"
          | I.Exp U1, I.Exp U2 -> matchExp (G, d, (U1, I.id), (U2, I.id), cands)
          | I.Exp U1, I.Idx n2 ->
              matchExp
  ...
  and matchTopW = function
    | ( G,
        d,
        (I.Root (I.Const c1, S1), s1),
        (I.Root (I.Const c2, S2), s2),
        ci,
        cands ) ->
        if c1 = c2 then
    ...
    | G, d, (I.Pi ((D1, _), V1), s1), (I.Pi ((D2, _), V2), s2), ci, cands ->
        matchTopW
          (I.Decl (G, D1), d + 1, (V1, I.dot1 s1), (V2, I.dot1 s2), ci, cands)

  and matchTopSpine = function
    | G, d, (I.Nil, _), (I.Nil, _), Cnil, cands -> cands
    | G, d, (I.SClo (S1, s1'), s1), Ss2, ci, cands ->
        matchTopSpine (G, d, (S1, I.comp (s1', s1)), Ss2, ci, cands)
    | G, d, Ss1, (I.SClo (S2, s2'), s2), ci, cands ->
        matchTopSpine (G, d, Ss1, (S2, I.comp (s2', s2)), ci, cands)
    | G, d, (I.App (U1, S1), s1), (I.App (U2, S2), s2), Match ci', cands ->
        let cands' = matchExp (G, d, (U1, s1), (U2, s2), cands) in
        matchTopSpine (G, d, (S1, s1), (S2, s2), ci', cands')
    | G, d, (I.App (U1, S1), s1), (I.App (U2, S2), s2), Skip ci', cands ->
        matchTopSpine (G, d, (S1, s1), (S2, s2), ci', cands)
  ...
  let rec matchSig = function
    ...
    | G, ps', V :: ccs', ci, klist ->
        let cands =
          Cs.CSManager.trail (fun () -> matchClause (G, ps', (V, I.id), ci))
        in
  ...
  let rec match_ = function
    | G, V, 0, ci, Input ccs ->
        matchCtx'
          (G, I.id, G, V, 1, ci, matchSig (G, (V, I.id), ccs, ci, CandList []))
    | G, V, 0, ci, Output (V', q) ->
  ...
  let rec lowerSplitW = function
    | X, W, sc ->
        ...
        let sc' =
         fun U ->
          if Unify.unifiable (G, (X, I.id), (U, I.id)) then sc () else ()
        in
        let _ = paramCases (G, (V, I.id), I.ctxLength G, sc') in
        let _ = worldCases (G, (V, I.id), W, sc') in
        let _ = constCases (G, (V, I.id), Index.lookup (I.targetFam V), sc') in
        ()
    | I.Lam (D, U), W, sc -> lowerSplitW (U, W, sc)
  (* splitEVar X W sc = ()
...
(*        val _ = print ("Consider C cases for_sml "  ^ Print.expToString (G, I.EClo Vs) ^ "\n") *)
  ...
  let rec splitVar (V, p, k, (W, ci)) =
    try
      ...
      let _ = resetCases () in
      let _ = splitEVar (X, W, fun () -> addCase (abstract V1 s)) in
      Some (getCases ())
  ...
  let rec occursInMatchPos = function
    | k, I.Pi (DP, V), ci -> occursInMatchPos (k + 1, V, ci)
    | k, I.Root (H, S), ci -> occursInMatchPosSpine (k, S, ci)

  and occursInMatchPosSpine = function
    | k, I.Nil, Cnil -> false
    | k, I.App (U, S), Match ci ->
        occursInExp (k, U) || occursInMatchPosSpine (k, S, ci)
    | k, I.App (U, S), Skip ci -> occursInMatchPosSpine (k, S, ci)
  ...
  and InstEVarsSkipW = function
    ...
    | (I.Pi ((I.Dec (xOpt, V1), _), V2), s), p, XsRev, ci ->
        (* p > 0 *)
        (* all EVars are global *)
        (* was val X1 = I.newEVar (I.Null, I.EClo (V1, s)) (* all EVars are global *)
             Mon Feb 28 15:25:42 2011 --cs *)
        let X1 = Whnf.newLoweredEVar (I.Null, (V1, s)) in
        let EVarOpt = if occursInMatchPos (1, V2, ci) then Some X1 else None in
        instEVarsSkip ((V2, I.Dot (I.Exp X1, s)), p - 1, EVarOpt :: XsRev, ci)
    | (I.Pi ((I.BDec (_, (l, t)), _), V2), s), p, XsRev, ci ->
        (* p > 0 *)
  ...
  let rec recursive = function
    | X ->
        ...
        let recp = List.exists (fun Y -> targetBelowEq (a, Y)) Ys in
        recp
  ...
  let rec finitarySplits = function
    | [], k, W, f, cands -> cands
    | None :: Xs, k, W, f, cands -> finitarySplits (Xs, k + 1, W, f, cands)
    | Some X :: Xs, k, W, f, cands ->
        finitarySplits
          ( Xs,
            ...
            W,
            f,
            Cs.CSManager.trail (fun () -> finitary1 (X, k, W, f, cands)) )
  ...
  let rec finitary (V, p, W, ci) =
    let _ =
      if !Global.doubleCheck then TypeCheck.typeCheck (I.Null, (V, I.Uni I.Type))
      else ()
    in
    let (V1, s), XsRev = instEVarsSkip ((V, I.id), p, [], ci) in
    finitarySplits (XsRev, 1, W, fun () -> (abstract V1 s, []))
  (***********************************)
  ...
  let rec eqInp = function
    ...
    | I.Decl (G', I.Dec (_, I.Root (I.Const a', S'))), k, a, Ss, ms ->
        if a = a' && eqInpSpine (ms, (S', I.Shift k), Ss) then
          k :: eqInp (G', k + 1, a, Ss, ms)
        else eqInp (G', k + 1, a, Ss, ms)
    | I.Decl (G', I.Dec (_, I.Pi _)), k, a, Ss, ms ->
        eqInp (G', k + 1, a, Ss, ms)
    | I.Decl (G', I.NDec _), k, a, Ss, ms -> eqInp (G', k + 1, a, Ss, ms)
  ...
  let rec contractionCands = function
    ...
    | I.Decl (G', I.Dec (_, I.Pi _)), k -> contractionCands (G', k + 1)
    | I.Decl (G', I.NDec _), k -> contractionCands (G', k + 1)
    | I.Decl (G', I.BDec (_, (b, t))), k -> contractionCands (G', k + 1)
  ...
  let rec isolateSplittable = function
    | G, V, 0 -> (G, V)
    | G, I.Pi ((D, _), V'), p -> isolateSplittable (I.Decl (G, D), V', p - 1)
  ...
  and unifyUOutTypeW
      ((I.Root (I.Const a1, S1), s1), (I.Root (I.Const a2, S2), s2)) =
    (* a1 = a2 by invariant *)
  ...
  let rec contractAll V p ucands =
    (* as in splitVar *)
    let (V1, s), XsRev = instEVars ((V, I.id), p, []) in
    if unifyUOut (XsRev, ucands) then Some (abstract V1 s)
  ...
  let rec contract (V, p, ci, lab) =
    ...
    let VpOpt' =
      if n > 0 then (
        ...
        with Constraints.Error _ ->
          chatter 6 (fun () -> "Contraction failed due to constraints\n");
    ...
    let _ =
      match VpOpt' with
      | None ->
          chatter 6 (fun () -> "Case impossible: conflicting unique outputs\n")
      | Some (V', p') ->
          chatter 6 (fun () -> showPendingGoal (V', p', ci, lab) ^ "\n")
    in
    VpOpt'
  (*********************)
  ...
  let rec cover (V, p, wci, ccs, lab, missing) =
    chatter 6 (fun () -> showPendingGoal (V, p, ci, lab) ^ "\n");
    cover' (contract (V, p, ci, lab), wci, ccs, lab, missing)

  and cover' = function
    | Some (V, p), wci, ccs, lab, missing ->
        split
          ( V,
            p,
            selectCand (match_ (I.Null, V, p, ci, ccs)),
            wci,
            ccs,
            lab,
            missing )
  ...
  and covers (cases, wci, ccs, lab, missing) =
    chatter 6 (fun () ->
        "Found "
        ^ Int.toString (List.length cases)
  ...
  let rec createCoverClause = function
    | I.Decl (G, D), V, p -> createCoverClause (G, I.Pi ((D, I.Maybe), V), p + 1)
  ...
  and createCoverSpine = function
    ...
    | ( G,
        ...
        in
        I.App (X, S2')
    | G, (I.App (U1, S2), s), (I.Pi (_, V2), s'), M.Mapp (_, ms') ->
        I.App
          ( I.EClo (U1, s),
            createCoverSpine
              ( G,
  ...
  let rec formatCGoal (CGoal (G, S)) =
    let _ = N.varReset I.Null in
    F.HVbox
      ([ Print.formatCtx (I.Null, G); F.Break; F.Break; F.String "|-"; F.Space ]
  ...
  let rec checkConstraints = function
    ...
    | G, (Si, ti), Eqns _ -> (
        ...
        let constrs = collectConstraints Xs in
        match constrs with [] -> Eqns [] | _ -> fail "Remaining constraints"
  ...
  let rec matchClauses = function
    | cg, [], klist -> klist
    | cg, CClause (Gi, Si) :: ccs, klist ->
        (* G |- ti : Gi *)
        let ti = newEVarSubst (G, Gi) in
        let cands = Cs.CSManager.trail (fun () -> matchClause (cg, (Si, ti))) in
        matchClauses' (cg, ccs, addKs (cands, klist))

  and matchClauses' = function
    | cg, ccs, Covered -> Covered
    | cg, ccs, klist -> matchClauses (cg, ccs, klist)
  (* match (cg, ccs) = klist
       matching coverage goal cg against coverage clauses ccs
       yields candidates klist
    *)

  let rec match_ (CGoal (G, S), ccs) =
  ...
  let rec splitVar cg k w =
    try
      ...
      let X = kthSub (s, k) in
      let _ = resetCases () in
      let _ = splitEVar (X, w, fun () -> addCase (abstractSpine S s)) in
      Some (getCases ())
      (* Constraints.Error could be raised by abstract *)
    with Constraints.Error constrs ->
      chatter 7 (fun () ->
  ...
  let rec finitary (CGoal (G, S), w) =
    ...
    finitarySplits (XsRev, 1, w, fun () -> (abstractSpine S s, []))
  (***************)
  ...
  let rec contractAll (CGoal (G, S), ucands) =
    (* for_sml unif, EVars are always global *)
    let s = newEVarSubst (I.Null, G) in
    let XsRev = subToXsRev s in
    if unifyUOut (XsRev, ucands) then Some (abstractSpine S s)
      (* as in splitVar, may raise Constraints.Error *)
    else None

  let rec contract cg lab =
    (* no candidates, no progress *)
    let ucands = contractionCands (G, 1) in
    let n = List.length ucands in
    let _ =
  ...
  let rec substToSpine' = function
    ...
    | I.Dot (_, s), I.Decl (G, I.NDec _), T -> substToSpine' (s, G, T)
    | I.Dot (I.Exp U, s), I.Decl (G, V), T -> substToSpine' (s, G, I.App (U, T))
    | I.Dot (I.Idx n, s), I.Decl (G, I.Dec (_, V)), T ->
        let Us, _ =
          Whnf.whnfEta ((I.Root (I.BVar n, I.Nil), I.id), (V, I.id))
        in
        substToSpine' (s, G, I.App (I.EClo Us, T))
    | I.Dot (_, s), I.Decl (G, I.BDec (_, (L, t))), T -> substToSpine' (s, G, T)
