...
module NetServer
  ...
  let rec vec2str v =
    String.implode (map (Char.chr o Word8.toInt) (Word8Vector.foldr :: [] v))
  ...
  let rec getNat = function
    | t :: [] -> (
        ...
        with Lexer.NotDigit char ->
          error (quote t ^ " is not a natural number"))
  ...
  let rec flashProto () =
    ...
    let rec recv (u : server) s =
      ...
      let _ = app (exec u) (rev cmds) in
      buf := rem
    in
    let rec send (u : server) s = (send u) (s ^ "\000") in
    {
  ...
  let rec humanProto () =
    ...
    let rec recv (u : server) s =
      ...
      let _ = app (exec u) (map stripcr (rev cmds)) in
      buf := rem
    in
    let rec send (u : server) s = (send u) ("> " ^ s) in
    {
      init = (fun () -> Msg.message (Twelf.version ^ "\n%%% OK %%%\n"));
      reset = (fun () -> buf := "");
      send;
  ...
  let rec httpProto dir =
    ...
    let rec handleRequest (u : server) =
      if !method_ = "GET" then handleGetRequest u
      else if !method_ = "POST" then handlePostRequest u
      else send u "HTTP/1.1 500 Server Error\n\n"
    in
    let rec headerExec s = headers := s :: !headers in
    let rec recvContent u =
      if size !ibuf >= !contentLength then handleRequest u else ()
    in
    let rec parseHeaders () =
      try
        let (request :: headers) = rev !headers in
        let [ m; u; httpVersion ] = String.tokens (fun x -> x = ' ') request in
        let _ = method_ := m in
        let _ = url := u in
        let rec getField s =
    ...
    let rec recv (u : server) s =
      ibuf := !ibuf ^ s;
      if !parsingHeaders then interp u (String.fields isnewl !ibuf)
      else recvContent u
    in
    ...
    let rec reset () =
      ...
      method_ := ""
    in
    { init = (fun () -> ()); reset; send; recv; done_ = (fun () -> ()) }
  ...
  let rec protoServer (proto : protocol) portNum =
    ...
    let rec read_one conn u () =
      ...
      if Word8Vector.length v = 0 then raise EOF else (recv proto) u (vec2str v)
    in
    let rec accept_one () =
      let conn, addr = S.accept sock in
      let _ = (reset proto) () in
      let u = { send = send conn; exec = exec conn } in
      let _ = Msg.setMessageFunc ((send proto) u) in
      let _ = (init proto) () in
