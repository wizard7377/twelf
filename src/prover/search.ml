Search  Global GLOBAL    State' STATE    Abstract ABSTRACT    Data DATA    CompSyn' COMPSYN    Whnf WHNF    Unify UNIFY    Assign ASSIGN    Index INDEX    Compile COMPILE    CPrint CPRINT    Print PRINT    Names NAMES    CSManager CS_MANAGER     SEARCH  struct (*! structure IntSyn = IntSyn' !*)
(*! structure Tomega = Tomega' !*)
module (*! structure CompSyn = CompSyn' !*)
exception module module (* isInstantiated (V) = SOME(cid) or NONE
...
let rec solve(max,  , depth,  , (atom p,  , s),  , dp,  , sc) matchAtom (max,  , depth,  , (p,  , s),  , dp,  , sc) solve(max,  , depth,  , (impl (r,  , a,  , ha,  , g),  , s),  , dProg (g,  , dPool),  , sc) let let  in in solve (max,  , depth + 1,  , (g,  , dot1 s),  , dProg (decl (g,  , d'),  , decl (dPool,  , dec (r,  , s,  , ha))),  , (fun m -> sc (lam (d',  , m)))) solve(max,  , depth,  , (all (d,  , g),  , s),  , dProg (g,  , dPool),  , sc) let let  in in solve (max,  , depth + 1,  , (g,  , dot1 s),  , dProg (decl (g,  , d'),  , decl (dPool,  , parameter)),  , (fun m -> sc (lam (d',  , m))))(* rsolve (max, depth, (p,s'), (r,s), (G,dPool), sc, (acc, k)) = ()
  ...
  *)
 rSolve(max,  , depth,  , ps',  , (eq q,  , s),  , dProg (g,  , dPool),  , sc) if unifiable (g,  , ps',  , (q,  , s)) then sc nil else () rSolve(max,  , depth,  , ps',  , (assign (q,  , eqns),  , s),  , dp as dProg (g,  , dPool),  , sc) (match assignable (g,  , ps',  , (q,  , s)) with sOME (cnstr) -> aSolve ((eqns,  , s),  , dp,  , cnstr,  , (fun () -> sc nil)) nONE -> ()) rSolve(max,  , depth,  , ps',  , (and (r,  , a,  , g),  , s),  , dp as dProg (g,  , dPool),  , sc) let (* is this EVar redundant? -fp *)
let  in in rSolve (max,  , depth,  , ps',  , (r,  , dot (exp (x),  , s)),  , dp,  , (fun s -> solve (max,  , depth,  , (g,  , s),  , dp,  , (fun m -> sc (app (m,  , s)))))) rSolve(max,  , depth,  , ps',  , (in (r,  , a,  , g),  , s),  , dp as dProg (g,  , dPool),  , sc) let (* G |- g goal *)
(* G |- A : type *)
(* G, A |- r resgoal *)
(* G0, Gl  |- s : G *)
...
(* aSolve ((ag, s), dp, sc) = res
 ...
 aSolve((trivial,  , s),  , dp,  , cnstr,  , sc) (if solveCnstr cnstr then sc () else ()) aSolve((unifyEq (g',  , e1,  , n,  , eqns),  , s),  , dp as dProg (g,  , dPool),  , cnstr,  , sc) let let  inlet  in in if unifiable (g'',  , (n,  , s'),  , (e1,  , s')) then aSolve ((eqns,  , s),  , dp,  , cnstr,  , sc) else ()(* matchatom ((p, s), (G, dPool), sc, (acc, k)) => ()
     G |- s : G'
     G' |- p :: atom
     G ~ dPool
     acc is the accumulator of results
  ...
  *)
 matchAtom(0,  , _,  , _,  , _,  , _) () matchAtom(max,  , depth,  , ps' as (root (ha,  , _),  , _),  , dp as dProg (g,  , dPool),  , sc) let let rec matchSig'nil () matchSig'(hc :: sgn') let let  inlet  in in matchSig' sgn'let rec matchBlock(nil,  , (n,  , i)) () matchBlock((r,  , s,  , h') :: rGs',  , (n,  , i)) if eqHead (ha,  , h') then let let  in in matchBlock (rGs',  , (n,  , i + 1)) else matchBlock (rGs',  , (n,  , i + 1))let rec matchDProg(null,  , _) matchSig' (lookup (cidFromHead ha)) matchDProg(decl (dPool',  , dec (r,  , s,  , ha')),  , n) if eqHead (ha,  , ha') then let let  in in matchDProg (dPool',  , n + 1) else matchDProg (dPool',  , n + 1) matchDProg(decl (dPool',  , parameter),  , n) matchDProg (dPool',  , n + 1) matchDProg(decl (dPool',  , bDec rGs),  , n) (matchBlock (rGs,  , (n,  , 1)); matchDProg (dPool',  , n + 1)) matchDProg(decl (dPool',  , pDec),  , n) matchDProg (dPool',  , n + 1) in matchDProg (dPool,  , 1)(* searchEx' max (GE, sc) = acc'
...
(* contexts of EVars are recompiled for each search depth *)
 searchEx'max(nil,  , sc) sc max searchEx'max((x as eVar (r,  , g,  , v,  , _)) :: gE,  , sc) solve (max,  , 0,  , (compileGoal (g,  , v),  , id),  , compileCtx false g,  , (fun u' -> try  with ))(* deepen (f, P) = R'
    ...
    *)
let rec deependepthfp let let rec deepen'level if level > depth then () else (if ! chatter > 5 then print "#" else (); (f level p; deepen' (level + 1))) in deepen' 1(* searchEx (G, GE, (V, s), sc) = acc'
       Invariant:
       If   G |- s : G'   G' |- V : level
...
let rec search(maxFill,  , gE,  , sc) searchEx (1,  , maxFill) (gE,  , sc)let  in(* local ... *)
end