...
struct
    ...
    *)
    fun eta_contract_var (Elt t) = eta_contract_var' 0 t
      | (* GEN CASE BRANCH *) eta_contract_var _ = raise EtaContract
    and eta_contract_var' n (ATerm(ARoot(Var n', s))) = 
    	let
    	    val s' = map eta_contract_var s
    	    fun decreasing_list 0 [] = true
    	      | (* GEN CASE BRANCH *) decreasing_list n (h::tl) = (n - 1 = h) andalso decreasing_list (n-1) tl
    	      | (* GEN CASE BRANCH *) decreasing_list _ _ = false
    	in
    	    if decreasing_list n s' then n' - n else raise EtaContract
    	end
      | (* GEN CASE BRANCH *) eta_contract_var' n (NTerm(Lam t)) = eta_contract_var' (n+1) t
      | (* GEN CASE BRANCH *) eta_contract_var' _ _ = raise EtaContract

    fun pattern_spine' (D, []) = true
      | (* GEN CASE BRANCH *) pattern_spine' (D, n::s) = let fun isn x = (x = n)
    ...
    fun spine_occ n (D, nil) = false
      | (* GEN CASE BRANCH *) spine_occ n (D, (Elt t)::s) = term_occ n (D, t) orelse
      				      spine_occ n (D, s)
      | (* GEN CASE BRANCH *) spine_occ n (D, (AElt t)::s) = aterm_occ n (D, t) orelse
      				       spine_occ n (D, s)
      | (* GEN CASE BRANCH *) spine_occ n (D, (Ascribe (t,a))::s) = nterm_occ n (D, t) orelse
      					      type_occ n (D, a) orelse
      					      spine_occ n (D, s)
      | (* GEN CASE BRANCH *) spine_occ n (D, Omit::s) = false (* Omit invalidates all strict
					    occurrences to the right *)

    and term_occ n (D, NTerm t) = nterm_occ n (D, t)
      | (* GEN CASE BRANCH *) term_occ n (D, ATerm t) = aterm_occ n (D, t)

    (* PERF: suspend these context shifts, do them at the end *)
    and nterm_occ n (D, Lam t) = term_occ (n+1) (0 :: (map (fn x => x+1) D), t)
      | (* GEN CASE BRANCH *) nterm_occ n (D, NRoot (h,s)) = root_occ n (D, h, s)

    and aterm_occ n (D, ARoot (h,s)) = root_occ n (D, h, s)
      | (* GEN CASE BRANCH *) aterm_occ n (D, ERoot _) = false
    ...
    and root_occ n (D, Var n', s) = if n = n' (* n = n' precludes n in D, right? *)
      ...
      | (* GEN CASE BRANCH *) root_occ n (D, Const n', s) = spine_occ n (D, s)

    and type_occ n (D, TRoot (_,s)) = spine_occ n (D, s)
      | (* GEN CASE BRANCH *) type_occ n (D, TPi (_, a, b)) = type_occ n (D, a) orelse
          (* PERF: suspend these context shifts, do them at the end *)
    ...
    fun check_strict_type' n p (TRoot(n', s)) = if p then false else spine_occ n ([], s)
      | (* GEN CASE BRANCH *) check_strict_type' n p (TPi(PLUS,a,b)) = type_occ n ([],a) orelse
      					       check_strict_type' (n+1) p b
      | (* GEN CASE BRANCH *) check_strict_type' n p (TPi(_,a,b)) = check_strict_type' (n+1) p b

    fun check_strict_kind' n Type = false
      | (* GEN CASE BRANCH *) check_strict_kind' n (KPi(PLUS,a,k)) = type_occ n ([],a) orelse
      					       check_strict_kind' (n+1) k
      | (* GEN CASE BRANCH *) check_strict_kind' n (KPi(_,a,k)) = check_strict_kind' (n+1) k
