  ...
  in
    ...
    fun foldl f a t =
        let fun fo (Empty, r) = r
              | (* GEN CASE BRANCH *) fo (Red (x, left, right), r) =
                  	          fo (right, f (x, fo (left, r)))
              | (* GEN CASE BRANCH *) fo (Black (x, left, right), r) =
                  		  fo (right, f (x, fo (left, r)))
    	in
    	  fo (t, a)
    	end
