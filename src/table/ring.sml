...
struct
  ...
  *)
  fun current (nil, nil) = raise Empty
    | (* GEN CASE BRANCH *) current (_, x :: _) = x
    | (* GEN CASE BRANCH *) current (l, nil) = current (nil, rev l)

  (* next [] = raise Empty
     next [a1, a2 ... an]) = [a2 ... an, a1]
  *)
  fun next (nil, nil) = raise Empty
    | (* GEN CASE BRANCH *) next (r, nil) = next (nil, rev r)
    | (* GEN CASE BRANCH *) next (r, x :: l) = (x :: r, l)

  (* previous [] = ERROR
     previous [a1, a2 ... an]) = [a2 ... an, a1]
  *)
  fun previous (nil, nil) = raise Empty
    | (* GEN CASE BRANCH *) previous (nil, l) = previous (rev l, nil)
    | (* GEN CASE BRANCH *) previous (x :: r, l) = (r, x :: l)

  (* delete [] = raise Empty
     delete [a1, a2 ... an] = [a2 ... an]
  *)
  fun delete (nil, nil) = raise Empty
    | (* GEN CASE BRANCH *) delete (r, nil) = delete (nil, rev r) 
    | (* GEN CASE BRANCH *) delete (r, x :: l) = (r, l)
