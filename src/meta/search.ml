MTPSearch  Global GLOBAL    Abstract ABSTRACT    MTPGlobal MTPGLOBAL    StateSyn' STATESYN    Whnf WHNF    Unify UNIFY    Assign ASSIGN    Index INDEX    Compile COMPILE    CPrint CPRINT    Print PRINT    Names NAMES     MTPSEARCH  struct (*! structure IntSyn = IntSyn' !*)
...
(* aSolve ((ag, s), dp, sc) = res
  ...
  *)
 matchAtom(0,  , _,  , _,  , _,  , _) () matchAtom(max,  , depth,  , ps' as (root (ha,  , _),  , _),  , dp as dProg (g,  , dPool),  , sc) let let rec matchSig'nil () matchSig'(hc :: sgn') let let  inlet  in in matchSig' sgn'let rec matchDProg(null,  , _) matchSig' (lookup (cidFromHead ha)) matchDProg(decl (dPool',  , dec (r,  , s,  , ha')),  , n) if eqHead (ha,  , ha') then let let  in in matchDProg (dPool',  , n + 1) else matchDProg (dPool',  , n + 1) matchDProg(decl (dPool',  , parameter),  , n) matchDProg (dPool',  , n + 1) in matchDProg (dPool,  , 1)(* searchEx' max (GE, sc) = acc'
