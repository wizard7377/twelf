  ...
  local
    ...
    *)
    fun patSpine (_, I.Nil) = true
      | (* GEN CASE BRANCH *) patSpine (k, I.App (I.Root (I.BVar (k'), I.Nil), S)) =
      ...
      | (* GEN CASE BRANCH *) patSpine _ = false
    ...
    *)
    fun strictExp (_, _, I.Uni _) = false
      | (* GEN CASE BRANCH *) strictExp (k, p, I.Lam (D, U)) =
          (* checking D in this case might be redundant -fp *)
          strictDec (k, p, D) orelse strictExp (k+1, p+1, U)
      | (* GEN CASE BRANCH *) strictExp (k, p, I.Pi ((D, _), U)) =
          strictDec (k, p, D) orelse strictExp (k+1, p+1, U)
      | (* GEN CASE BRANCH *) strictExp (k, p, I.Root (H, S)) =
          (case H
             of (I.BVar (k')) =>
      ...
      | (* GEN CASE BRANCH *) strictExp (k, p, I.FgnExp (cs, ops)) = false
    ...
    *)
    and strictSpine (_, _, I.Nil) = false
      | (* GEN CASE BRANCH *) strictSpine (k, p, I.App (U, S)) =
    ...
    *)
    fun strictArgParm (p, U as I.Root _) = strictExp (0, p, U)
      | (* GEN CASE BRANCH *) strictArgParm (p, U as I.Pi _) = strictExp (0, p, U)
      | (* GEN CASE BRANCH *) strictArgParm (p, U as I.FgnExp _) = strictExp (0, p, U)
      | (* GEN CASE BRANCH *) strictArgParm (p, I.Lam (D, U)) = strictArgParm (p+1, U)

    fun occToString (SOME(ocd), occ) = Paths.wrap (Paths.occToRegionDef1 ocd occ, "")
      | (* GEN CASE BRANCH *) occToString (NONE, occ) = "Error: "

    fun decToVarName (I.Dec (NONE, _)) = "implicit variable"
      | (* GEN CASE BRANCH *) decToVarName (I.Dec (SOME(x), _)) = "variable " ^ x
    ...
    *)
    fun strictTop ((U, V), ocdOpt) =
        let fun strictArgParms (I.Root (I.BVar _, _), _, occ) =
              ...
              | (* GEN CASE BRANCH *) strictArgParms (I.Root _, _, _) = ()
              | (* GEN CASE BRANCH *) strictArgParms (I.Pi _, _, _) = ()
              | (* GEN CASE BRANCH *) strictArgParms (I.FgnExp _, _, _) = ()
                  (* may not be sound in general *)
                  (* Wed Aug 25 16:39:57 2004 -fp *)
              | (* GEN CASE BRANCH *) strictArgParms (I.Lam (D, U'), I.Pi (_, V'), occ) =
              ...
              | (* GEN CASE BRANCH *) strictArgParms (U as I.Lam _, V as I.Root (I.Def _, _), occ) =
   ...
   fun occursInType ((i, V), ocdOpt) =
       let fun oit ((0, V), occ) = ()
             | (* GEN CASE BRANCH *) oit ((i, I.Pi((D,P), V)), occ) =
