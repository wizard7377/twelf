  ...
  local
    ...
    *)
    fun matchExpW (G, Us1 as (FgnExp csfe1, _), Us2) =
      ...
      | (* GEN CASE BRANCH *) matchExpW (G, Us1, Us2 as (FgnExp csfe2, _)) =
      ...
      | (* GEN CASE BRANCH *) matchExpW (G, (Uni (L1), _), (Uni(L2), _)) =
          (* L1 = L2 = type, by invariant *)
          (* matchUni (L1, L2) - removed Mon Aug 24 12:18:24 1998 -fp *)
          ()

      | (* GEN CASE BRANCH *) matchExpW (G, Us1 as (Root (H1, S1), s1), Us2 as (Root (H2, S2), s2)) =
          (* s1 = s2 = id by whnf *)
      ...
      | (* GEN CASE BRANCH *) matchExpW (G, (Pi ((D1, _), U1), s1), (Pi ((D2, _), U2), s2)) =
          (matchDec (G, (D1, s1), (D2, s2)) ;
           matchExp (Decl (G, decSub (D1, s1)), (U1, dot1 s1), (U2, dot1 s2)))

      | (* GEN CASE BRANCH *) matchExpW (G, Us1 as (Pi (_, _), _), Us2 as (Root (Def _, _), _)) =
          matchExpW (G, Us1, Whnf.expandDef (Us2))
      | (* GEN CASE BRANCH *) matchExpW (G, Us1 as  (Root (Def _, _), _), Us2 as (Pi (_, _), _)) =
          matchExpW (G, Whnf.expandDef (Us1), Us2)

      | (* GEN CASE BRANCH *) matchExpW (G, (Lam (D1, U1), s1), (Lam (D2, U2), s2)) =
          (* D1[s1] = D2[s2]  by invariant *)
          matchExp (Decl (G, decSub (D1, s1)), (U1, dot1 s1),(U2, dot1 s2))

      | (* GEN CASE BRANCH *) matchExpW (G, (Lam (D1, U1), s1), (U2, s2)) =
      ...
      | (* GEN CASE BRANCH *) matchExpW (G, (U1, s1), (Lam (D2, U2), s2)) =
...
*)
      | (* GEN CASE BRANCH *) matchExpW (G, Us1 as (EVar(r, GX, V, cnstrs), s), Us2 as (U2,s2)) =
...
(*      | matchExpW (G, Us1 as (U1,s1), Us2 as (EVar (r, GX, V, cnstrs), s)) =
      ...
      | (* GEN CASE BRANCH *) matchExpW (G, Us1, Us2) =
    ...
    *)
    and matchSpine (G, (Nil,_), (Nil,_)) = ()
      | (* GEN CASE BRANCH *) matchSpine (G, (SClo (S1, s1'), s1), Ss) = matchSpine (G, (S1, comp (s1', s1)), Ss)
      | (* GEN CASE BRANCH *) matchSpine (G, Ss, (SClo (S2, s2'), s2)) = matchSpine (G, Ss, (S2, comp (s2', s2)))
      | (* GEN CASE BRANCH *) matchSpine (G, (App (U1, S1), s1), (App (U2, S2), s2)) =
        (matchExp (G, (U1, s1), (U2, s2)) ;
         matchSpine (G, (S1, s1), (S2, s2)))
    ...
    *)
    ...
    and matchSub (G, Shift (n1), Shift (n2)) = ()
         (* by invariant *)
      | (* GEN CASE BRANCH *) matchSub (G, Shift(n), s2 as Dot _) =
          matchSub (G, Dot(Idx(n+1), Shift(n+1)), s2)
      | (* GEN CASE BRANCH *) matchSub (G, s1 as Dot _, Shift(m)) =
          matchSub (G, s1, Dot(Idx(m+1), Shift(m+1)))
      | (* GEN CASE BRANCH *) matchSub (G, Dot(Ft1,s1), Dot(Ft2,s2)) =
          ((case (Ft1, Ft2) of
             (Idx (n1), Idx (n2)) =>
    ...
    and matchBlock (G, LVar (ref (SOME(B1)), s, _), B2) = matchBlock (G, blockSub (B1, s), B2)
      | (* GEN CASE BRANCH *) matchBlock (G, B1, LVar (ref (SOME(B2)), s, _)) = matchBlock (G, B1, blockSub (B2, s))
      | (* GEN CASE BRANCH *) matchBlock (G, B1, B2) = matchBlockW (G, B1, B2)
    ...
    and matchBlockW (G, LVar (r1, Shift(k1), (l1, t1)), LVar (r2, Shift(k2), (l2, t2))) =
      ...
      | (* GEN CASE BRANCH *) matchBlockW (G, LVar (r1, s1, (l1, t1)),  B2) =
            (r1 := SOME (blockSub (B2, Whnf.invert s1)) ; ()) (* -- ABP *)

      | (* GEN CASE BRANCH *) matchBlockW (G,  B1, LVar (r2, s2, (l2, t2))) =
...
*)
      ...
      | (* GEN CASE BRANCH *) matchBlockW (G, Bidx (n1), (Bidx (n2))) =
         if n1 <> n2
...
*)
    ...
    and awakeCnstr (NONE) = ()
      | (* GEN CASE BRANCH *) awakeCnstr (SOME(ref Solved)) = awakeCnstr (Unify.nextCnstr ())
