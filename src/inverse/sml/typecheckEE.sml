...
struct 
  ...
  fun check_exp (ctx,Uni Type,Uni Kind) = ()
    | (* GEN CASE BRANCH *) check_exp (ctx,Lam {body=M,...}, Pi {var,arg=U,body=V,...}) =
      check_exp (C.push (ctx, (var, U)), M, V)
    | (* GEN CASE BRANCH *) check_exp (ctx,Root(Const con,S), V) = 
      let 
    ...
    | (* GEN CASE BRANCH *) check_exp (ctx, Root(BVar i,S), V) = 
    ...
    | (* GEN CASE BRANCH *) check_exp (ctx, Pi {var,arg=A1,body=A2,...}, uni as Uni _) = 
      (check_exp (ctx, A1, expType);
       check_exp (C.push (ctx, (var, A1)), A2, uni))
    | (* GEN CASE BRANCH *) check_exp _ = raise Fail "check: bad case"

  and focus (ctx, Nil, ty as Uni Type) = ty
    | (* GEN CASE BRANCH *) focus (ctx, Nil, hd as Root (Const _,_)) = hd
    | (* GEN CASE BRANCH *) focus (ctx ,App(M,S), Pi {arg=A1,body=A2,...}) =
      (check_exp (ctx, M, A1);
       focus (ctx, S, apply_exp (Dot(M,id_sub), A2)))
    | (* GEN CASE BRANCH *) focus (_,S,E) = raise Fail_spine_exp("focus: bad case",S,E)
  ...
  and apply_exp (_, uni as Uni _) = uni
    | (* GEN CASE BRANCH *) apply_exp (sub, Pi {var,arg=U,depend,body=V}) =
      Pi {var = var,
          arg = apply_exp(sub, U),
          depend = depend,
          body = apply_exp (push_sub sub, V)}
    | (* GEN CASE BRANCH *) apply_exp (sub, Lam {var,body=U}) =
      Lam {var=var,
           body=apply_exp (push_sub sub, U)}
    | (* GEN CASE BRANCH *) apply_exp (sub, exp as Root(H,S)) =
      let
  ...
  and apply_spine (_, Nil) = Nil
    | (* GEN CASE BRANCH *) apply_spine (sub, App(M,S)) = App(apply_exp (sub, M),apply_spine (sub, S))
  ...
  and apply_var (Dot(M,sub), i) =
    ...
    | (* GEN CASE BRANCH *) apply_var (Shift n, i) = RetVar (i+n)

  and compose (Dot(M,sigma),sigma') = Dot(apply_exp (sigma', M),compose(sigma,sigma'))
    | (* GEN CASE BRANCH *) compose (Shift n,Shift m) = Shift (n + m)
    | (* GEN CASE BRANCH *) compose (Shift 0,sigma) = sigma
    | (* GEN CASE BRANCH *) compose (Shift n,Dot(M,sigma)) = compose (Shift (n-1),sigma)
  ...
  and reduce (exp as Root(_,_), Nil) = exp
    | (* GEN CASE BRANCH *) reduce (Lam {body=M,...}, App(M',S)) =
      reduce (apply_exp (Dot(M',id_sub), M), S)
    | (* GEN CASE BRANCH *) reduce (E, S) = raise Fail_exp_spine ("reduce: bad case: head: ",E,S)
  ...
  and equiv_exp (Uni u1, Uni u2) = u1 = u2
    | (* GEN CASE BRANCH *) equiv_exp (Pi {arg=U1,body=V1,...}, Pi {arg=U2,body=V2,...}) =
      equiv_exp (U1, U2) andalso equiv_exp (V1, V2)
    | (* GEN CASE BRANCH *) equiv_exp (Lam {body=U,...}, Lam {body=U',...}) =
      equiv_exp (U, U')
    | (* GEN CASE BRANCH *) equiv_exp (Root(BVar i,S1), Root(BVar i',S2)) =
      i = i' andalso equiv_spine (S1, S2)
    | (* GEN CASE BRANCH *) equiv_exp (exp as Root(Const c,S), exp' as Root(Const c',S')) =
    ...
    | (* GEN CASE BRANCH *) equiv_exp _ = false

  and equiv_spine (S.Nil, Nil) = true
    | (* GEN CASE BRANCH *) equiv_spine (S.App(E,S), S.App(E',S')) =
      equiv_exp (E ,E') andalso equiv_spine (S, S')
    | (* GEN CASE BRANCH *) equiv_spine _ = false
  ...
  fun check_dec (c,Decl {id,name,exp,uni}) = 
    ...
    | (* GEN CASE BRANCH *) check_dec (c,Def {id,name,exp,def,height,root,uni}) =
      let
        val uni' = Uni uni
        val exp' = eta_expand(exp,uni')
    ...
    | (* GEN CASE BRANCH *) check_dec (c,Abbrev {id,name,exp,def,uni}) =
      let
        val uni' = Uni uni
    (*         val exp' = eta_expand(exp,uni') *)
    (*         val def' = eta_expand(def,exp') *)
