...
struct 
  ...
  fun check_exp sgn ctx (Uni Type) (Uni Kind) = ()
    | (* GEN CASE BRANCH *) check_exp sgn ctx (Lam {body=M,...}) (Pi {var,arg=U,body=V,...}) =
      check_exp sgn (C.push ctx (var,U)) M V
    | (* GEN CASE BRANCH *) check_exp sgn ctx (Root(Const con,S)) V = 
      let 
    ...
    | (* GEN CASE BRANCH *) check_exp sgn ctx (Root(BVar i,S)) V = 
    ...
    | (* GEN CASE BRANCH *) check_exp sgn ctx (Pi {var,arg=A1,body=A2,...}) (uni as Uni _) = 
      (check_exp sgn ctx A1 expType;
       check_exp sgn (C.push ctx (var,A1)) A2 uni)
    | (* GEN CASE BRANCH *) check_exp _ _ _ _ = raise Check "check: bad case"

  and focus sgn ctx Nil (ty as Uni Type) = ty
    | (* GEN CASE BRANCH *) focus sgn ctx Nil (hd as Root (Const _,_)) = hd
    | (* GEN CASE BRANCH *) focus sgn ctx (App(M,S)) (Pi {arg=A1,body=A2,...}) =
      (check_exp sgn ctx M A1;
       focus sgn ctx S (apply_exp (Dot(M,id_sub)) A2))
    | (* GEN CASE BRANCH *) focus _ _ S E = raise Fail_spine_exp("focus: bad case",S,E)
  ...
  and apply_var (Dot(M,sub)) i =
    ...
    | (* GEN CASE BRANCH *) apply_var (Comp(s1,s2)) i = 
  ...
  and equiv_exp sgn (Uni u1) (Uni u2) = u1 = u2
    | (* GEN CASE BRANCH *) equiv_exp sgn (Pi {arg=U1,body=V1,...}) (Pi {arg=U2,body=V2,...}) =
      equiv_exp sgn U1 U2 andalso equiv_exp sgn V1 V2
    | (* GEN CASE BRANCH *) equiv_exp sgn (Lam {body=U,...}) (Lam {body=U',...}) =
      equiv_exp sgn U U'
    | (* GEN CASE BRANCH *) equiv_exp sgn (Root(BVar i,S1)) (Root(BVar i',S2)) =
      i = i' andalso equiv_spine sgn S1 S2
    | (* GEN CASE BRANCH *) equiv_exp sgn (exp as Root(Const c,S)) (exp' as Root(Const c',S')) =
    ...
    | (* GEN CASE BRANCH *) equiv_exp _ _ _ = false

  and equiv_spine sgn Nil Nil = true
    | (* GEN CASE BRANCH *) equiv_spine sgn (App(E,S)) (App(E',S')) =
      equiv_exp sgn E E' andalso equiv_spine sgn S S'
    | (* GEN CASE BRANCH *) equiv_spine _ _ _ = false
