CSEqIntegers  Integers INTEGERS    Whnf WHNF    Unify UNIFY     CS_EQ_INTEGERS  struct (*! structure CSManager = CSManager !*)
...
(* A monomial (n * U1[s1] * U2[s2] * ...) is said to be normal iff
  ...
  *)
open IntSyn open Integers module module (* CSManager.ModeSyn *)
exception let rec extractSum(myIntsynRep sum) sum extractSumfe raise ((unexpectedFgnExp fe))let  inlet  inlet  inlet  inlet rec number() root (const (! numberID),  , nil)let  inlet  inlet  inlet  inlet rec unaryMinusExp(u) root (const (! unaryMinusID),  , app (u,  , nil))let rec plusExp(u,  , v) root (const (! plusID),  , app (u,  , app (v,  , nil)))let rec minusExp(u,  , v) root (const (! minusID),  , app (u,  , app (v,  , nil)))let rec timesExp(u,  , v) root (const (! timesID),  , app (u,  , app (v,  , nil)))let rec numberConDec(d) conDec (toString (d),  , nONE,  , 0,  , normal,  , number (),  , type)let rec numberExp(d) root (fgnConst (! myID,  , numberConDec (d)),  , nil)(* parseNumber str = SOME(conDec) or NONE
...
let rec minusSum(sum1,  , sum2) plusSum (sum1,  , unaryMinusSum (sum2))(* fromExpW (U, s) = sum
       ...
       and sum is normal
    *)
let rec fromExpW(us as (fgnExp (cs,  , fe),  , _)) if (cs = ! myID) then normalizeSum (extractSum fe) else sum (zero,  , [mon (one,  , [us])]) fromExpW(us as (root (fgnConst (cs,  , conDec),  , _),  , _)) if (cs = ! myID) then (match (fromString (conDecName (conDec))) with sOME (m) -> sum (m,  , nil)) else sum (zero,  , [mon (one,  , [us])]) fromExpWus sum (zero,  , [mon (one,  , [us])])(* fromExp (U, s) = sum
 ...
 mapMon(f,  , mon (n,  , usL)) mon (n,  , map (fun us -> whnf (f (eClo us),  , id)) usL)let rec appSum(f,  , sum (m,  , monL)) app (fun mon -> appMon (f,  , mon)) monL appMon(f,  , mon (n,  , usL)) app (fun us -> f (eClo us)) usL(* solvableSum (m + M1 + ....) =
         true iff the generalized gcd of the coefficients of the Mi
                  divides m
    *)
let rec solvableSum(sum (m,  , monL)) let let rec gcd_list(n1 :: nil) n1 gcd_list(n1 :: n2 :: nil) gcd (n1,  , n2) gcd_list(n1 :: n2 :: l) gcd (gcd (n1,  , n2),  , gcd_list l)let  inlet  in in rem (m,  , gcd_list coeffL) = zero(* findMon f (G, sum) =
         SOME(x) if f(M) = SOME(x) for some monomial M in sum
         NONE    if f(M) = NONE for all monomials M in sum
    *)
let rec findMonf(g,  , sum (m,  , monL)) let let rec findMon'(nil,  , monL2) nONE findMon'(mon :: monL1,  , monL2) (match (f (g,  , mon,  , sum (m,  , monL1 @ monL2))) with (result as sOME _) -> result nONE -> findMon' (monL1,  , mon :: monL2)) in findMon' (monL,  , nil)(* divideSum (sum, k) =
         SOME(sum') if sum is divisible by the scalar k, and sum' = sum/k
         NONE       if sum is not divisible by k
    *)
let rec divideSum(sum (m,  , monL),  , k) let exception let rec dividen if rem (n,  , k) = zero then quot (n,  , k) else raise (err)let rec divideMon(mon (n,  , usL)) mon (divide n,  , usL) in try  with (* delaySum (G, sum) = Delay (U, cnstr)
       where U the foreign expression corresponding to sum
       and cnstr is the constraint G |- sum = 0 : integer
    *)
let rec delaySum(g,  , sum) let let  inlet  in in delay (u,  , cnstr)(* unifySum (G, sum1, sum2) = result
    ...
    *)
 solveSum(g,  , sum as sum (m,  , [mon (n,  , [(x as eVar (r,  , _,  , _,  , _),  , s)])])) if isPatSub s then [assign (g,  , x,  , numberExp (~ (quot (m,  , n))),  , invert s)] else [delaySum (g,  , sum)] solveSum(g,  , sum) let let rec invertMon(g,  , mon as mon (n,  , [(eVar (r,  , _,  , _,  , _),  , s)]),  , sum) if isPatSub s then let let  inlet  in in if invertible (g,  , (rHS,  , id),  , ss,  , r) then sOME (mon,  , ss,  , sum) else nONE else nONE invertMon(g,  , mon,  , sum) nONE in match findMon invertMon (g,  , sum) with sOME (mon (n1,  , [(x1,  , s1)]),  , ss1,  , sum1) -> (match findMon invertMon (g,  , sum1) with sOME (mon (n2,  , [(x2,  , s2)]),  , ss2,  , sum2) -> let let  inlet  inlet  inlet  inlet  inlet  inlet  in in assign (g,  , x1,  , toFgn (plusSum (sum (zero,  , [mon (quot (n2,  , g),  , [(k,  , ss)])]),  , timesSum (sum (x1,  , nil),  , sum (zero,  , [mon (one,  , [(z,  , ss)])])))),  , ss1) :: assign (g,  , x2,  , toFgn (plusSum (sum (zero,  , [mon (~ (quot (n1,  , g)),  , [(k,  , ss)])]),  , timesSum (sum (x2,  , nil),  , sum (zero,  , [mon (one,  , [(z,  , ss)])])))),  , ss2) :: solveSum (g,  , plusSum (sum (zero,  , [mon (g,  , [(z,  , ss)])]),  , sum2)) nONE -> (match divideSum (sum1,  , n1) with sOME (sum1') -> [assign (g,  , x1,  , toFgn (unaryMinusSum (sum1')),  , ss1)] nONE -> [delaySum (g,  , sum)])) nONE -> [delaySum (g,  , sum)](* unifySum (G, sum1, sum2) = result
    ...
    *)
 unifySum(g,  , sum1,  , sum2) let let rec invertMon(g,  , mon (n,  , [(lHS as eVar (r,  , _,  , _,  , _),  , s)]),  , sum) if isPatSub s then let let  inlet  in in if invertible (g,  , (rHS,  , id),  , ss,  , r) then sOME (g,  , lHS,  , rHS,  , ss) else nONE else nONE in match minusSum (sum2,  , sum1) with sum (m,  , nil) -> if (m = zero) then succeed nil else fail sum -> if (solvableSum sum) then succeed (solveSum (g,  , sum)) else fail(* toFgn sum = U
