...
struct
  ...
  in
    ...
    and auxToString t (G, Trivial) = ""
      | (* GEN CASE BRANCH *) auxToString t (G, UnifyEq(G', p1, N, ga)) =
    ...
    and clauseToString t (G, Eq(p)) =
      ...
      | (* GEN CASE BRANCH *) clauseToString t (G, Assign(p, ga)) =
         (t ^ "ASSIGN  " ^ (Print.expToString (G, p)  handle _ => "<exc>")
         ^ "\n" ^ (auxToString t (G, ga)))
      | (* GEN CASE BRANCH *) clauseToString t (G, And(r, A, g)) =
         clauseToString t (IntSyn.Decl(G, IntSyn.Dec(NONE, A)), r) ^
         goalToString t (G, g)
      | (* GEN CASE BRANCH *) clauseToString t (G, In(r, A, g)) =
         let
      ...
      | (* GEN CASE BRANCH *) clauseToString t (G, Exists(D, r)) =
         let
      ...
      | (* GEN CASE BRANCH *) clauseToString t (G, Axists(D as IntSyn.ADec(SOME(n), d), r)) =
         let
           val D' = Names.decEName (G, D)
         in
           t ^ "EXISTS'  " ^
    ...
    fun subgoalsToString t (G, True) = t ^ "True "
      | (* GEN CASE BRANCH *) subgoalsToString t (G, Conjunct(Goal, A, Sg)) =
    ...
    fun conDecToString (c, SClause(r)) =
      ...
      | (* GEN CASE BRANCH *) conDecToString (c, Void) =
    ...
    fun dProgToString (DProg (IntSyn.Null, IntSyn.Null)) = ""
      | (* GEN CASE BRANCH *) dProgToString (DProg (IntSyn.Decl(G,IntSyn.Dec(SOME x,_)),
                       IntSyn.Decl(dPool, CompSyn.Dec (r,_,_)))) =
          dProgToString (DProg (G,dPool))
          ^ "\nClause    " ^ x ^ ":\n"
          ^ clauseToString "\t" (G, r)
      | (* GEN CASE BRANCH *) dProgToString (DProg (IntSyn.Decl(G, IntSyn.Dec(SOME x,A)),
