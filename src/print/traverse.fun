...
local
  ...
  *)
  fun inferConW (G, I.BVar (k')) =
    ...
    | (* GEN CASE BRANCH *) inferConW (G, I.Const(c)) = (I.constType (c), I.id)
    | (* GEN CASE BRANCH *) inferConW (G, I.Def(d))  = (I.constType (d), I.id)
    (* no case for FVar, Skonst *)

  fun fromHead (G, I.BVar(n)) = T.bvar (Names.bvarName (G, n))
    | (* GEN CASE BRANCH *) fromHead (G, I.Const(cid)) =
      let
    ...
    | (* GEN CASE BRANCH *) fromHead (G, I.Def (cid)) =
      let
        val Names.Qid (ids, id) = Names.constQid (cid)
      in
        T.def (ids, id)
  ...
  *)
  ...
  fun fromTpW (G, (I.Root (C, S), s)) =
    ...
    | (* GEN CASE BRANCH *) fromTpW (G, (I.Pi ((D as I.Dec(_,V1), I.No), V2), s)) =
        T.arrow (fromTp (G, (V1, s)),
                 fromTp (I.Decl (G, I.decSub (D, s)), (V2, I.dot1 s)))
    | (* GEN CASE BRANCH *) fromTpW (G, (I.Pi ((D, I.Maybe), V2), s)) =
      let
    ...
    | (* GEN CASE BRANCH *) fromTpW _ = raise Error ("Type not recognized")
  ...
  and fromObjW (G, (I.Root (C, S), s), (V, t)) =
        T.root (fromHead (G, C),
                fromSpine (impCon C, G, (S, s), inferConW (G, C)),
                fromTp (G, (V, t)))
    | (* GEN CASE BRANCH *) fromObjW (G, (I.Lam (D, U), s), (I.Pi (_, V), t)) =
      let
        val D' = Names.decUName (G, D)
      in
        T.lam (fromDec (G, (D', s)),
    ...
    | (* GEN CASE BRANCH *) fromObjW _ = raise Error ("Object not recognized")

  and fromObj (G, Us, Vt) = fromObjW (G, Whnf.whnf Us, Whnf.whnf Vt)

  and fromSpine (i, G, (I.Nil, s), Vt) = T.nils
    | (* GEN CASE BRANCH *) fromSpine (i, G, (I.SClo (S, s'), s), Vt) =
        fromSpine (i, G, (S, I.comp (s', s)), Vt)
    | (* GEN CASE BRANCH *) fromSpine (i, G, (I.App (U, S), s),
  ...
  fun fromConDec (I.ConDec (c, parent, i, _, V, I.Type)) =
    ...
    | (* GEN CASE BRANCH *) fromConDec _ = NONE
  ...
  fun const (name) =
      let val qid = case Names.stringToQid name
          ...
          fun getConDec (NONE) = raise Error ("Undeclared identifier " ^ Names.qidToString qid)
            | (* GEN CASE BRANCH *) getConDec (SOME cid) = IntSyn.sgnLookup cid
          val conDec = getConDec cidOpt
          val _ = Names.varReset IntSyn.Null
          fun result (NONE) = raise Error ("Wrong kind of declaration")
            | (* GEN CASE BRANCH *) result (SOME(r)) = r
      in
