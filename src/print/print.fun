...
local
  ...
  fun isNil (I.Nil) = true
    | (* GEN CASE BRANCH *) isNil (I.App _) = false
    | (* GEN CASE BRANCH *) isNil (I.SClo(S,_)) = isNil S
  ...
  *)
  ...
  fun sclo' (TooFew, s) = TooFew
    | (* GEN CASE BRANCH *) sclo' (Exact(S), s) = Exact (I.SClo(S,s))
    | (* GEN CASE BRANCH *) sclo' (TooMany (S, S'), s) = TooMany (I.SClo (S, s), I.SClo (S', s))

  fun sclo'' (TooFew, s) = TooFew
    | (* GEN CASE BRANCH *) sclo'' (Exact(S), s) = Exact (S)
    | (* GEN CASE BRANCH *) sclo'' (TooMany (S, S'), s) = TooMany (S, I.SClo (S', s))
  ...
  *)
  fun dropImp (0, S, 0) = Exact(S)
    | (* GEN CASE BRANCH *) dropImp (0, S, n) = (* n >= 1 *)
    ...
    | (* GEN CASE BRANCH *) dropImp (i, I.App(U,S), n) = dropImp (i-1, S, n)
    | (* GEN CASE BRANCH *) dropImp (i, I.SClo(S,s), n) =
        sclo' (dropImp (i, S, n), s)
    | (* GEN CASE BRANCH *) dropImp (i, I.Nil, n) = TooFew

  (* exceeded (n:int, b:bound) = true if n exceeds bound b *)
  fun exceeded (_,NONE) = false
    | (* GEN CASE BRANCH *) exceeded (n:int,SOME(m:int)) = n >= m
  ...
  *)
  ...
  fun fixityCon (I.Const(cid)) = Names.getFixity (cid)
    | (* GEN CASE BRANCH *) fixityCon (I.Skonst(cid)) = FX.Nonfix
    | (* GEN CASE BRANCH *) fixityCon (I.Def(cid)) = Names.getFixity (cid)
    | (* GEN CASE BRANCH *) fixityCon (I.NSDef(cid)) = Names.getFixity (cid)
    | (* GEN CASE BRANCH *) fixityCon _ = FX.Nonfix (* BVar, FVar *)

  (* impCon (c) = number of implicit arguments to c *)
  fun impCon (I.Const(cid)) = I.constImp (cid)
    | (* GEN CASE BRANCH *) impCon (I.Skonst(cid)) = I.constImp (cid)
    | (* GEN CASE BRANCH *) impCon (I.Def(cid)) = I.constImp (cid)
    | (* GEN CASE BRANCH *) impCon (I.NSDef(cid)) = I.constImp (cid)
    | (* GEN CASE BRANCH *) impCon _ = 0                      (* BVar, FVar *)

  (* argNumber (fixity) = number of required arguments to head with fixity *)
  fun argNumber (FX.Nonfix) = 0
    | (* GEN CASE BRANCH *) argNumber (FX.Infix _) = 2
    | (* GEN CASE BRANCH *) argNumber (FX.Prefix _) = 1
    | (* GEN CASE BRANCH *) argNumber (FX.Postfix _) = 1
  ...
  fun parmDec (D::L, 1) = D
    | (* GEN CASE BRANCH *) parmDec (D::L, j) = parmDec (L, j-1)
  ...
  fun projName (G, I.Proj (I.Bidx(k), i)) =
    ...
    | (* GEN CASE BRANCH *) projName (G, I.Proj (I.LVar(r, _, (cid, t)), i)) =
      (* note: this obscures LVar identity! *)
      (* no longer Tue Mar  1 13:32:21 2011 -cs *)
         "_"  ^ parmName (cid, i)
    | (* GEN CASE BRANCH *) projName (G, I.Proj (I.Inst iota, i)) =
       "*"    (* to be fixed --cs *)
  ...
  fun constQid (cid) =
    ...
    fun formatCids (nil) = nil
      | (* GEN CASE BRANCH *) formatCids (cid::nil) = [cidToFmt cid]
      | (* GEN CASE BRANCH *) formatCids (cid::cids) = cidToFmt cid
  ...
  *)
  fun fmtCon (G, I.BVar(n)) = Str0 (Symbol.bvar (Names.bvarName(G, n)))
    | (* GEN CASE BRANCH *) fmtCon (G, I.Const(cid)) = fmtConstPath (Symbol.const, constQid (cid))
    | (* GEN CASE BRANCH *) fmtCon (G, I.Skonst(cid)) = fmtConstPath (Symbol.skonst, constQid (cid))
    | (* GEN CASE BRANCH *) fmtCon (G, I.Def(cid)) = fmtConstPath (Symbol.def, constQid (cid))
    | (* GEN CASE BRANCH *) fmtCon (G, I.NSDef (cid)) = fmtConstPath (Symbol.def, constQid (cid))
    | (* GEN CASE BRANCH *) fmtCon (G, I.FVar (name, _, _)) = Str0 (Symbol.fvar (name))
    | (* GEN CASE BRANCH *) fmtCon (G, H as I.Proj (I.Bidx(k), i)) =
        Str0 (Symbol.const (projName (G, H)))
    | (* GEN CASE BRANCH *) fmtCon (G, H as I.Proj (I.LVar(r as ref NONE, sk, (cid, t)), i)) =
        let
          val n = lookuplvar (r)
        in
    ...
    | (* GEN CASE BRANCH *) fmtCon (G, I.FgnConst (cs, conDec)) =
        let
  ...
  *)
  ...
  fun fst (I.App (U1, _), s) = (U1, s)
    | (* GEN CASE BRANCH *) fst (I.SClo (S, s'), s) = fst (S, I.comp (s', s))

  (* snd (S, s) = U2, the second argument in S[s] *)
  fun snd (I.App (U1, S), s) = fst (S, s)
    | (* GEN CASE BRANCH *) snd (I.SClo (S, s'), s) = snd (S, I.comp (s', s))
  ...
  *)
  fun eqFix (FX.Infix(p,FX.Left), FX.Infix(p',FX.Left)) = (p = p')
    | (* GEN CASE BRANCH *) eqFix (FX.Infix(p,FX.Right), FX.Infix(p', FX.Right)) = (p = p')
      (* Infix(_,None) should never be asked *)
    | (* GEN CASE BRANCH *) eqFix (FX.Prefix(p), FX.Prefix(p')) = (p = p')
    | (* GEN CASE BRANCH *) eqFix (FX.Postfix(p), FX.Postfix(p')) = (p = p')
      (* Nonfix should never be asked *)
    | (* GEN CASE BRANCH *) eqFix _ = false
  ...
  *)
  fun addAccum (fmt, _, nil) = fmt
    | (* GEN CASE BRANCH *) addAccum (fmt, FX.Infix(_, FX.Left), accum) = F.HVbox ([fmt] @ accum)
    | (* GEN CASE BRANCH *) addAccum (fmt, FX.Infix(_, FX.Right), accum) = F.HVbox (accum @ [fmt])
    | (* GEN CASE BRANCH *) addAccum (fmt, FX.Prefix _, accum) = F.HVbox (accum @ [fmt])
    | (* GEN CASE BRANCH *) addAccum (fmt, FX.Postfix _, accum) = F.HVbox ([fmt] @ accum)
  ...
  *)
  ...
  fun fmtUni (I.Type) = sym "type"
    | (* GEN CASE BRANCH *) fmtUni (I.Kind) = sym "kind"   (* impossible, included for robustness *)
  ...
  *)
  fun fmtExpW (G, d, ctx, (I.Uni(L), s)) = aa (ctx, fmtUni(L))
    | (* GEN CASE BRANCH *) fmtExpW (G, d, ctx, (I.Pi((D as I.Dec(_,V1),P),V2), s)) =
    ...
    | (* GEN CASE BRANCH *) fmtExpW (G, d, ctx, (I.Pi((D as I.BDec _, P), V2), s)) =
      let
        val D' = Names.decLUName (G, D)
      in
    ...
    (* -bp *)
    | (* GEN CASE BRANCH *) fmtExpW (G, d, ctx, (I.Pi((D as I.ADec _, P), V2), s)) =
      let
    (*      val D' = Names.decLUName (G, D) *)
    ...
    | (* GEN CASE BRANCH *) fmtExpW (G, d, ctx, (U as I.Root R, s)) = (* s = id *)
         fmtOpArgs (G, d, ctx, opargs (G, d, R), s)
    (* I.Redex not possible *)
    | (* GEN CASE BRANCH *) fmtExpW (G, d, ctx, (I.Lam(D, U), s)) =
      let
        val D' = Names.decLUName (G, D)
      in
        fmtLevel (I.Decl (G, D'), (* I.decSub (D', s) *)
    ...
    | (* GEN CASE BRANCH *) fmtExpW (G, d, ctx, (X as I.EVar _, s)) =
    ...
    | (* GEN CASE BRANCH *) fmtExpW (G, d, ctx, (X as I.AVar _, s)) =
      (* assume dereferenced during whnf *)
      if !implicit then aa (ctx, F.HVbox (fmtAVar(G,X)::fmtSub(G,d,s)))
      else fmtOpArgs (G, d, ctx, evarArgs' (G, d, X, s), I.id)

    | (* GEN CASE BRANCH *) fmtExpW (G, d, ctx, (U as I.FgnExp csfe, s)) =
  ...
  and opargsExplicit (G, d, R as (C,S)) =
      let
        ...
        fun oe (Exact(S')) =
          ...
          | (* GEN CASE BRANCH *) oe (TooFew) = EtaLong (Whnf.etaExpandRoot (I.Root R))
          | (* GEN CASE BRANCH *) oe (TooMany (S', S'')) =
  ...
  *)
  and fmtOpArgs (G, d, ctx, oa as OpArgs(_, opFmts, S'), s) =
    ...
    | (* GEN CASE BRANCH *) fmtOpArgs (G, d, ctx, EtaLong(U'), s) =
        fmtExpW (G, d, ctx, (U', s))
  ...
  *)
  ...
  and fmtSub'' (G, d, l, I.Shift(k)) = [Str ("^" ^ Int.toString k), Str "]"]
    | (* GEN CASE BRANCH *) fmtSub'' (G, d, l, I.Dot(I.Idx(k), s)) =
        Str (Names.bvarName (G, k)) :: Str "." :: F.Break :: fmtSub' (G, d, l+1, s)
    | (* GEN CASE BRANCH *) fmtSub'' (G, d, l, I.Dot(I.Exp(U), s)) =
  ...
  *)
  and fmtSpine (G, d, l, (I.Nil, _)) = []
    | (* GEN CASE BRANCH *) fmtSpine (G, d, l, (I.SClo (S, s'), s)) =
         fmtSpine (G, d, l, (S, I.comp(s',s)))
    | (* GEN CASE BRANCH *) fmtSpine (G, d, l, (I.App(U, S), s)) =
  ...
  *)
  and fmtSpine' (G, d, l, (I.Nil, _)) = []
    | (* GEN CASE BRANCH *) fmtSpine' (G, d, l, (I.SClo (S, s'), s)) =
        fmtSpine' (G, d, l, (S, I.comp(s', s)))
    | (* GEN CASE BRANCH *) fmtSpine' (G, d, l, (S, s)) =
  ...
  *)
  and fmtLevel (G, d, Ctxt (fixity', accum, l),
    ...
    | (* GEN CASE BRANCH *) fmtLevel (G, d, Ctxt (fixity', accum, l),
  ...
  *)
  and fmtDec (G, d, (I.Dec (x, V), s)) =
      ...
      *)
    | (* GEN CASE BRANCH *) fmtDec (G, d, (I.BDec (x, (cid, t)), s)) =
      let
        val (Gsome, Gblock) = I.constBlock cid
      in
    ...
    | (* GEN CASE BRANCH *) fmtDec (G, d, (I.ADec (x, _), s)) =
      F.HVbox [Str0 (Symbol.bvar (nameOf (x))), sym ":_"]
     | (* GEN CASE BRANCH *) fmtDec (G, d, (I.NDec (SOME name), s)) =
      F.HVbox [ sym name]
     (* alternative with more whitespace *)
      (* F.HVbox [Str0 (Symbol.bvar (nameOf (x))), F.Space, sym ":", F.Break,
                  fmtExp (G, d+1, noCtxt, (V,s))]
  ...
  and fmtDecList' (G0, (nil, s)) = nil
    | (* GEN CASE BRANCH *) fmtDecList' (G0, (D::nil, s)) =
        sym "{" :: fmtDec (G0, 0, (D, s)) :: sym "}" :: nil
    | (* GEN CASE BRANCH *) fmtDecList' (G0, (D::L, s)) =
        sym "{" :: fmtDec (G0, 0, (D, s)) :: sym "}" :: F.Break
        :: fmtDecList' (I.Decl (G0, D), (L, I.dot1 s))

  fun skipI (0, G, V) = (G, V)
    | (* GEN CASE BRANCH *) skipI (i, G, I.Pi ((D, _), V)) = skipI (i-1, I.Decl (G, Names.decEName (G, D)), V)

  fun skipI2 (0, G, V, U) = (G, V, U)
    | (* GEN CASE BRANCH *) skipI2 (i, G, I.Pi ((D, _), V), I.Lam (D', U)) =
        skipI2 (i-1, I.Decl (G, Names.decEName (G, D')), V, U)

  fun ctxToDecList (I.Null, L) = L
    | (* GEN CASE BRANCH *) ctxToDecList (I.Decl (G, D), L) = ctxToDecList (G, D::L)

  fun fmtDecList (G0, nil) = nil
    | (* GEN CASE BRANCH *) fmtDecList (G0, D::nil) =
        sym"{"::fmtDec (G0, 0, (D, I.id))::sym"}"::nil
    | (* GEN CASE BRANCH *) fmtDecList (G0, D::L) =
  ...
  fun fmtBlock (I.Null, Lblock)=
    ...
    | (* GEN CASE BRANCH *) fmtBlock (Gsome, Lblock) =
  ...
  *)
  fun fmtConDec (hide, condec as I.ConDec (_, _, imp, _, V, L)) =
    ...
    | (* GEN CASE BRANCH *) fmtConDec (hide, condec as I.SkoDec (_, _, imp, V, L)) =
      let
        val qid = Names.conDecQid condec
        val _ = Names.varReset IntSyn.Null
        val (G, V) = if hide then skipI (imp, I.Null, V) else (I.Null, V)
    ...
    | (* GEN CASE BRANCH *) fmtConDec (hide, condec as I.BlockDec (_, _, Gsome, Lblock)) =
      let
        val qid = Names.conDecQid condec
        val _ = Names.varReset IntSyn.Null
      in
    ...
    | (* GEN CASE BRANCH *) fmtConDec (hide, condec as I.BlockDef (_, _, W)) =
      let
        val qid = Names.conDecQid condec
        val _ = Names.varReset IntSyn.Null
      in
    ...
    | (* GEN CASE BRANCH *) fmtConDec (hide, condec as I.ConDef (_, _, imp, U, V, L, _)) =
    ...
    *)      end
    | (* GEN CASE BRANCH *) fmtConDec (hide, condec as I.AbbrevDef (_, _, imp, U, V, L)) =
      (* reset variable names in between to align names of type V and definition U *)
      let
        val qid = Names.conDecQid condec
        val _ = Names.varReset IntSyn.Null
  ...
  fun fmtCnstr (I.Solved) = [Str "Solved Constraint"]
    | (* GEN CASE BRANCH *) fmtCnstr (I.Eqn (G, U1, U2)) =
        let
    ...
    | (* GEN CASE BRANCH *) fmtCnstr (I.FgnCnstr (csfc as (cs, _))) =
        let
  ...
  fun fmtCnstrL (nil) = [Str "Empty Constraint"]
    | (* GEN CASE BRANCH *) fmtCnstrL (ref Cnstr :: nil) = (fmtCnstr Cnstr) @ [Str "."]
    | (* GEN CASE BRANCH *) fmtCnstrL (ref Cnstr :: cnstrL) =
  ...
  *)
  fun abstractLam (I.Null, U) = U
    | (* GEN CASE BRANCH *) abstractLam (I.Decl (G, D), U) = abstractLam (G, I.Lam (D, U))
  ...
  fun fmtNamedEVar (U as I.EVar(_,G,_,_), name) =
    ...
    | (* GEN CASE BRANCH *) fmtNamedEVar (U, name) = (* used for proof term variables in queries *)
      F.HVbox [Str0 (Symbol.evar (name)), F.Space, sym "=", F.Break,
               fmtExp (I.Null, 0, noCtxt, (U, I.id))]

  fun fmtEVarInst (nil) = [Str "Empty Substitution"]
    | (* GEN CASE BRANCH *) fmtEVarInst ((U,name)::nil) = [fmtNamedEVar (U, name)]
    | (* GEN CASE BRANCH *) fmtEVarInst ((U,name)::Xs) =
  ...
  *)
  fun collectEVars (nil, Xs) = Xs
    | (* GEN CASE BRANCH *) collectEVars ((U,_)::Xnames, Xs) =
  ...
  fun mergeConstraints (nil, cnstrs2) = cnstrs2
    | (* GEN CASE BRANCH *) mergeConstraints (cnstr::cnstrs1, cnstrs2) =
  ...
  fun collectConstraints (nil) = nil
    | (* GEN CASE BRANCH *) collectConstraints (I.EVar(ref(NONE),_,_,cnstrs) :: Xs) =
        mergeConstraints (Constraints.simplify (!cnstrs),
                          collectConstraints Xs)
    | (* GEN CASE BRANCH *) collectConstraints (_ :: Xs) = collectConstraints (Xs)
